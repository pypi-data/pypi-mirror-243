...
class Symbol(_ContextAwareBase, Connection):
    ...
    @property
    def df(self, start_postion=1) -> pd.DataFrame:
        return self.get_rates_bar(ohlcv=True,start_postion=start_postion)
    ...
    @property
    def time_bars(self, start_postion=1) -> pd.Series:
        """
        last bar time for defined timeframe, by default daily
        """
        df = self.get_rates_bar(ohlcv=True,start_postion=start_postion)
    ...
    @property
    def close_bars(self,start_postion=1) -> pd.Series:
        """
    ...
    @property
    def low_bars(self, start_postion=1) -> pd.Series:
        """
    ...
    @property
    def high_bars(self, start_postion=1) -> pd.Series:
        """
        all low price for defined timeframe, by default daily
        """
        df = self.get_rates_bar(ohlcv=True,start_postion=start_postion)
    ...
    @property
    def open_bars(self, start_postion=1) -> pd.Series:
        """
    ...
    @property
    def real_volume_bars(self, start_postion=1) -> pd.Series:
        """
    ...
    @property
    def volume_bars(self, start_postion=1) -> pd.Series:
        """
    ...
    def get_rates_bar(self, ohlcv: bool = True, start_postion=1,datetime_from: Union[datetime, int] = None, datetime_to: Union[datetime, int] = None,):
        """
        This will get array of bar data, if you want dataframe use make_ohlcv function
        ...
        with self.connected:
            if self.max_bars:
                ...
                bars = copy_rates(self.name, self.timeframe,
                                  start_pos=start_postion, count=self.max_bars, datetime_from=datetime_from, datetime_to=datetime_to)
            else:
                bars = copy_rates(
