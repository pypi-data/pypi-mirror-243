...
class I2S(nn.Module):
    ...
    def forward(self, img):
        ...
        pixels_delta = [0, 1, -1, 2, -2, -5, 5]
        pixels_translations = list(itertools.product(pixels_delta, repeat=2))
        shifted_imgs = torch.concat([img.roll(shift, dims=(-2,-1)) for shift in pixels_translations])
        x = self._forward(shifted_imgs)
        grid_signal = torch.matmul(x, self.output_wigners).squeeze(1)
        grid_signal = grid_signal.reshape(batch_size, 49, grid_signal.shape[-1])

        #I am using max as aggregation function
        grid_signal = grid_signal.max(1).values
