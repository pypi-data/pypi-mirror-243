...
def test_convert_type(cloud_test_catalog):
    ...
    assert all(type(c) == float for c in converted)
    ...
    assert all(type(c) == float for c in converted[0])
    assert all(type(c) == float for c in converted[1])
