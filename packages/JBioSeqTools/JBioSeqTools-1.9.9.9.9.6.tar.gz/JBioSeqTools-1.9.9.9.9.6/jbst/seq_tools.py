...
def get_sequences_gene(gene_name, species = 'human', max_results = 20):
    ...
    while(stop == False):
        try:
            ...
            if n == 3:
                print('\Something went wrong. This query is unable to return. Try again later or contact us!')                
                return None
...
def MuscleMultipleSequenceAlignment(fasta_string, gapopen = 10, gapextend = 0.5, output = None, source = _cwd):
    ...
    try:
        ...
        if system == "Windows":
            ...
            muscle_path = os.path.join(source, 'muscle/linux/muscle')
        ...
        try:
            subprocess.check_call(muscle_cmd, cwd=muscle_path)
...
def remove_specific_to_sequence(RNAi_data, sequences, min_length = 4):
...
def predict_structure(sequence, height = None, width = None, len_factor = 0.028, show_plot = True):
...
def repair_sequences(sequence, metadata, restriction_df, enzyme_list, species):
    ...
    if len(pd.DataFrame(restriction_df)) != 0:
        ...
        if len(not_repaired) == 0:    
            print('\nRestriction places in sequence repaired...')
        else:
