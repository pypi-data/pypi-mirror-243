...
def MuscleMultipleSequenceAlignment(fasta_string, gapopen = 10, gapextend = 0.5, output = None, source = _cwd):
    ...
    try:
        ...
        if output != None: 
            ...
            random_prefix = random_name(length=30)
            output_file = os.path.join(source, 'tmp', random_prefix + '_tmp_aligment.fasta')
...
def FindRNAi(sequence, metadata, length = 23, n = 200, max_repeat_len = 3, max_off = 1, species = 'human', output = None, database_name = "refseq_select_rna",  evalue = 1e-3, outfmt =  5, word_size = 7, max_hsps = 20, reward = 1, penalty = -3, gapopen = 5, gapextend = 2, dust = "no", extension = 'xml', source = _cwd):
