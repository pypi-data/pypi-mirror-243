...
def get_sequences_gene(gene_name, species = 'human', max_results = 20):
...
def get_sequences_accesion(accesion_list):
...
def generate_fasta_string(data_dict):
...
def MuscleMultipleSequenceAlignment(fasta_string, gapopen = 10, gapextend = 0.5, output = None, source = _cwd):
    
    """
    This function conducts alignments of sequences provided in FASTA format
    
    Args:
       fasta_string (txt/FASTA) - sequences provided in FASTA format from generate_fasta_string() or loaded from external sources
       gapopen (int) -  description under the link provided below. Default: 10 
       gapextend (float) - description under the link provided below. Default:  0.5 
    ...
    try:
        ...
        input_file = os.path.join(source, random_prefix + '_tmp/tmp_fasta.fasta')
        ...
        if system == "Windows":
            ...
            muscle_executable = os.path.join(source, 'muscle/linux/muscle3.8.31_i86linux64')
        ...
        if output != None: 
            ...
            output_file = os.path.join(source, random_prefix + '_tmp/tmp_aligment.fasta')
...
def decode_aligments(alignment_file):
...
def write_fasta(fasta_file, path = None, name = 'fasta_file'):
    
    """
    This function saves into FASTA *.fasta
    
    Args:
        fasta_file (txt/FASTA) - sequences provided in FASTA format from generate_fasta_string() or loaded from external sources
        path (str) - the path to save. If None save it to the current working directory. Default: None
...
def write_aligments(decoded_aligment_file, path = None, name = 'aligments_file'):
    
    """
    This function saves into FASTA-ALIGNMENT *.align
    
    Args:
       decoded_aligment_file (txt/FASTA) - sequences provided in FASTA format from decode_aligments()
       path (str) - the path to save. If None save it to the current working directory. Default: None
       name (str) - the name of the saving file. Default: 'aligments_file'
    ...
    try:
        ...
        with open(path, 'w') as file:
            file.write(decoded_aligment_file + '\n')
...
def DisplayAligment(alignment_file, color_scheme="Taylor", wrap_length=80, show_grid=True, show_consensus=True):
...
def load_sequence():
    ...
    Returns:
        txt: Input sequence in a variable
...
def clear_sequence(sequence, upac_code:list() = ['A','C','T','G','N','M','R','W','S','Y','K','V','H','D','B','U', 'Ψ']):
...
def check_upac(sequence, upac_code:list() = ['A','C','T','G','N','M','R','W','S','Y','K','V','H','D','B','U', 'Ψ']):
...
def dna_to_rna(sequence, enrichment = False):
...
def seuqence_to_protein(sequence, metadata):
...
def FindRNAi(sequence, metadata, length = 23, n = 200, max_repeat_len = 3, max_off = 1, species = 'human', output = None, database_name = "refseq_select_rna",  evalue = 1e-3, outfmt =  5, word_size = 7, max_hsps = 0, reward = 1, penalty = -3, gapopen = 5, gapextend = 2, dust = "no", extension = 'xml', source = _cwd):
    ...
    Args:
       ...
       evalue (float) - description under the link provided below. Default: 1e-3
       outfmt (int) - description under the link provided below. Default: 5
       ...
       reward (int) - description under the link provided below. Default: 1
       penalty (int) - description under the link provided below. Default: -3
       gapopen (int) - description under the link provided below. Default: 5
       gapextend (int) - description under the link provided below. Default: 2
    ...
    try:
        ...
        if output != None: 
            ...
            output_file = os.path.join(source, random_prefix + '_tmp/tmp_blast.' + extension)
        ...
        if len(sequence) > length:
            ...
            input_file = os.path.join(source, random_prefix + '_tmp/tmp_rnai.fasta')
            with open(input_file, "w") as fasta_file:
                    fasta_file.write(fasta_string)
           
            command_list = ["cd", blast_executable, "&&", command, "-query", input_file, "-db", database_name, "-out", output_file, "-evalue", str(evalue),  "-outfmt", str(outfmt) , "-word_size" , str(word_size), "-max_hsps", str(max_hsps), "-reward" , str(reward), "-penalty", str(penalty), "-gapopen", str(gapopen), "-gapextend", str(gapextend),  "-dust",  str(dust)]        
...
def loop_complementary_adjustment(RNAi_data, loop_seq, min_length=3):
    
    
    """
    This function takes output DataFrame from Find RNAi() or remove_specific_to_sequence() reducing the RNAi score on their complementarity to the provided external genetic sequence. eg sequence after codon optimization which is not included in NCBI ref_seq db.
    ...
    Args:
       ...
       min_length (int) - min value of sequence complementary nucleotides. Default: 3
...
def remove_specific_to_sequence(RNAi_data, sequences):
    
    """
    This function takes output DataFrame from Find RNAi() or loop_complementary_adjustment() reducing the RNAi score on their complementarity to the provided loop sequence.
    It allows the choice of RNAi with better biological functionality.
    ...
    try:
        ...
        def complementary(seq1, seq2, min_length):
            complement = {'A': 'T', 'T': 'A', 'C': 'G', 'G': 'C'}
            complementary_regions = []
        ...
        for s in RNAi_data.index:
            for seq in sequences:
                comp = complementary(RNAi_data['RNAi_sense'][s], seq, 4)
                if len(comp) > 0:
                    inx_to_rm.append(s)
                    break
...
def predict_structure(sequence, height = None, width = None, len_factor = 0.028):
    ...
    Args:
       ...
       width (float) - height of the graph
...
def load_metadata(linkers = True, loops = True, regulators = True, fluorescent_tag = True, promoters = True,  polya = True, marker = True, utr5 = True, utr3 = True):
    ...
    try:
        ...
        codons = pkg_resources.resource_filename('JBioSeqTools', 'data/codons.xlsx')
        codons = pd.read_excel(codons)

        vectors = pkg_resources.resource_filename('JBioSeqTools', 'data/vectors.xlsx')
        vectors = pd.read_excel(vectors)
        
        backbone = pkg_resources.resource_filename('JBioSeqTools', 'data/backbone.xlsx')
        backbone = pd.read_excel(backbone)
        
        restriction = pkg_resources.resource_filename('JBioSeqTools', 'data/restriction_enzymes.xlsx')
        restriction = pd.read_excel(restriction)
        
        rnai = pkg_resources.resource_filename('JBioSeqTools', 'data/rnai_scoring.xlsx')
        rnai = pd.read_excel(rnai)

        capacity = pkg_resources.resource_filename('JBioSeqTools', 'data/vector_capacity.xlsx')
        ...
        if linkers == True:
            linkers = pkg_resources.resource_filename('JBioSeqTools',  'data/linkers.xlsx')
        ...
        if regulators == True:
            regulators = pkg_resources.resource_filename('JBioSeqTools', 'data/regulators.xlsx')
        ...
        if fluorescent_tag == True:
            fluorescent_tag = pkg_resources.resource_filename('JBioSeqTools',  'data/fluorescent_tag.xlsx')
        ...
        if promoters == True:
            promoters = pkg_resources.resource_filename('JBioSeqTools', 'data/promoters.xlsx')
        ...
        if polya == True: 
            polya = pkg_resources.resource_filename('JBioSeqTools', 'data/polya.xlsx')
        ...
        if marker == True: 
            marker = pkg_resources.resource_filename('JBioSeqTools', 'data/selectors.xlsx')
        ...
        if loops == True: 
            loops = pkg_resources.resource_filename('JBioSeqTools', 'data/sh_loops.xlsx')
            loops = pd.read_excel(marker)
        else:
            loops = None
        ...
        if utr5 == True: 
            utr5 = pkg_resources.resource_filename('JBioSeqTools', 'data/vector_capacity.xlsx')
        ...
        if utr3 == True: 
            ...
            utr3 = pkg_resources.resource_filename('JBioSeqTools', 'data/vector_capacity.xlsx')
        ...
        metadata = {'linkers':linkers, 'regulators':regulators, 'fluorescent_tag':fluorescent_tag, 'promoters':promoters, 'polya_seq':polya, 'selection_markers':marker, 'utr5':utr5, 'utr3':utr3, 'codons':codons, 'vectors':vectors, 'rnai':rnai, 'capacity':capacity, 'backbone':backbone, 'restriction':restriction,}
...
def codon_otymization(sequence, metadata, species):
    ...
    Args:
       ...
       metadata (dict) - set of metadata loaded vie load_metadata()
       species (str) - specie for which the gene sequence is searching (human / mouse / rat). Default: 'human'
...
def check_restriction(sequence, metadata):
...
def choose_restriction_to_remove(restriction_df, enzyme_list = []):
...
def sequence_restriction_removal(sequence, metadata, restriction_places = [], species = 'human'):
    ...
    try:
        ...
        for rp in restriction_places:
            enzyme_list += restriction_df['index'][restriction_df['name_upper'] == rp]
...
def decode_fasta_to_dataframe(fasta):
...
def extract_header_info(df_fasta):
