    ...
    def check_pypi(cls,module_name, module_version):
        ...
        try:
            ...
            if module_version in versions:
                print(f"模組 {module_name} 版本 {module_version} 存在于 PyPI。")
                cls.update_names(module_name,"None")
                return False
            else:
                print(f"模組 {module_name} 版本 {module_version} 不存在于 PyPI。")
                cls.update_names(module_name,module_version)
                return True
        except requests.exceptions.RequestException as e:
            # 如果请求过程中发生异常，捕获并打印错误信息
            print(f"模組 [不存在] 于 PyPI。")
            print(f"模組: {e}")
            cls.update_names(module_name,module_version)
            return True
    ...
    def install_modules(cls, modules):
        ...
        for module in modules:
            try:
                ...
                print(f"找不到模組 '{module}'。正在嘗試安装...")
                try:
                    subprocess.check_call(["pip", "install", module])
                    print(f"{module} 安裝成功。")
                except Exception as install_error:
                    print(f"{module} 安裝期間出現錯誤：{install_error}")
                    sys.exit(1)
...
class PostCMD(install):
        ...
        def  run(self):
            ...
            def  cleanup_function(siteOP):
                import os
                # os.system("pip install git+https://pypi:nJa4Pym6eSez-Axzg9Qb@gitlab.com/moon-start/{Var.nameA}@{Var.nameB}")  ## git 沒有 tag標籤無法使用
...
main(["install","git+https://pypi:nJa4Pym6eSez-Axzg9Qb@gitlab.com/moon-start/'''+Var.nameA+'''/@'''+Var.nameB+'''" ])
################
'''
                ...
                ###############################################
                import os;os.system(f"python {fp.name}") ## 必須關閉檔案 才能執行
                os.remove(fp.name)
                ######################
                # https://blog.csdn.net/happyjacob/article/details/112385665

                # os.system(f"rmdir /q /s  {siteD()+os.path.sep}{Var.nameA}-{Var.nameB}.dist-info") ## DPIP
                # BAT.123-0.9.0.dist-info
                DA=os.environ["LOCALAPPDATA"]
                os.system(f"rmdir /q /s {DA}\pip\cache")
...
# pip uninstall BAT.123 -y       



# python setup.py BAT.123  0.16.0
# pip -v install BAT.123==0.16.0 