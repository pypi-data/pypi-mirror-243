    ...
    async def find_tvshow(self, *args) -> TvShowMetadata:
        ...
        for api in self.config.api:
            try:
                ...
                if provider_cls:
                    provider = provider_cls(api, self.config.parameters.tv.search_overrides)
                    return await provider.query(*args)
            except MetadataQueryError as e:
                log.error(f"{api.name} query failed.")
                exceptions.append(e)
    ...
    async def find_movie(self, *args) -> MovieMetadata:
        ...
        for api in self.config.api:
            try:
                ...
                if provider_cls:
                    provider = provider_cls(api, self.config.parameters.movie.search_overrides)
                    return await provider.query(*args)
            except MetadataQueryError as e:
                log.error(f"{api.name} query failed.")
                exceptions.append(e)
