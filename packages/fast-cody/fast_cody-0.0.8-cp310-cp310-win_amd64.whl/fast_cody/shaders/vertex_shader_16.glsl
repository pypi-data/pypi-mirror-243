#version 150
 ...
 uniform mat4x3 primary_bones[16];
 uniform mat4x3 secondary_bones[16];
 ...
 in vec4 primary_weights_3;
 in vec4 primary_weights_4;

 in vec4 secondary_weights_1;
 in vec4 secondary_weights_2;
 in vec4 secondary_weights_3;
 in vec4 secondary_weights_4;
 ...
 void main()
 {
     ...
     for (int i = 0; i < 4; i++)
     {
         ...
         P += primary_weights_3[i] * primary_bones[i+8];
         P += primary_weights_4[i] * primary_bones[i+12];


         P += secondary_weights_1[i] * secondary_bones[i];
         P += secondary_weights_2[i] * secondary_bones[i+4];
         P += secondary_weights_3[i] * secondary_bones[i+8];
         P += secondary_weights_4[i] * secondary_bones[i+12];
