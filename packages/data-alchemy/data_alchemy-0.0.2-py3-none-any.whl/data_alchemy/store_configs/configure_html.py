...
from data_alchemy.ingest.cloud_storage import StorageConfiguration, StorageManager

from data_alchemy.ingest.database import DatabaseConfig, DatabaseManager
...
class HTMLConfigurer:
    ...
    def __init__(
        ...
        cloud_cofig: StorageConfiguration,
    ):
        ...
        self.cloud_store_manager = StorageManager(cloud_cofig)
    ...
    def get_cloud_storage_configuration(self) -> Optional[ExtractionConfig]:
        ...
        if self.cloud_store_manager.storage is None:
            raise ValueError("Cloud storage is not configured.")

        response = self.cloud_store_manager.storage.get(key=yaml_file_name)
    ...
    def file_exists_in_cloud_storage(self, yaml_file_name: str = None) -> bool:
        ...
        return self.cloud_store_manager.storage.file_exists(yaml_file_name)
    ...
    def write_configuration_to_cloud_storage(
        ...
        if (
            ...
            with tempfile.NamedTemporaryFile("w+") as tmp:
                ...
                self.cloud_store_manager.storage.put(
