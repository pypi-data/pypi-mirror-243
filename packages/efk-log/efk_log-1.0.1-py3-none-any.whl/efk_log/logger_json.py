...
TRACE_ID_CONTEXT = contextvars.ContextVar('trace_code')

HOST = get_host_ip()
# skip natural LogRecord attributes
# http://docs.python.org/library/logging.html#logrecord-attributes
RESERVED_ATTRS = (
    'args', 'asctime', 'asctime_s', 'created', 'exc_info', 'exc_text', 'filename',
    'funcName', 'levelname', 'levelno', 'lineno', 'module',
    'msecs', 'message', 'msg', 'name', 'pathname', 'process',
    'processName', 'relativeCreated', 'stack_info', 'thread', 'threadName', 'trace_code')


def get_trace_code():
    try:
        t = TRACE_ID_CONTEXT.get()
        return t
    except:
        TRACE_ID_CONTEXT.set(str(uuid.uuid4()))
...
class JsonFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if "trace_code" in self._required_fields:
            record.trace_code = get_trace_code()
        if "asctime" in self._required_fields:
            record.asctime = self.formatTime(record, self.datefmt)
            record.asctime_s = record.asctime[:19]
        if 'host' in self._required_fields:
            record.host = HOST
        if self.project:
            record.project = self.project

        record.logtype = 'python'
