...
def stun_test(sock, host, port, source_ip, source_port, send_data=""):
    ...
    while not recvCorr:
        ...
        tranid_match = (
            tranid.upper() == binascii.b2a_hex(buf[4:20]).decode().upper()
        )
                ...
                if attr_type == MappedAddress:
                    ...
                    ip = ".".join(
                        [
                            str(
                                int(
                                    binascii.b2a_hex(buf[base + 8 : base + 9]),
                                    16,
                                )
                            ),
                            str(
                                int(
                                    binascii.b2a_hex(buf[base + 9 : base + 10]),
                                    16,
                                )
                            ),
                            str(
...
def get_nat_type(s, source_ip, source_port, stun_host=None, stun_port=3478):
    ...
    if ret["ExternalIP"] == source_ip:
        ...
        ret = stun_test(
            s, stun_host, port, source_ip, source_port, changeRequest
        )
        if ret["Resp"]:
            typ = OpenInternet
        else:
