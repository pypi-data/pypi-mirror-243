...
def get_chromedriver_url(chromedriver_version, download_options, no_ssl=False):
    ...
    if get_major_version(
            chromedriver_version) >= "115":  # new CfT ChromeDriver versions have their URLs published, so we already have a list of options
    ...
    if platform == "linux":
        ...
        try:
            ...
            if dirs:
                ...
                dirs = [f.name for f in os.scandir("C:\\Program Files (x86)\\Google\\Chrome\\Application") if
                        f.is_dir() and re.match("^[0-9.]+$", f.name)]
...
def download_chromedriver(path: Optional[AnyStr] = None, no_ssl: bool = False):
    ...
    if not os.path.isfile(chromedriver_filepath) or not check_version(
        ...
        url = get_chromedriver_url(chromedriver_version=chromedriver_version, download_options=download_options,
                                   no_ssl=no_ssl)
        try:
            response = None
            try:
...
def get_driver_path():
    ...
    base_path = os.environ.get("PROJECT_ROOT_PATH", os.getcwd())
    dest_path = os.path.join(base_path, "build", executable_name)
    return dest_path
