...
class GeneralEigenvalues:
	...
	def approxW (self, n, verb=False):
		...
		Wn = matrix(self.W*(self.m**n), ring=QQ).row_space()
		if verb > 0:
			print("Wn = %s" % Wn)
...
def pseudo_inverse (m, verb=False):
	...
	J,P = matrix(m, ring=QQbar).jordan_form(transformation=True)
	if verb > 0:
		print(J)
		print()
	...
	return matrix(P*mi*P.inverse(), ring=QQ)
...
def subshift_eigenvalues(s, m0=None, check_primitive=True, check_periodic=True, simplify=2, verb=False):
	...
	C = coboundary_basis(s, verb=verb-1)
	if verb >= 2:
	...
	if simplify > 0:
		...
		if abs(m.det()) == 1 or vp == 1 or vp == -1:
			...
			if simplify > 1:
				...
				mm = matrix(m, ring=Integers(m.det()))
				seen = set()
				mm.set_immutable()
