...
if os.name == "nt" and sys.version_info < (3, 0):
    try:
        ...
        raise ImportError(
            "To run libsock on Windows you must install win_inet_pton")
...
class socksocket(_BaseSocket):
        ...
        if status_code != 200:
            ...
            if status_code in (400, 403, 405):
                ...
                error += ("\n[*] Note: The HTTP proxy server may not be"
                          " supported by libsock (must be a CONNECT tunnel"
                          " proxy)")
    ...
    def connect(self, dest_pair, catch_errors=None):
        ...
        if len(dest_pair) != 2 or dest_pair[0].startswith("["):
            ...
            # connection works...
            raise socket.error("libsock doesn't support IPv6: %s"
                               % str(dest_pair))
