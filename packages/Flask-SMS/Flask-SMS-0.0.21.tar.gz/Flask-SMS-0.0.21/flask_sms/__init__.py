...
class SMS(object):
    def __init__(self, app=None, **kwargs):
        ...
        if app is not None:
            ...
            self.create_redis_instance(app)
    ...
    def limit_sms_frequency(self, phone):
        redis_instance = self.app.sms_redis_instance

        # 设置限制，例如每分钟只能发送一次  秒
        limit = self.app.config.get("SMS_RATE_LIMIT")
        # 设置每天最大发送次数
        max_daily_limit = self.app.config.get("SMS_DAILY_LIMIT")
