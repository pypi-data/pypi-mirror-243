...
def get_tecton_objects(
    ...
    _maybe_load_repo_config(repo_config_path, Path(repo_root))
...
def _maybe_load_repo_config(repo_config_path: Optional[Path], repo_root: Path) -> None:
    ...
    if repo_config_path is None:
        default_repo_config = Path(repo_root) / "tecton.yaml"
        if not default_repo_config.exists():
            logger.debug("Default repo config (tecton.yaml) not found. Proceeding without a repo config.")
            return
        repo_config_path = default_repo_config
