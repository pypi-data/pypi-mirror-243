...
def SCHEMA_VALIDATION_MISSING_COLUMNS_ERROR(
    missing_columns: List[str], expected_columns: List[str]
) -> TectonValidationError:
    return TectonValidationError(
        f"Missing columns found in the dataframe schema: {', '.join(missing_columns)}. Expected schema: {', '.join(expected_columns)}"
    )


def SCHEMA_VALIDATION_EXTRANEOUS_COLUMNS_ERROR(
    extraneous_columns: List[str], expected_columns: List[str]
) -> TectonValidationError:
    return TectonValidationError(
        f"Unexpected columns found in the dataframe schema: {', '.join(extraneous_columns)}. Expected schema: {', '.join(expected_columns)}"
    )
