...
class ExplodeTimestampByTimeWindowsSparkNode(SparkExecNode):
    ...
    def _to_dataframe(self, spark: pyspark.sql.SparkSession) -> pyspark.sql.DataFrame:
        ...
        # [0] is added so that we keep the original timestamp in the exploded spine
        time_window_ns = [0] + [
            convert_timedelta_for_version(td, self.fdw.get_feature_store_format_version)
            for td in self._time_window_durations()
        ]

        new_anchor_times = F.array(*(F.col(anchor_time()) + F.lit(td) for td in time_window_ns))
        df_exploded = spine_df.withColumn(anchor_time(), F.explode(new_anchor_times)).dropDuplicates()

        return df_exploded

    def _time_window_durations(self) -> List[datetime.timedelta]:
        return [
