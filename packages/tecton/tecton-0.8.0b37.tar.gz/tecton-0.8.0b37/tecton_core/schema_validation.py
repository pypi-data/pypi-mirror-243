...
def validate_schema_arrow_table(table: pa.Table, schema: Schema) -> Optional[DiffResult]:
    expected_schema = tecton_schema_to_arrow_schema(schema)
    actual_schema = table.schema

    diff = _diff_arrow_schemas(expected_schema, actual_schema)
    if not diff:
...
def validate_schema_pandas_df(df: pd.DataFrame, schema: Schema) -> Optional[DiffResult]:
    return validate_schema_arrow_table(pa.Table.from_pandas(df), schema)
...
def _arrow_types_equal(expected: pa.DataType, actual: pa.DataType, allow_null: bool = True) -> bool:
    if isinstance(actual, pa.TimestampType) and actual.tz == "UTC":
...
def _diff_arrow_schemas(expected: pa.Schema, actual: pa.Schema) -> DiffResult:
    ...
    for column in set(expected.names) & set(actual.names):
        ...
        if _arrow_types_equal(expected_field.type, actual_field.type):
            continue
