...
then
  wasmtime \
    ...
    --dir "$(dirname $REPORT)"\
    --dir "$(dirname $TYPES)"\
    "$WASM" "$CONFIG" "$INPUT" "$REPORT" "$TYPES"
  exit 0
fi
...
# Passed to split. Reads a task from the task fifo, dumps stdin to an input file, spawns wasmtime in the background to
...
(
  ...
  wasmtime \
    ...
    --dir "$(dirname $OUTPUT)"\
    --dir "$(dirname $TYPES)"\
    "$WASM" "$CONFIG" "$INPUT" "$OUTPUT" "$TYPES"
...
# Merges individual wasmtime results
merge_sh = '''
#!/usr/bin/env bash
set -euo pipefail
...
if __name__ == "__main__":
    try:
        ...
        if process.returncode != 0:
            raise Exception(f"WASM validation error")
