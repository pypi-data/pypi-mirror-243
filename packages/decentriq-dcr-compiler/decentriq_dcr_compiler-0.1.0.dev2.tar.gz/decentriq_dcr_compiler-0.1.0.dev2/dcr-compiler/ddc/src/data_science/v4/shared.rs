...
#[derive(Debug, Clone)]
pub struct CommitCompileContextV4 {
    ...
    pub previous_commits: Vec<(v4::DataScienceCommitV4, ConfigurationCommit, DataScienceCommitMergeMetadata)>,
}

impl CommitCompileContextV4 {
    ...
    match &node.kind {
        v2::NodeKindV2::Leaf(leaf_node) => match &leaf_node.kind {
            ...
            v2::LeafNodeKindV2::Table(_) => DependencyNodeIds {
                result_node_id: crate::validation::v1::get_validation_id(&node.id),
            ...
            if table_node_ids_with_validation.contains(&node.node_id) {
                if let Some(permissions) = permissions_by_user.get_mut(&participant.user) {
                    permissions.push(Permission {
                        permission: Some(permission::Permission::RetrieveComputeResultPermission(
                            RetrieveComputeResultPermission {
                                compute_node_id: crate::validation::v1::get_validation_check_id(&node.node_id),
                            },
                        )),
                    });
                    permissions.push(Permission {
                    ...
                    permissions.push(Permission {
                        permission: Some(permission::Permission::RetrieveComputeResultPermission(
                            RetrieveComputeResultPermission {
                                compute_node_id: crate::validation::v1::get_validation_report_id(&node.node_id),
                            },
                        )),
                    });
                    permissions.push(Permission {
...
pub fn get_enclave_dependency_node_id_from_node_permissions(node: &v2::NodeV2) -> String {
    match &node.kind {
        v2::NodeKindV2::Leaf(leaf_node) => match &leaf_node.kind {
            ...
            v2::LeafNodeKindV2::Table(_) => crate::validation::v1::get_validation_id(&node.id),
        },
        v2::NodeKindV2::Computation(compute_node) => match &compute_node.kind {
