...
const PYTHON_WRAPPER_PY: &[u8] = include_bytes!("../v3/python-wrapper.py");
const MATCH_PY: &[u8] = include_bytes!("../v3/match.py");
...
pub fn add_node_configuration_elements(
    ...
    nodes_map: &HashMap<String, v2::NodeV2>,
) -> Result<(), CompileError> {
    match node.kind {
        v2::NodeKindV2::Leaf(leaf_node) => match leaf_node.kind {
            v2::LeafNodeKindV2::Raw(_) => {
            ...
            v2::LeafNodeKindV2::Table(table_leaf_node) => {
                ...
                let driver_protocol = enclave_specifications_map
                    ...
                    .worker_protocol;
                crate::validation::v3::add_nodes_for_validation(
        ...
        v2::NodeKindV2::Computation(computation_node) => match computation_node.kind {
            v2::ComputationNodeKindV2::Scripting(scripting_computation_node) => {
                ...
                let (main_script_id, additional_mount_point) = match scripting_computation_node.scripting_language {
                    ScriptingLanguage::Python => {
                        ...
                        let wrapper_script_configuration = DriverTaskConfig {
                            driver_task_config: Some(driver_task_config::DriverTaskConfig::StaticContent(
                                StaticContentConfig { content: PYTHON_WRAPPER_PY.to_vec() },
                            )),
                        };
                ...
                dependencies.push(main_script_id.clone());
                dependencies.extend(additional_scripts.iter().map(|(id, _)| id.clone()));
                let node_dependencies = scripting_computation_node
                    .dependencies
                    .into_iter()
                    .map(|id| {
                        let enclave_id = get_enclave_dependency_node_ids(&id, nodes_map)
                            .ok_or_else(|| CompileError("Node not found".to_string()))?
                            .add_to_dependencies(&mut dependencies);
                        Ok((id, enclave_id))
                    })
                ...
                let container_worker_configuration = ContainerWorkerConfiguration {
                    configuration: Some(container_worker_configuration::Configuration::Static(StaticImage {
                        ...
                        minimum_container_memory_size: scripting_computation_node.minimum_container_memory_size,
                        extra_chunk_cache_size_to_available_memory_ratio: scripting_computation_node
            ...
            v2::ComputationNodeKindV2::Match(match_node) => {
                ...
                for dependency_id in &match_node.dependencies {
                    let enclave_id = get_enclave_dependency_node_ids(dependency_id, nodes_map)
                        .ok_or_else(|| CompileError(format!("No node found for '{}'", dependency_id)))?
                        .add_to_dependencies(&mut dependency_ids);
                    let path = get_dependency_node_name(dependency_id, nodes_map)?;
                    dependencies.push(path.clone());
                    mount_points.push(MountPoint { path, dependency: enclave_id });
                }
                    ...
                    element: Some(Element::ComputeNode(ComputeNode {
                        ...
                        node: Some(compute_node::Node::Branch(ComputeNodeBranch {
                            config: DriverTaskConfig {
                                driver_task_config: Some(driver_task_config::DriverTaskConfig::StaticContent(
                                    StaticContentConfig { content: MATCH_PY.to_vec() },
                                )),
                            }
            ...
            v2::ComputationNodeKindV2::Sqlite(sqlite_computation_node) => {
                ...
                let sql_computation_configuration = v2::construct_sql_worker_configuration(
                    ...
                    &None,
                    dependencies.iter().map(|(_, _, _, table_dep)| table_dep.clone()),
                );
                ...
                for (id, enclave_id, _, _) in &dependencies {
                    let path = nodes_map
                        .get(id)
                    ...
                    node_dependencies_mount_points.push(MountPoint { path, dependency: enclave_id.clone() });
                }
                ...
                let container_worker_configuration = ContainerWorkerConfiguration {
                    configuration: Some(container_worker_configuration::Configuration::Static(StaticImage {
                        ...
                        minimum_container_memory_size: Some(2 * 1024 * 1024 * 1024),
                        extra_chunk_cache_size_to_available_memory_ratio: Some(1.0),
                    })),
                };
                let dependencies: Vec<_> = node_dependencies_mount_points
                    .iter()
                    .map(|mount_point| mount_point.dependency.to_string())
                    .chain(dependencies.iter().flat_map(|(_, _, additional, _)| additional.iter()).cloned())
                    .collect();
                configuration_elements.push(ConfigurationElement {
                    id: format!("{}_container", node.id),
                    element: Some(Element::ComputeNode(ComputeNode {
            ...
            v2::ComputationNodeKindV2::Sql(sql_computation_node) => {
                let mut table_dependency_mappings = vec![];
                let mut dependencies = vec![];
                for dependency in sql_computation_node.dependencies {
                    let dependency_id = get_enclave_dependency_node_ids(&dependency.node_id, nodes_map)
            ...
            v2::ComputationNodeKindV2::SyntheticData(synthetic_data_computation_node) => {
                    ...
                    columns: synthetic_data_computation_node
                        ...
                        .filter_map(|column| {
                            if column.should_mask_column {
                                Some(delta_synth_data_worker_api::Column {
                                    index: column.index,
                ...
                let mut dependencies = vec![];
                dependencies.push(synthetic_data_configuration_id.clone());
                ...
                let synthetic_data_dependency =
                    get_enclave_dependency_node_ids(&synth_data_computation_node_dependency, nodes_map)
                        .ok_or_else(|| {
                ...
                let container_worker_configuration = ContainerWorkerConfiguration {
                    configuration: Some(container_worker_configuration::Configuration::Static(StaticImage {
                        ...
                        mount_points: vec![
                            MountPoint { dependency: synthetic_data_configuration_id, path: "config".to_string() },
            ...
            v2::ComputationNodeKindV2::S3Sink(s3_sink_computation_node) => {
                let mut dependencies = vec![];
                let credentials_dependency =
                    get_enclave_dependency_node_ids(&s3_sink_computation_node.credentials_dependency_id, nodes_map)
                        .ok_or_else(|| {
                            CompileError(format!(
                                "No node found for '{}'",
                ...
                let upload_dependency_id =
                    get_enclave_dependency_node_ids(&s3_sink_computation_node.upload_dependency_id, nodes_map)
                        .ok_or_else(|| {
                            CompileError(format!(
                                "No node found for '{}'",
            ...
            v2::ComputationNodeKindV2::Post(post_computation_node) => {
                let mut dependencies = vec![];
                let _input_dependency_id =
                    get_enclave_dependency_node_ids(&post_computation_node.dependency, nodes_map)
                        .ok_or_else(|| {
...
#[derive(Debug, Clone)]
pub struct CommitCompileContextV5 {
    pub nodes_map: HashMap<String, v2::NodeV2>,
    ...
    pub previous_commits: Vec<(v5::DataScienceCommitV5, ConfigurationCommit, DataScienceCommitMergeMetadata)>,
}

impl CommitCompileContextV5 {
...
// TODO
pub struct DependencyNodeIds {
    // The node id with the actual result content.
    result_node_id: String,
    // Additional node ids that should be depended on by downstream nodes, but do not have data.
    // Example would be validation nodes or perhaps export nodes.
    additional_dependency_node_ids: Vec<String>,
}

impl DependencyNodeIds {
    fn only_result(result_node_id: String) -> Self {
        Self { result_node_id, additional_dependency_node_ids: Vec::with_capacity(0) }
    }

    // Convenience function, adds all nodes to dependencies and returns the result node id.
    fn add_to_dependencies(self, dependencies: &mut Vec<String>) -> String {
        dependencies.push(self.result_node_id.clone());
        dependencies.extend(self.additional_dependency_node_ids);
        self.result_node_id
    }
}

pub fn get_enclave_dependency_node_ids(
    dependency_id: &String,
    nodes_map: &HashMap<String, v2::NodeV2>,
) -> Option<DependencyNodeIds> {
    nodes_map.get(dependency_id).map(get_enclave_dependency_node_id_from_node)
}

pub fn get_enclave_dependency_node_id_from_node(node: &v2::NodeV2) -> DependencyNodeIds {
    match &node.kind {
        v2::NodeKindV2::Leaf(leaf_node) => match &leaf_node.kind {
            v2::LeafNodeKindV2::Raw(_) => DependencyNodeIds::only_result(node.id.clone()),
            v2::LeafNodeKindV2::Table(_) => DependencyNodeIds {
                result_node_id: crate::validation::v3::get_validation_id(&node.id),
                additional_dependency_node_ids: vec![crate::validation::v1::get_validation_check_id(&node.id)],
            },
        },
        v2::NodeKindV2::Computation(compute_node) => match &compute_node.kind {
            v2::ComputationNodeKindV2::Sql(_) => DependencyNodeIds::only_result(node.id.clone()),
            v2::ComputationNodeKindV2::Sqlite(_) => {
                let container_id = format!("{}_container", node.id);
                DependencyNodeIds::only_result(container_id)
            }
            v2::ComputationNodeKindV2::Scripting(_) => {
                let container_id = format!("{}_container", node.id);
                DependencyNodeIds::only_result(container_id)
            }
            v2::ComputationNodeKindV2::SyntheticData(_) => {
                let container_id = format!("{}_container", node.id);
                DependencyNodeIds::only_result(container_id)
            }
            v2::ComputationNodeKindV2::S3Sink(_) => DependencyNodeIds::only_result(node.id.clone()),
            v2::ComputationNodeKindV2::Match(_) => {
                let container_id = format!("{}_match_node", node.id);
                DependencyNodeIds::only_result(container_id)
            }
            v2::ComputationNodeKindV2::Post(_) => DependencyNodeIds::only_result(node.id.clone()),
        },
    }
}
...
pub fn construct_table_dependency_mappings(
    ...
    nodes_map: &HashMap<String, v2::NodeV2>,
) -> Result<Vec<(String, String, Vec<String>, TableDependencyMapping)>, CompileError> {
    let mut dependencies = vec![];
    for dependency in table_mappings {
        let enclave_dependency_id = get_enclave_dependency_node_ids(&dependency.node_id, nodes_map)
            .ok_or_else(|| CompileError(format!("No node found for '{}'", &dependency.node_id)))?;
        dependencies.push((
            dependency.node_id.clone(),
            enclave_dependency_id.result_node_id.clone(),
            enclave_dependency_id.additional_dependency_node_ids,
            TableDependencyMapping {
                table: dependency.table_name.clone(),
                dependency: enclave_dependency_id.result_node_id,
            },
        ))
    }
    Ok(dependencies)
            ...
            ParticipantPermission::DataOwner(data_owner_perm) => {
                permissions.push(Permission {
                    permission: Some(permission::Permission::LeafCrudPermission(LeafCrudPermission {
                        leaf_node_id: v2::get_enclave_leaf_node_id(&data_owner_perm.node_id, nodes_map)
...
pub fn add_permissions_for_validation_report(
    ...
    let table_node_ids_with_validation: HashSet<&String> = nodes_map
        ...
        .filter_map(|(node_id, node)| {
            match &node.kind {
                v2::NodeKindV2::Leaf(leaf) => match &leaf.kind {
                    v2::LeafNodeKindV2::Raw(_) => (),
                    v2::LeafNodeKindV2::Table(_) => {
                        return Some(node_id);
                    }
                },
                v2::NodeKindV2::Computation(_) => (),
            };
            None
        })
        .collect();
            ...
            if table_node_ids_with_validation.contains(&node.node_id) {
                if let Some(permissions) = permissions_by_user.get_mut(&participant.user) {
                    permissions.push(Permission {
                        permission: Some(permission::Permission::RetrieveComputeResultPermission(
                            RetrieveComputeResultPermission {
                                compute_node_id: crate::validation::v3::get_validation_check_id(&node.node_id),
                            },
                        )),
                    });
                    permissions.push(Permission {
                    ...
                    permissions.push(Permission {
                        permission: Some(permission::Permission::RetrieveComputeResultPermission(
                            RetrieveComputeResultPermission {
                                compute_node_id: crate::validation::v3::get_validation_report_id(&node.node_id),
                            },
                        )),
                    });
                    permissions.push(Permission {
...
pub fn get_enclave_dependency_node_id_from_node_permissions(node: &v2::NodeV2) -> String {
    match &node.kind {
        v2::NodeKindV2::Leaf(leaf_node) => match &leaf_node.kind {
            v2::LeafNodeKindV2::Raw(_) => node.id.clone(),
            v2::LeafNodeKindV2::Table(_) => crate::validation::v3::get_validation_id(&node.id),
        },
        v2::NodeKindV2::Computation(compute_node) => match &compute_node.kind {
            v2::ComputationNodeKindV2::Sql(_) => node.id.clone(),
            v2::ComputationNodeKindV2::Sqlite(_) => {
                let container_id = format!("{}_container", node.id);
                container_id
            }
            ...
            v2::ComputationNodeKindV2::S3Sink(_) => node.id.clone(),
            v2::ComputationNodeKindV2::Match(_) => {
                let container_id = format!("{}_match_filter_node", node.id);
                container_id
            }
            v2::ComputationNodeKindV2::Post(_) => node.id.clone(),
        },
    }
}
