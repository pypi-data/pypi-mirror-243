...
pub type ActivationTypeV1 = v0::ActivationType;
pub type EnclaveSpecificationV1 = v0::EnclaveSpecification;
pub type MediaDataRoomConfigV1 = v0::MediaDataRoomConfig;
pub type MediaAuxiliaryStateV1 = v0::MediaAuxiliaryStateV0;
pub type DirectActivationConfigV1 = v0::DirectActivationConfigV0;
...
pub const OVERLAP_BASIC_PY: &[u8] = include_bytes!("../v0/overlap_basic.py");
pub const DIRECT_ACTIVATION_ALL_PY: &[u8] = include_bytes!("../v0/direct_activation_all.py");
pub const DIRECT_ACTIVATION_PY: &[u8] = include_bytes!("../v1/direct_activation.py");
pub const CONSENTLESS_OVERLAP_INSIGHTS_PY: &[u8] = include_bytes!("../v0/consentless_overlap_insights.py");

#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub enum ColumnTypeV1 {
    ActivationId,
    MatchingId,
    AudienceType,
    Segment,
...
struct MediaDataRoomCompilerV1<'a> {
    data_room: &'a MediaDataRoomV1,
...
pub fn compile_media_data_room_v1(data_room: &MediaDataRoomV1) -> Result<DataRoom, CompileError> {
    MediaDataRoomCompilerV1::new(data_room).compile()
}

fn data_room_to_data_room_config(data_room: &MediaDataRoomV1) -> CompileResult<MediaDataRoomConfigV1> {
...
impl<'a> MediaDataRoomCompilerV1<'a> {
    pub fn new(data_room: &'a MediaDataRoomV1) -> Self {
    ...
    pub fn compile(mut self) -> CompileResult<DataRoom> {
        ...
        match &self.data_room.activation_type {
            None => {}
            Some(ActivationTypeV1::Direct) => {
                self.add_direct_activation_nodes()?;
            }
            Some(ActivationTypeV1::Consentless) => {
...
pub fn convert_data_room_v0_to_v1(data_room: v0::MediaDataRoomV0) -> CompileResult<MediaDataRoomV1> {
    fn convert_schema(schema: Vec<v0::ColumnType>) -> Vec<ColumnTypeV1> {
        schema
            .into_iter()
            .map(|column_type| match column_type {
                v0::ColumnType::ActivationId => ColumnTypeV1::ActivationId,
                v0::ColumnType::MatchingId(_) => ColumnTypeV1::MatchingId,
                v0::ColumnType::AudienceType => ColumnTypeV1::AudienceType,
                v0::ColumnType::Segment => ColumnTypeV1::Segment,
            })
            .collect()
    }
    ...
    Ok(MediaDataRoomV1 {
        id: data_room.id,
        name: data_room.name,
        main_publisher_email: data_room.publisher_email.clone(),
        main_advertiser_email: data_room.advertiser_email.clone(),
        publisher_emails: vec![data_room.publisher_email],
        advertiser_emails: vec![data_room.advertiser_email],
        observer_emails: data_room.observer_user.into_iter().collect(),
        agency_emails: vec![],
        publisher_schema: convert_schema(data_room.publisher_schema),
        advertiser_schema: convert_schema(data_room.advertiser_schema),
