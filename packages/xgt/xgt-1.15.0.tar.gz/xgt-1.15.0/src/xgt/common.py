...
def _create_flight_ticket(name, offset, length, columns = None,
...
def _get_data_pandas_from_table(arrow_table, schema, conn):
  ...
  for i, (name, col) in enumerate(zip(arrow_table.column_names, arrow_table)):
    if i < len(schema) and schema[i][1] == IPADDRESS:
    ...
    elif i < len(schema) and schema[i][1] == ROWID:
      pandas_dict[name] = \
      ...
      pandas_dict[name] = pd.Series([_convert_ip_address_list_pandas(x.as_py(),
                                                                     depth)
                                     for x in col])
    elif i < len(schema) and schema[i][1] == LIST and schema[i][2] == ROWID:
      depth = 1 if len(schema[i]) == 3 else schema[i][3]
      pandas_dict[name] = \
