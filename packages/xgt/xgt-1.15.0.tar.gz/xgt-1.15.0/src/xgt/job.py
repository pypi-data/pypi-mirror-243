...
class Job(object):
  ...
  # returned.
  def _load_arrow_table(self, offset, length, columns):
  ...
  def get_data(self, offset=0, length=None, columns=None, format='python'):
    """
    ...
    columns : list of int or str
      The columns to retrieve.  Given as a mixed list of column positions and
      schema column names.  A value of 'None' means all columns.
      Optional. Default=None.
    ...
    columns = _validated_columns(columns, self._data['schema'])
    ...
    if lc_format == 'python':
      return _get_data_python_from_table(
                 self._load_arrow_table(offset, length, columns),
      ...
      return self._load_arrow_table(offset, length, columns)
    else:
  ...
  def get_data_arrow(self, offset=0, length=None):
    ...
    return self._load_arrow_table(offset, length, None)
  ...
  def get_data_pandas(self, offset=0, length=None):
    ...
    return _get_data_pandas_from_table(
               self._load_arrow_table(offset, length, None),
               self._data['schema'], self._conn)
