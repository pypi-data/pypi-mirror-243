...
from .common import (_assert_isstring, _assert_noerrors,
                     ...
                     MAX_PACKET_SIZE, BOOLEAN, INT, UINT, FLOAT, DATE,
                     TIME, DATETIME, IPADDRESS, TEXT, DURATION, ROWID, LIST,
...
class TableFrame(object):
  ...
  def modify_columns(self, new_columns):
    """
  ...
  def load(self, paths,
           ...
           column_mapping = None, suppress_errors = False, row_filter = None,
  ...
  def _load(self, paths, header_mode = HeaderMode.NONE, record_history = True,
    ...
    if header_mode not in HeaderMode._all:
      raise TypeError(f'Invalid header mode: "{header_mode}"')
    ...
    if row_filter is not None and not isinstance(row_filter, str):
      raise TypeError("Input filter must be a string representing a TQL "
                      "fragment.")
  ...
  def insert(self, data, row_labels = None, row_label_columns = None,
             column_mapping = None, suppress_errors = False, row_filter = None,
             frame_to_input_column_mapping = None, input_filter = None):
    """
    ...
    if input_filter is not None:
      _deprecated('input_filter', 'row_filter')
      if row_filter is not None:
        raise ValueError('Cannot pass the row filter in both the '
                         '"row_filter" and "input_filter" parameters. '
                         '"input_filter" is deprecated.')
  ...
  def get_data(self, offset = 0, length = None, columns = None,
               format = 'python', include_row_labels = False,
               row_label_column_header = None, duration_as_interval = False,
               row_filter = None):
    """
    ...
    if lc_format == 'python':
      return self._get_data(schema, offset, length, columns, include_row_labels,
                            row_filter = row_filter)
    elif lc_format == 'pandas':
      ticket = _create_flight_ticket(
                 self._name, offset, length, columns = columns,
                 include_row_labels = include_row_labels,
                 row_label_column_header = row_label_column_header,
                 row_filter = row_filter)
  ...
  def _get_data(self, schema, offset = 0, length = None, columns = None,
                include_row_labels = False, validation_id = None,
                row_filter = None):
    ticket = _create_flight_ticket(
               self._name, offset, length, columns = columns,
               include_row_labels = include_row_labels,
...
class VertexFrame(TableFrame):
  ...
  def load(self, paths,
           ...
           row_labels = None, row_label_columns = None, delimiter = ',',
           column_mapping = None, suppress_errors = False, row_filter = None,
           on_duplicate_keys = 'error', frame_to_input_column_mapping = None,
           input_filter = None):
    """
    Loads data from one or more files specified in the list of paths.  These
    files may be CSV, Parquet, or compressed CSV.  Some limitations exist for
  ...
  def insert(self, data, row_labels = None,
             row_label_columns = None, column_mapping = None,
