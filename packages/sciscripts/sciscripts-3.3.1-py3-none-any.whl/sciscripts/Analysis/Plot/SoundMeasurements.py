...
from sciscripts.IO import Bin, DAqs
plt = Plot.plt
...
def PxxSp_Freq(SoundIntensity, PSD, CalibrationPath, Intensities=[80, 70, 60, 50, 40, 30, 0], Ax=None, AxArgs={}, File='PowerSpectrumDensity_AllFreqs', Ext=['svg'], Save=False, Show=True):

    Colormaps = [
        plt.get_cmap('Reds'), plt.get_cmap('Greens'), plt.get_cmap('Blues'),
        plt.get_cmap('Purples'), plt.get_cmap('Oranges'), plt.get_cmap('Greys'),
        plt.get_cmap('Blues'), plt.get_cmap('Greens')
    ]
...
def All(CalibrationPath, FigPath=None, Ext=['svg'], Save=False, Show=True):
    SoundIntensity = Bin.Read(f"{CalibrationPath}/SoundIntensity")[0]
    PSD = Bin.Read(f"{CalibrationPath}/PSD")[0]
    if not FigPath: FigPath = CalibrationPath

    Intensity_AmpF(SoundIntensity, File=FigPath+'/IntensityCurve-Freq_AmpF', Ext=Ext, Save=Save, Show=Show)
    PxxSp_Freq(SoundIntensity, PSD, CalibrationPath, File=FigPath+'/PowerSpectrumDensity_AllFreqs', Ext=Ext, Save=Save, Show=Show)
