...
print('[IO.Klusta] Loading dependencies...')
import os
...
def Run(PrmFile, Path, Overwrite=False, Klusta=os.environ['HOME']+'/.local/bin/klusta'):
...
def Phy_Run(KwikFile, Path, Phy=os.environ['HOME']+'/.local/bin/phy'):
    Here = os.getcwd(); os.chdir(Path)
