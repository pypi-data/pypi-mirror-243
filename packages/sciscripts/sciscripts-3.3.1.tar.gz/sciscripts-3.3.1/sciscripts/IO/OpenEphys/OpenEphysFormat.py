...
def load(filepath, dtype = float):
    """
    Redirects to code for loading individual file types.
    """
...
def loadFolderToArray(
        folderpath, channels = 'all', chprefix = 'CH',
        dtype = float, session = '0', source = '100'
    ):
    '''
    Load continuous files in specified folder to a single numpy array.

    By default all CH continous files are loaded in numerical order,
    ordering can be specified with optional channels argument which
    should be a list of channel numbers.
    '''
...
def pack(folderpath,source='100',**kwargs):
    """
    Convert single channel open ephys channels to a .dat file for
    compatibility with the KlustaSuite, Neuroscope and Klusters.

    Should not be necessary for versions of open ephys which write data
    into HDF5 format.

    Loads .continuous files in the specified folder and saves a .DAT in
    that folder.

    Optional arguments:
       source: string name of the source that openephys uses as the
       prefix. is usually 100, if the headstage is the first source
       added, but can specify something different.

       data: pre-loaded data to be packed into a .DAT.

       dref: int specifying a channel # to use as a digital reference.
       Is subtracted from all channels.

       order: the order in which the .continuous files are packed into
       the .DAT. should be a list of .continuous channel numbers.
       length must equal total channels.

       suffix: appended to .DAT filename, which is openephys.DAT if no
       suffix provided.
   """
...
def pack_2(folderpath, filename = '', channels = 'all', chprefix = 'CH',
    ...
    '''
    Alternative version of pack which uses numpy's tofile function to write
    data.  pack_2 is much faster than pack and avoids quantization noise
    incurred in pack due to conversion of data to float voltages during
    loadContinous followed by rounding back to integers for packing.

    filename: Name of the output file. By default, it follows the same layout
    of continuous files, but without the channel number, for example,
    '100_CHs_3.dat' or '100_ADCs.dat'.

    channels:  List of channel numbers specifying order in which channels are
    packed. By default all CH continous files are packed in numerical order.

    chprefix:  String name that defines if channels from headstage, auxiliary
    or ADC inputs will be loaded.

    dref:  Digital referencing - either supply a channel number or 'ave' to
    reference to the average of packed channels.

    source: String name of the source that openephys uses as the prefix. It is
    usually 100, if the headstage is the first source added, but can specify
    something different.
    '''
