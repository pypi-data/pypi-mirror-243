...
from . import run_tests_module as rtu
...
def show_cmd(ctx, **kwargs):
    ...
    nodes, dependencies = mu.get_nodes_and_dependencies()
...
def merge_cmd(ctx, **kwargs):
    ...
    nodes, dependencies = mu.get_nodes_and_dependencies(default_seed=existing_node_ids[0] if ctx.params['default_seed'] is None else ctx.params['default_seed'])
...
def delete_cmd(ctx, **kwargs):
    ...
    nodes, _ = mu.get_nodes_and_dependencies()
...
def update_sql_cmd(ctx, **kwargs):
    ...
    for node_str in nodes:
        ...
        try:
            ...
            transform_body.view.operator.spark_function.executable.code.source.inline = tu.translate_sql(mu.get_compiled_sql(node_str=node_str), input_ids)
...
def create_component_cmd(ctx, **kwargs):
    ...
    if ctx.params['with_tests']:
        ...
        # Get test data
        test_data, nodes_with_test_data = rtu._get_tests_and_nodes()
