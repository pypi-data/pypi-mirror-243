...
def create_transform(
    ...
    prev_sql = sql
    sql = translate_sql(sql, input_ids)
        ...
        operator=operator.Operator(
            spark_function=operator.Spark.Function(
                executable=io.Executable(
                    code=io.Code(
                        language=get_language(dataplane_type),
...
def get_language(type):
    ...
    raise Exception(f"Dataplane type {type} not supported.")
...
def translate_sql(sql, input_ids):
    """
