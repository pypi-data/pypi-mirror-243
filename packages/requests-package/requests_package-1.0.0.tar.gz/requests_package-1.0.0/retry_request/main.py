...
class RetryRequest:
    ...
    def __init__(self, retries = 70, sleep_time_sec = 5) -> None:
        self.retries = retries
        self.sleep_time_sec = sleep_time_sec

    def _request(self, url, method, logs=False, **kwargs) -> Response:
        for index in range(1, self.retries+1):
            if logs:
                logging.info(f"[RequestRetry] endpoint: {url} args: {kwargs} attempt: {index}")
            response: Response = method(url=url, **kwargs)
            if response.status_code not in [429, 504]:
                break
            time.sleep(self.sleep_time_sec)
        if logs:
            logging.error(f"[RequestRetry] Error Response: {response.status_code} - {response.content}")
        return response

    def get(self, url, logs=False, **kwargs) -> Response:
        method = requests.get
        return self._request(url, method, logs, **kwargs)

    def post(self, url, logs=False, **kwargs) -> Response:
        method = requests.post
        return self._request(url, method, logs, **kwargs)

    def put(self, url, logs=False, **kwargs) -> Response:
        method = requests.put
        return self._request(url, method, logs, **kwargs)

    def delete(self, url, logs=False, **kwargs) -> Response:
