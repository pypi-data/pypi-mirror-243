...
@patch("requests_package.main.requests.get")
def test_get_423(mock_get: Mock):
    mock_get.return_value.status_code = 423
    response = RetryRequest(sleep_time_sec=0, retries=10).get("url", params="mock_params", headers="mock_headers")
    assert mock_get.call_count == 10
...
def test_post_504(mock_post: Mock):
    ...
    response = RetryRequest(sleep_time_sec=0, retries=10).post(
        "url", json="json_mock", params="mock_params", headers="mock_headers"
    )
...
@patch("requests_package.main.requests.post")
def test_post_423(mock_post: Mock):
...
def test_put_504(mock_put: Mock):
    ...
    response = RetryRequest(sleep_time_sec=0, retries=10).put(
        "url", json="json_mock", params="mock_params", headers="mock_headers"
    )
...
@patch("requests_package.main.requests.put")
def test_put_423(mock_put: Mock):
    mock_put.return_value.status_code = 423
    response = RetryRequest(sleep_time_sec=0, retries=10).put(
        "url", json="json_mock", params="mock_params", headers="mock_headers"
    )
    assert mock_put.call_count == 10
...
def test_delete_504(mock_delete: Mock):
    ...
    response = RetryRequest(sleep_time_sec=0, retries=10).delete(
        "url", json="json_mock", params="mock_params", headers="mock_headers"
    )
...
@patch("requests_package.main.requests.delete")
def test_delete_423(mock_delete: Mock):
    mock_delete.return_value.status_code = 423
    response = RetryRequest(sleep_time_sec=0, retries=10).delete(
        "url", json="json_mock", params="mock_params", headers="mock_headers"
    )
    assert mock_delete.call_count == 10
