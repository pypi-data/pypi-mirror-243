...
class VeloxGrid(BasePageObject):
    ...
    def set_value(self, row: int, column_name: str, value: Optional[str]):
        ...
        actions = ActionChains(self.sapio_driver.selenium_driver)
        actions.key_down(Keys.CONTROL).send_keys("A").key_up(Keys.CONTROL).pause(.25).send_keys(Keys.BACKSPACE)
        actions.perform()
        self.sapio_driver.wait_seconds(.25)
    ...
    def get_cell_value(self, row: int, column_name: str) -> Optional[str]:
        ...
        cell_el = self.sapio_driver.get_element_at_point(cell_rect.x, cell_rect.y + 5)
        return cell_el.text
    ...
    def first_row_number(self) -> int:
        """
        Return the number of the first row in the grid. My not necessarily be 1 if the grid is from a paged table.
        """
        self.scroll_to_top()
        self.sapio_driver.wait_seconds(.2)
