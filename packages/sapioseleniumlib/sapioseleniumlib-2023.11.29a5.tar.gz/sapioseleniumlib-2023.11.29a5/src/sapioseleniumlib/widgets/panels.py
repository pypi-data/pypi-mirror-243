...
class ProcessQueue(BasePageObject):
    ...
    def processes(self) -> List[WebElement]:
        ret: List[WebElement] = self.wait_for_many(
            lambda d: self.body.find_elements(By.XPATH, "./div[1]/div/div/div/div/div/div")
        )
        return [x for x in ret if x.get_attribute("data-processrecordid")]
    ...
    def start_process_step(self, process_name: str, step_id: str | int, scan_values: list[str],
                           scan_type: str = "None", wait_for_eln_load: bool = True) -> None:
        """
        ...
        if scan_type.lower() == "samples":
            self.click_sample_buttons()
        elif scan_type.lower() == "plates":
            self.click_plates_button()
        elif scan_type.lower() != "none":
    ...
    def click_process(self, process_name):
        ...
        # if num of processes is = 1, it's already automatically expanded now.
        if len(self.processes) > 1:
            self.sapio_driver.click(process_ele)

    def scan_values(self, values: List[str]) -> None:
    ...
    def _process_x_path(self, process_name: str) -> str:
        return "./div[1]/div/div/div/div/div/div[div[2]" + self.sapio_driver.x_path_ci_text_equals(process_name) + "]"
