...
def limiter(limit_value: str = '1/second', callback=None):
    """https://github.com/alisaifee/limits"""
    from limits import storage, strategies, parse
    memory_storage = storage.MemoryStorage()
    _limiter = strategies.FixedWindowRateLimiter(memory_storage)
    item = parse(limit_value)

    @wrapt.decorator
    def inner(wrapped, instance, args, kwargs):
        if _limiter.hit(item, *args):
            return wrapped(*args, **kwargs)
        else:
            if callback:
                callback()
            else:
                logger.debug(f"Rate limited: {item}")
