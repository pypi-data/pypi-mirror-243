...
class ganada:
	...
	def data_for_selection(self):
		...
		self.var_word["end_line_no_for_selection"] = self.end_line_no_of_selection()
		self.var_word["word_nos_in_selection"] = self.selection.Words.Count
		self.var_word["start_word_no_in_selection"] = self.start_word_no_of_selection()
		...
		self.var_word["start_para_no_for_selection"] = self.start_para_no_of_selection()
		...
		self.var_word["selection"]["end_line_no"] = self.end_line_no_of_selection()
		self.var_word["selection"]["start_word_no"] = self.start_line_no_of_selection()
		self.var_word["selection"]["start_char_no"] = self.selection.Start
		self.var_word["selection"]["end_char_no"] = self.selection.End
		self.var_word["selection"]["start_para_no"] = self.start_para_no_of_selection()
	...
	def delete_line_by_no(self, input_line_no=1):
		...
		:return:
		"""
		self.select_nth_line_from_begin_of_doc(input_line_no)
		self.word_program.Selection.range.Text = ""
	...
	def end_char_no_of_selection(self):
		start_char = self.get_cursor_position()
		return start_char


	def end_line_no_of_selection(self):
		"""
		선택한 영역의 마지막 줄번호
	...
	def end_page_no_of_selection(self):
		end_page = self.word_program.Selection.Range.Information(3)
		return end_page

	def end_para_no_of_selection(self):
		"""
		선택영역에서 마지막 문단번호
		형태적인 분류 - active_doc(화일) > sentence(문장) > word(한 단어) > character(한글자)
		의미적인 분류 - active_doc(화일) > paragraph(문단) > line(줄) > word(한 단어) > character(한글자)
		...
		:return:
		"""
		start_para_no = self.start_para_no_of_selection()
		count_para = self.selection.Paragraphs.Count
		result = start_para_no + count_para - 1
		return result

	def end_word_no_of_selection(self):
		"""
	...
	def insert_new_para_with_properties(self, input_text, size=14, font="Arial", align="right", bold=True,
		...
		para_no = self.start_para_no_of_selection()
		self.select_nth_para_from_begin_of_doc(para_no - 1)
	...
	def move_cursor_to_nth_char_from_begin_of_doc(self, x_char=1):
		"""
	...
	def move_cursor_to_next_nth_line_from_selection(self, input_no=1):
		"""
	...
	def move_cursor_to_next_nth_word_form_selection(self, input_index):
		"""
	...
	def move_cursor_to_next_nth_char_from_selection(self, input_no=1):
		"""
	...
	def move_cursor_to_next_nth_para_from_selection(self, input_no):
		"""
		...
		:return:
		"""
		self.select_next_nth_para_from_selection(input_no)
		self.selection.MoveLeft()

	def move_cursor_to_next_nth_word_from_selection(self, input_no):
		"""
		...
		:return:
		"""
		self.select_next_nth_word_from_selection(input_no)
		self.selection.MoveLeft()
	...
	def move_cursor_to_previous_nth_char_from_selection(self, input_no):
		...
		:return:
		"""
		self.move_cursor_to_next_nth_char_from_selection(-1 * input_no)
	...
	def move_cursor_to_previous_nth_line_from_selection(self, input_no):
		...
		:return:
		"""
		self.move_cursor_to_next_nth_line_from_selection(-1 * input_no)
	...
	def move_cursor_to_previous_nth_para_from_selection(self, input_no):
		...
		:return:
		"""
		self.move_cursor_to_next_nth_para_from_selection(-1 * input_no)
	...
	def move_cursor_to_previous_nth_word_from_selection(self, input_no):
		...
		:return:
		"""
		self.move_cursor_to_next_nth_word_from_selection(-1 * input_no)
	...
	def select_from_begin_of_doc_to_next_nth_char(self, line_len=3):
		self.select_multi_selection_basic(1, line_len, "character")

	def select_from_begin_of_doc_to_next_nth_line(self, line_len=3):
		self.select_multi_selection_basic(1, line_len, "line")

	def select_from_begin_of_doc_to_next_nth_para(self, line_len=3):
		self.select_multi_selection_basic(1, line_len, "paragraph")

	def select_from_begin_of_doc_to_next_nth_word(self, line_len=3):
	...
	def select_from_cursor_to_next_nth_char(self, line_no_begin=1, line_len=3):
		self.select_multi_selection_basic(line_no_begin, line_len, "character")

	def select_from_cursor_to_next_nth_sentense(self, line_no_begin=1, line_len=3):
		self.select_multi_selection_basic(line_no_begin, line_len, "sentence")

	def select_from_cursor_to_next_nth_word(self, line_no_begin=1, line_len=3):
		self.select_multi_selection_basic(line_no_begin, line_len, "word")

	def select_from_cursor_to_previous_nth_char(self, input_no):
		line_no = self.start_word_no_of_selection()
	...
	def select_next_nth_line_from_cursor(self, line_no):
		"""
		(라인 선택) 전체 문서에서 줄수로 선택하는것
	...
	def select_next_nth_line_from_selection(self, line_no=1):
		"""
		(라인 선택) 전체 문서에서 줄수로 선택하는것
	...
	def select_next_nth_sentence_from_selection(self, input_no):
		"""
	...
	def select_nth_para_from_selection(self, input_no=1):
		"""
	...
	def select_nth_word_from_begin_of_doc(self, line_no):
		"""
		(라인 선택) 문서의 처음부터 원하는 n번째 라인을 선택
	...
	def select_one_word_by_no_from_begin_of_doc(self, input_no=1):
		"""
	...
	def select_some_char_in_line(self, line_no, start_no, count_no):
		"""
	...
	def select_some_char_in_para(self, para_no, y, length):
		"""
	...
	def selection_information(self):
		...
		start_word = self.start_char_no_of_selection()
		print("선택영역의 단어의 시작번호 ==> ", start_word)
		start_line = self.start_line_no_of_selection()
	...
	def start_char_no_of_selection(self):
		start_char = self.get_cursor_position()
		len_char = self.selection.Range.ComputeStatistics(3)
		return start_char + len_char -1

	def start_line_no_of_selection(self):
		start_line = self.start_line_no_of_selection()
		len_line = self.selection.Range.ComputeStatistics(1)
		return start_line + len_line -1


	def start_para_no_of_selection(self):
		start_para = self.get_par_num()
		len_para = self.selection.Range.ComputeStatistics(4)

		return start_para + len_para -1

	def start_word_no_of_selection(self):
		start_word = self.start_char_no_of_selection()
		len_word = self.selection.Range.ComputeStatistics(0)
