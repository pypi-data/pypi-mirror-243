...
class ZerocapWebsocketClient:
    ...
    def hashing(self, timestamp):
        return hmac.new(
            str(timestamp).encode("utf-8"), self.api_key.encode("utf-8"), hashlib.sha256
        ).hexdigest()
    ...
    def create_connection(self, timestamp: int = 0):
        ...
        try:
            ...
            with connect(self.base_url,
                         additional_headers={"api-key": self.api_key, "signature": self.hashing(timestamp), "timestamp": timestamp}
                         ) as self.websocket:
                while True:
