...
class ETSModel(BaseModel):
    ...
    def __init__(self, seasonal_period=24, method="additive", damped=False):
    ...
    def fit(self, y, key):
        ...
        if len(y) >= 2 * self.seasonal_period and self.seasonal_period > 1:
            ...
            period = None
            seasonal_mode = None
        self.model_obj = ExponentialSmoothing(
...
if __name__ == "__main__":
    ...
    predictions = model.predict(24)
    plt.plot(
        np.append(fitted_values, predictions), label="multiplicative-damped"
    )
    plt.legend()
