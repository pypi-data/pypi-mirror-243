...
class DQN:
    ...
    def learn(self, episodes = 100000, stat1 = 10, stat2 = 100, plots = -1):
        ...
        for episode in range(1, episodes+1):
            rew, t = self.run_episode( self.params.steps )
            rews.append( rew )
            lens.append(t)
            ...
            if  plots > 0 and episode % plots == 0:
                ...
                #self.test(episodes = 1, ticks = self.params.steps, render = True)
                #env.close()
    #------------------------------------------------------------------------------------
    ...
    def multi_agent_policy(self, model, state):
        ...
        if self.params.choice:
            probs = torch.softmax(y/y.amax(axis=-1)).numpy()
            return np.random.choice(self.nA, size=(len(state,1)), p=probs)
        else:
            return np.argmax(y.numpy(),axis=-1)              # best action (N, )
    #------------------------------------------------------------------------------------

    def multi_agent_training(self, episodes = 100000, reset_steps=-1, stat1 = 10, stat2 = 100, plots = -1, start_episode_check=-1, check_rate=1., fname=""):
        ...
        for step in range(1, episodes*self.params.steps + 1):
            s1, r1, done, _ = self.env.step(a0)
            s1 = self.scale (s1)
            ...
            for i in range(N):
                rew[i] += r1[i]
                ticks[i] += 1
                expired = (ticks[i] >= self.params.steps)
                # для обычных сред done=True и при expired
                dn  = (done[i] and not expired) if self.params.timeout else done[i]                
                ...
                if expired or done[i]:           # один из агентов закончил эпизод
                    ...
                    if  self.episode % stat2 == 0 or self.episode > episodes:
                        maxQ = self.maxQ.to('cpu')
                        print(f"{self.episode:6d} rew:{mean:7.1f} ± {std/stat2**0.5:3.1f}, best:{self.best.reward:7.2f}, ticks:{lensM:3.0f} ± {lensS/stat2**0.5:.0f}, dones:{dones-dones_last:5d}, eps:{self.epsilon:.3f}, Q:{maxQ.mean():8.2f} ±{maxQ.std():6.2f}, loss:{self.last_loss:7.3f}, {(time.process_time() - beg):3.0f}s")
                        dones_last = dones
                        beg = time.process_time()
    ...
    def multi_agent_testing(self, model, steps = 100000, episodes=-1, reset_steps=-1, epsilon=0, info="", sufix="", verbose=1):
        ...
        rew = np.zeros((N,))                     # total reward
        ticks = np.zeros((N,))                   # время эпизода i-го анента
        for step in range(1, steps + 1 if steps > 0 else episodes*self.params.steps):
            s1, r1, done, _ = self.env.step(a0)
            s1 = self.scale (s1)
            a1 = self.multi_agent_policy(model, s1)
