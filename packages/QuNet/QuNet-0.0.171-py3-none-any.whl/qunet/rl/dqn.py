            ...
            for i in range(len(s0)):
                ...
                if expired or done[i]:           # один из агентов закончил эпизод
                    ...
                    if  self.episode % stat2 == 0:
                        ...
                        maxQ = self.maxQ.to('cpu')
                        print(f"{self.episode:6d} rew:{mean:7.1f} ± {std/stat2**0.5:3.1f}, best:{self.best.reward:7.2f}, ticks:{lensM:3.0f} ± {lensM:3.0f}, eps:{self.epsilon:.3f}, Q:{maxQ.mean():8.2f} ±{maxQ.std():6.2f}, loss:{self.last_loss:7.3f}, {(time.process_time() - beg):3.0f}s")
                        beg = time.process_time()
                ...
                if expired or done[i]:           # один из агентов закончил эпизод
                    ...
                    if episode > episodes:
                        if verbose:
                            ...
                            print(f"{info}rew:{mean:7.1f} ± {std:3.1f}, ticks:{lensM:3.0f} ± {lensS:3.0f}, eps:{self.epsilon:.3f}, {(time.process_time() - beg):3.0f}s")
                        return rews, lens                        
