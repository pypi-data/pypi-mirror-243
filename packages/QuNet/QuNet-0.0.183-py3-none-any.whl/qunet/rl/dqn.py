...
class DQN:
    ...
    def learn(self, episodes = 100000, stat1 = 10, stat2 = 100, plots = -1):
        ...
        for episode in range(1, episodes+1):
            ...
            if  plots > 0 and episode % plots == 0:
                self.plot(f"{self.cfg.env}  Episode: {episode}  best: {self.best.reward:7.1f} [{self.best.episode}]")
    ...
    def multi_agent_training(self, episodes = 100000, steps=-1, reset_steps=-1, stat1 = 10, stat2 = 100, mark_episode=1000, plots = -1, start_episode_check=-1, patience=-1, check_rate=1., fname=""):
            ...
            for i in range(N):                         # viewing agent states
                ...
                if expired or done[i]:                 # one of the agents ended the episode
                    ...
                    if  len(rews) >= stat2:
                        ...
                        elif len(fname) and  self.episode - self.last_saved_episode > patience and mean > self.best.reward * check_rate:                            
                    ...
                    if  plots > 0 and self.episode % plots == 0:
                        self.plot(f"{self.cfg.env}  Episode: {self.episode}  best: {self.best.reward:7.1f} [{self.best.episode}]")
    ...
    def save(self, model, reward=0, fname="model", sufix=""):
        ...
        fname = fname + f"_s{self.nS}a{self.nA}_{self.best.file_id:04d}_r{self.best.reward:.2f}_e{self.best.episode}{sufix}.pt"        
        state = {
