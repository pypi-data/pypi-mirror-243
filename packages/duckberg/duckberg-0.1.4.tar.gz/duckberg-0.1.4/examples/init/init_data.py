...
def main(spark):
    ...
    df = spark.read.parquet("/home/iceberg/init/data/yellow_tripdata_2022-12.parquet")

    logging.info("Inserting data to table ...")
    df = df.sortWithinPartitions("payment_type")
    df.writeTo("demo.nyc.taxis").using("iceberg").partitionedBy(bucket(8, "payment_type")).createOrReplace()
