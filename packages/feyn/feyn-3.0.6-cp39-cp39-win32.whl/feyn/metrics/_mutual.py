...
def _calculate_mi_for_output(samples: Iterable, output_mi: Iterable, output_bins: Iterable) -> np.array:
    sample_rv , sample_bins = _mi_inputs(samples)

    joint_dist = _normalize_hist(np.histogramdd([sample_rv, output_mi], bins=[sample_bins, output_bins])[0])
...
def calculate_mi_for_output(df: pd.DataFrame, output_name: str) -> pd.DataFrame:
    ...
    return df[inputs].apply(lambda col: _calculate_mi_for_output(col, output_mi, output_bins), axis=0)
