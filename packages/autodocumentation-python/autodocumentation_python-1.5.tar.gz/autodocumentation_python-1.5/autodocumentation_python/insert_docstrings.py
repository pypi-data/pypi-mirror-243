...
# I want to update the old docstring to a new one generated by GPT. Compare these two and if there is any
# information in the old one that is not in the new one, add it to the new one. If there is no difference,
# just return the new docstring - no further notification. Start a new line after 90 characters. 
# Your output whould start with "start" and end with "end".
        command = f"""
I want to replace the old docstring with a new one generated by GPT. 
However, it may be that the new one does not contain all the 
...
the information from the new docstring. If there is no difference, 
...
Your output should start with "start" and end with "end".
...
{new_docstring}
"""
        edited_docstring = gpt_compare(command, Model=Model)
...
# file_name = 'Train.py'
# path_doc = os.path.join(os.getcwd(), 'edited_repository', 'gpt_output', file_name)
# path_code = os.path.join(os.getcwd(), 'edited_repository', file_name)
