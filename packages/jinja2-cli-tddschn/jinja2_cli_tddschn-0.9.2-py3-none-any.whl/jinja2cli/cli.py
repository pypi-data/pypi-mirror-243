...
def cli(opts, args):
    ...
    data.update(parse_kv_string(opts.D or [], opts.file or []))
...
def parse_kv_string(pairs, file_pairs=None):
    file_pairs = file_pairs or []
    dict_ = {}
    for pair in pairs:
        k, v = pair.split("=", 1)
        dict_[force_text(k)] = force_text(v)
    for file_pair in file_pairs:
        k, file_path = file_pair.split("=", 1)
        with open(file_path, "r") as file:
            dict_[force_text(k)] = file.read()
    return dict_
...
def main():
    ...
    parser.add_option(
        "-F",
        "--file",
        help="Define template variable with a file content in the form of key=<path-to-file>",
        action="append",
