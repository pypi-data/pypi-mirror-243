        ...
        if schema_field['fieldType'] == 'hardCoded':
            ...
            if has_planned_change:
                try:
                    ...
                    pass
            if schema_field['fieldType'] == 'dataProfile':
                output = data_profile_field_value(output,c1Field, data_profiles, original_value_type, schema_field['dataProfileField']['label'])
                ...
                if not condition_passed:       
                    try:
                        ...
                        if test_condition['pass_condition']:
                            if condition['fieldType'] == 'concat':
                            ...
                            elif condition['fieldType'] == 'c1Field' or condition['fieldType'] == 'dataProfile':
                                schema_field['fieldType']
                                value_type = condition['valueType']['value']
                                if condition['fieldType'] == 'dataProfile':
                                    value_type = 'uniqueImportId'
                                ...
                                if has_planned_change:
                                    try:
                                        ...
                                        pass
                                if condition['fieldType'] == 'dataProfile':
                                    output = data_profile_field_value(output,emp[condition['fieldValue']['value']], data_profiles, condition['valueType']['value'], condition['dataProfileField']['label'])
...
def data_profile_field_value(output, c1Field, data_profiles, original_value_type, data_profile_field):

    try:
        output['field_value'] = data_profiles[output['field_value']][data_profile_field][original_value_type]
    except Exception as e:
        logging.error(f'Data profile error {e}')
        pass
    try:
        output['field_value_planned'] = data_profiles[output['field_value_planned']][data_profile_field][original_value_type]
    ...
    try:
        previous_data_profile_value = data_profiles[c1Field['timelineChange'][1]['data']['uniqueImportId']][data_profile_field][original_value_type]
