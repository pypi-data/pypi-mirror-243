...
#             return video_quality
    ...
    def check_document_quality(self, side, data):
        ...
        if side=='front':
            ...
            processed_front_id = self.process_image(data)
            ...
            processed_back_id = self.process_image(data)

            processed_back_id = pytesseract.image_to_string(processed_back_id)
            
            text = processed_back_id
            print(text)
        ...
        if country == 'UAE':
            ...
            if is_colored1:
                try:
                    ...
                    pattern2 = r'Identity Card'
                    pattern3 = r'Golden Card'
                    m = re.search(pattern1, front_id_text, re.IGNORECASE)
                    n = re.search(pattern2, front_id_text, re.IGNORECASE)
                    g= re.search(pattern3, front_id_text, re.IGNORECASE)
                    if not (m or n or g):
                        front_data["error"] = "not_front_id"
                        return front_data
        ...
        if country=='UAE':
            ...
            if is_colored2:
                # try:
                    ...
                    pattern4 = r'(Card Number|ILARE|IDARE)'
                    k= re.search(pattern4, text.replace(" ",""), re.IGNORECASE)
