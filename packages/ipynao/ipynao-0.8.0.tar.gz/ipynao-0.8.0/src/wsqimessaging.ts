...
export class QiSession implements IQiSession {
    ...
    constructor(
        ...
        connected?: () => void,
        disconnected?: () => void) {
      console.log(`DBG Emile qim about to connect wss to ${port}`);
      this.socket = new WebSocket(`wss://${host ? host : window.location.host}:${port}`);
      console.log(`DBG Emile qim connecting.. to ${port}`);
      ...
      this.socket.onmessage = (event) => {
        console.log("DBG Emile qim received raw data:", event.data);
        const data = JSON.parse(event.data);
        console.log("DBG Emile qim received:", data);
    ...
    private handleMessage(data: any): void {
        if (data.name === 'reply') {
          ...
          console.log("DBG Emile qim error", data);
  ...
  // Global function available without the need for "export"
  (window as any).QiSession = QiSession;
