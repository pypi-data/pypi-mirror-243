...
class AssistedClient(object):
    ...
    def set_default_values(self, overrides, existing=False):
        ...
        legacy_vips = False
        if 'legacy_vips' in overrides:
            if isinstance(overrides['legacy_vips'], bool):
                legacy_vips = overrides['legacy_vips']
            else:
                warning("Ignoring legacy_vips variable as it's not a boolean")
            del overrides['legacy_vips']
        ...
        api_vip = overrides.get('api_vip')
        ingress_vip = overrides.get('ingress_vip')
        missing_vips = (api_vip is not None and ingress_vip is None) or (ingress_vip is not None and api_vip is None)
        if legacy_vips and missing_vips:
        ...
        missing_vips = (api_vips and not ingress_vips) or (ingress_vips and not api_vips)
        if not legacy_vips and missing_vips:
            error("It's mandatory to define both api and ingress ips")
            sys.exit(1)
        if not existing:
