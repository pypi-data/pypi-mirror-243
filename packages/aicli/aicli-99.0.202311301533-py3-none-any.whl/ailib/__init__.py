    ...
    def set_default_values(self, overrides, existing=False, quiet=False):
        ...
        if api_ip is not None:
            if not dual:
                ...
                warning("Ignoring api_ip at creation time", quiet=quiet)
        ...
        if ingress_ip is not None:
            if not dual:
                ...
                warning("Ignoring ingress_ip at creation time", quiet=quiet)
        api_vips = overrides.get('api_vips', [])
        if not dual and api_vips:
            warning("Ignoring api_vips at creation time", quiet=quiet)
            del overrides['api_vips']
        ingress_vips = overrides.get('ingress_vips', [])
        if not dual and ingress_vips:
            warning("Ignoring ingress_vips at creation time", quiet=quiet)
    ...
    def update_cluster(self, name, overrides):
        ...
        if api_vip is not None:
            ...
            if api_vip not in [x.ip for x in api_vips]:
                overrides['api_vips'] = [{'ip': x.ip for x in api_vips}] + [{'ip': api_vip}]
        ...
        if ingress_vip is not None:
            ...
            if ingress_vip not in [x.ip for x in ingress_vips]:
                overrides['ingress_vips'] = [{'ip': x.ip for x in ingress_vips}] + [{'ip': ingress_vip}]
