...
class PrepareForComparison:
    ...
    def __init__( self, tested: pl.DataFrame, expected: pl.DataFrame, key=None, tolerance: float = 10e-6, verbose: bool = False, progress_bar=None, low_memory=False) -> None:
    ...
    def matchRowCounts(self):
        if not self.key is None:
            ...
            unique_keys_match = (number_unique_keys['expected'] == number_unique_keys['tested'])
            if self.verbose:
                logger.info(f"Unique keys match: {unique_keys_match} | Low memory: {self.low_memory}")
