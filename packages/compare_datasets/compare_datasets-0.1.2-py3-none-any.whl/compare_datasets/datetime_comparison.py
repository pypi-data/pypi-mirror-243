...
from compare_datasets.structure import Comparison, stringify_result, timeit, format_float
...
class DateTimeComparisons(Comparison):
    ...
    def compare(self):
        ...
        self.report["value_by_value"][
            "explanation"
        ] = "The date-time comparison is done by calculating the time-delta between the expected and tested dataframes. The time-delta is calculated by subtracting the expected dataframe from the tested dataframe. If the time-delta is 0, then the expected and tested dataframes have the same date-time values for the same column(s)."

        if not self.report["value_by_value"]["result"]:
            self.report["value_by_value"][
                "explanation"
            ] += f"\nThe time-delta between the expected and tested dataframes is not 0 for all columns. This means that the expected and tested dataframes do not have the same date-time values for the same column(s)."
        else:
            self.report["value_by_value"][
                "explanation"
            ] += f"\nThe time-delta between the expected and tested dataframes is 0 for all columns. This means that the expected and tested dataframes have the same date-time values for the same column(s)."

    def generate_report(self):
        report = f"""COMPARISON FOR {self.data_type} COLUMNS
-----------------------------------------
RESULT: {stringify_result(self.report['overall_result'])}
-----------------------------------------
