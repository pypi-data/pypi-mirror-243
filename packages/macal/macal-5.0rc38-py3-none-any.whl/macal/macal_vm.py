...
class MacalVm:
    ...
    def _execute_instruction(self, op: int, instruction, scope: NewVMScope) -> NewVMScope:
        ...
        if op == Opcode_NEW_SCOPE:
            ...
            self._execute_print(instruction, scope)
        elif op == Opcode_RET:
    ...
    def _execute_print(self, instruction, scope: NewVMScope):
    ...
    def _apply_field_filters(self, fields: List[AstNodeSelectField], source_data: List[Dict]) -> List[Dict]:
        if len(source_data) == 0:
            return [{field.altfieldname: AstNodetype.NIL for field in fields}]
        return [{field.altfieldname: item.get(field.fieldname, AstNodetype.NIL) for field in fields} for item in source_data]
