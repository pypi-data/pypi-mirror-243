...
class MacalVm:
    ...
    def _execute_expression_for_value(self, expression, scope: NewVMScope, 
        ...
        if not isinstance(result, ExpressionResult):
            raise Exception(f"Expected expression result, got {pythonTypeToStr(result)}")
        output = result.value
    ...
    def _execute_walk_index(self, source: Any, index_to_walk: List) -> Any:
        if source is None:
            raise ValueError("Invalid input - nil.")
        if not (isinstance(source, list) or isinstance(source, dict) or isinstance(source, str)):
            raise Exception(f"This variable type ({pythonTypeToStr(source)}) does not support indexing")
        if index_to_walk is None:
            raise Exception("Invalid index - nil.")
        ...
        for index in index_to_walk:
            if isinstance(value, dict):
                if not isinstance(index, str):
                    raise Exception(f"Invalid index type, expected string (got {pythonTypeToStr(index)}).")
                if index not in value.keys():
                    raise Exception(f"Field ({index}) not found in record ({','.join(value.keys())}).")
            elif isinstance(value, list) or isinstance(value, str):
                if isinstance(index, int):
                    if index < 0 or index >= len(value):
                        raise Exception(f"Array index ({index}) out of range (0-{len(value)-1}).")
                else:
                    raise Exception(f"Invalid index type, expected integer (got {pythonTypeToStr(index)}).")
            value = value[index]
        return value

    # Assign to index of variable
    def _assign(self, var: MacalVariable, index_to_walk: List, value: Any) -> None:
        if var is None:
            raise ValueError("Assign - Invalid input - nil.")
        if not isinstance(var, MacalVariable):
            raise Exception(f"Assign - Invalid input type, not a variable (got {pythonTypeToStr(var)}).")
        if index_to_walk is None:
            raise ValueError(f"Assign ({var.name}) - Invalid index - nil.")
        if not (isinstance(var.value, list) or isinstance(var.value, dict) or isinstance(var.value, str)):
            raise Exception(f"Assign ({var.name}) - Invalid value type ({pythonTypeToStr(var.value)}), index not supported.")
        if len(index_to_walk) == 0:
            var.value = value
            return
        ...
        if len(index_to_walk) > 1:
            for index in index_to_walk[:-1]:
                if isinstance(value_to_index, dict):
                    if not isinstance(index, str):
                        raise Exception(f"Assign ({var.name}) - Invalid index type, expected string (got {pythonTypeToStr(index)}).")
                elif isinstance(value_to_index, list) or isinstance(value_to_index, str):
                    if isinstance(index, int):
                        if index < 0 or index >= len(value_to_index):
                            raise Exception(f"Assign ({var.name}) - Array index ({index}) out of range (0-{len(value)-1}).")
                    else:
                        raise Exception(f"Assign ({var.name}) - Invalid index type, expected integer (got {pythonTypeToStr(index)}).")
                value_to_index = value_to_index[index]
        if isinstance(value_to_index, dict):
            if not isinstance(index_to_walk[-1], str):
                raise Exception(f"Assign ({var.name}) - Invalid index type, expected string (got {pythonTypeToStr(index_to_walk[-1])}).")
        if (isinstance(value_to_index, list) or isinstance(value_to_index, str)):
            if not isinstance(index_to_walk[-1], int):
                raise Exception(f"Assign ({var.name}) - Invalid index type, expected integer (got {pythonTypeToStr(index_to_walk[-1])}).")
            if index_to_walk[-1] < 0 or index_to_walk[-1] >= len(value_to_index):
                raise Exception(f"Assign ({var.name}) - Array index ({index_to_walk[-1]}) out of range (0-{len(value_to_index)-1}).")
    ...
    def _execute_expression(self, expression, 
        ...
        if op == Opcode_LOAD_VARIABLE:
            ...
            if self.verbose:
                ...
                print(var)
                print("name: ", name)
                print("iswhere: ", iswhere)
                print("where_item_data: ", where_item_data)
                print("Where item data keys: ", where_item_data.keys() if where_item_data is not None else None)
                print("Where item data name: ", where_item_data[name] if where_item_data is not None and name in where_item_data.keys() else None)
                print()
            if var is None and iswhere and where_item_data is not None and name in where_item_data.keys():
                #print("Found where item data: ", where_item_data[name])
            ...
            if var.is_indexed:
                type_to_validate = typeFromValue(self._execute_walk_index(var_to_check.value, var.index))
            ...
            if var.is_indexed:
                result = typeFromValue(self._execute_walk_index(var_to_check.value, var.index))
            return ExpressionResult(result, False, False, None)
        elif len(expr) == 2:
    ...
    def _execute_instruction(self, op: int, instruction, scope: NewVMScope) -> NewVMScope:
        ...
        if op == Opcode_NEW_SCOPE:
            ...
            if scope is None:
                ...
                return scope
            child: NewVMScope = scope.new_child(name)
            return child
    ...
    def _execute_assign(self, instruction, scope: NewVMScope):
        ...
        if self.verbose:
            ...
            print("Index:             = ", var_instruction[0][2:] if len(var_instruction[0]) > 2 else '')
        ...
        if self.verbose:
            print("variable_to_assign_to = ", variable_to_assign_to)
            print("Result is indexed:     ", result.is_indexed)
        ...
        if result.is_indexed:
            self._assign(variable_to_assign_to, result.index, value_to_assign)
        else:
            if append:
                if isinstance(value_to_assign, list) or isinstance(value_to_assign, dict):
                    value_to_assign = value_to_assign.copy()
    ...
    def _execute_select(self, instruction, scope: NewVMScope):
        ...
        if into_expr_result.is_indexed:
            self._assign(into_var, into_expr_result.index, destination_data)
        else:
