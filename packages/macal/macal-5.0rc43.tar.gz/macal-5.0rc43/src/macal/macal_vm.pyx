...
class MacalVm:
    ...
    def _assign(self, var: MacalVariable, index_to_walk: List, value: Any, scope: NewVMScope) -> None:
        if var is None:
    ...
    def _execute_assign(self, instruction, scope: NewVMScope):
        ...
        if result.is_indexed:
            self._assign(variable_to_assign_to, result.index, value_to_assign, scope)
        else:
            if append:
    ...
    def _execute_select(self, instruction, scope: NewVMScope):
        ...
        if into_expr_result.is_indexed:
            self._assign(into_var, into_expr_result.index, value, scope)
        else:
