...
from .macal_conversions import typeFromValue, typeToStr
...
class MacalVm:
        ...
        for index in index_to_walk:
            if isinstance(value, dict):
                if not isinstance(index, str):
                    raise Exception(f"Invalid index type, expected string (got {type(index)}).")
                ...
                if isinstance(index, int):
                    ...
                    raise Exception(f"Invalid index type, expected integer (got {type(index)}).")
            value = value[index]
        return value
    ...
    def _execute_walk_index_for_store(self, var: MacalVariable, index_to_walk: List, value: Any) -> None:
        ...
        if not isinstance(var, MacalVariable):
            raise Exception(f"Invalid input type, not a variable (got {type(var)}).")
        if index_to_walk is None:
            raise ValueError("Invalid index - nil.")
        if not (isinstance(var.value, list) or isinstance(var.value, dict) or isinstance(var.value, str)):
            raise Exception(f"Invalid value type ({type(var.value)}), index not supported.")
        if len(index_to_walk) == 0:
            var.value = value
            return
