...
class MacalCompiler:
    ...
    def _compile_program(self, program: AstNodeProgram, scope: NewScope, instructions: list):
        for statement in program.statements:
            self._compile_statement(statement, scope, instructions)

    def _compile_statement(self, statement: AstNodeStatement, scope: NewScope, instructions: list):
        if statement.expr_type == AstNodetype.ASSIGNMENT_STATEMENT:
            ...
            self._error(f"Unknown statement type {statement.expr_type} @ {statement.line}:{statement.column}")

    def _compile_block(self, statement: AstNodeBlock, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.BLOCK:
            self._error(f"Expected block, got {statement.expr_type} @ {statement.line}:{statement.column}")
    ...
    def _compile_print_statement(self, statement: AstNodePrintStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.PRINT_STATEMENT:
            self._error(f"Expected print statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        args = []
        for arg in statement.args:
            self._compile_expression(arg, scope, args)
        instructions.append((Opcode_PRINT, len(statement.args), args))

    def _compile_function_definition(self, statement: AstNodeFunctionDefinition, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.FUNCTION_DEFINITION:
            self._error(f"Expected function definition, got {statement.expr_type} @ {statement.line}:{statement.column}")
        ...
        if fndef is None:
            self._error(f"Function definition without body @ {statement.line}:{statement.column}")
        
        child.function_definition.append(fndef)
        instructions.append(fndef)
       
    def _compile_assignment_statement(self, statement: AstNodeAssignmentStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.ASSIGNMENT_STATEMENT:
            self._error(f"Expected assignment statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        ...
        if op == "=":
            ...
            return
        if statement.append:
            self._error(f"Operator {op} not supported for append to array @ {statement.line}:{statement.column}")
        if new_var:
            self._error(f"Operator {op} not supported for new variable @ {statement.line}:{statement.column}")
        opcode = 0
        if op == "+=":
            opcode = Opcode_ADD
        elif op == "-=":
            ...
            self._error(f"Unknown assignment operator {op} @ {statement.line}:{statement.column}")
        instructions.append((Opcode_STORE_VARIABLE, lhs, [(opcode, lhs, rhs)]))

    def _compile_return_statement(self, statement: AstNodeReturnStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.RETURN_STATEMENT:
            self._error(f"Expected return statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        value = []
        if statement.expr is not None:
            self._compile_expression(statement.expr, scope, value)
        instructions.append((Opcode_RET, value))

    def _compile_halt_statement(self, statement: AstNodeHaltStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.HALT_STATEMENT:
            self._error(f"Expected halt statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        value = []
        if statement.expr is not None:
            self._compile_expression(statement.expr, scope, value)
        instructions.append((Opcode_HALT, value))

    def _compile_break_statement(self, statement: AstNodeBreakStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.BREAK_STATEMENT:
            self._error(f"Expected break statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        instructions.append((Opcode_BREAK,))

    def _compile_continue_statement(self, statement: AstNodeContinueStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.CONTINUE_STATEMENT:
            self._error(f"Expected continue statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        instructions.append((Opcode_CONTINUE,))

    def _compile_if_statement(self, statement: AstNodeIfStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.IF_STATEMENT:
            self._error(f"Expected if statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        expr = []
    ...
    def _compile_elif_statement(self, statement: AstNodeElifStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.ELIF_STATEMENT:
            self._error(f"Expected elif statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        expr = []
        self._compile_expression(statement.condition, scope, expr)
        blk = []
        self._compile_block(statement.block, scope, blk)
    ...
    def _compile_foreach_statement(self, statement: AstNodeForeachStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.FOREACH_STATEMENT:
            self._error(f"Expected foreach statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        expr = []
    ...
    def _compile_while_statement(self, statement: AstNodeWhileStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.WHILE_STATEMENT:
            self._error(f"Expected while statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        expr = []
        self._compile_expression(statement.condition, scope, expr)
        blk = []
        self._compile_block(statement.block, scope, blk)
    ...
    def _compile_switch_statement(self, statement: AstNodeSwitchStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.SWITCH_STATEMENT:
            self._error(f"Expected select statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        expr = []
        self._compile_expression(statement.expr, scope, expr)
        ...
        for case in statement.cases:
            ...
            if csw[0][1] in switch.keys():
                self._error(f"Duplicate case statement {csw[0][1]} @ {statement.line}:{statement.column}")
    ...
    def _compile_case_statement(self, statement: AstNodeCaseStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.CASE_STATEMENT:
            self._error(f"Expected case statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        expr = []
        self._compile_expression(statement.expr, scope, expr)
        if expr[0][0] != Opcode_LOAD_CONSTANT:
            self._error(f"Case expression must be a constant, got {expr[0][0]} @ {statement.line}:{statement.column}")
        blk = []
        self._compile_block(statement.block, scope, blk)
        instructions.append((Opcode_CASE, expr[0][1], blk))

    def _compile_select_statement(self, statement: AstNodeSelectStatement, scope: NewScope, instructions: list):
        ...
        instructions.append((Opcode_SELECT, statement.Fields, from_expr, where_expr, statement.distinct, statement.merge, into_expr))

    def _compile_function_call_statement(self, statement: AstNodeFunctionCallStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.FUNCTION_CALL:
            self._error(f"Expected function call, got {statement.expr_type} @ {statement.line}:{statement.column}")
        func = scope.get_function(statement.name)
        if func is None:
            self._error(f"Unknown function {statement.name} @ {statement.line}:{statement.column}")
        args = []
        for arg in statement.args:
            self._compile_expression(arg, scope, args)
        instructions.append((Opcode_CALL, statement.name, len(statement.args), args))

    def _compile_is_type_statement(self, statement: AstNodeIsType, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.IS_TYPE_STATEMENT:
            self._error(f"Expected is type statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        expr = []
        self._compile_expression(statement.expr, scope, expr)
        instructions.append((Opcode_IS_TYPE, expr, statement.TypeToCheck))

    def _compile_type_statement(self, statement: AstNodeTypeStatement, scope: NewScope, instructions: list):
        if statement.expr_type != AstNodetype.TYPE_STATEMENT:
            self._error(f"Expected type statement, got {statement.expr_type} @ {statement.line}:{statement.column}")
        expr = []
        self._compile_expression(statement.expr, scope, expr)
    ...
    def _compile_include_statement(self, statement: AstNodeIncludeStatement, scope: NewScope, instructions: list) -> None:
    ...
    def _compile_function_call_expression(self, expression: AstNodeFunctionCallExpression, scope: NewScope, instructions: list):
        if expression.expr_type != AstNodetype.FUNCTION_CALL_EXPRESSION:
            self._error(f"Expected function call expression, got {expression.expr_type} @ {expression.line}:{expression.column}")
        args = []
        func = scope.get_function(expression.name)
        if func is None:
            self._error(f"Unknown function {expression.name} @ {expression.line}:{expression.column}")
        for arg in expression.args:
            self._compile_expression(arg, scope, args)
        instructions.append((Opcode_CALL, expression.name, len(expression.args), args))

    def _compile_expression(self, expression: AstNodeExpression, scope: NewScope, instructions: list, allow_new_variable: bool = False):
        if expression.expr_type == AstNodetype.BINARY_EXPRESSION:
            self._compile_binary_expression(expression, scope, instructions, allow_new_variable=allow_new_variable)
            ...
            self._compile_variable_expression(expression, scope, instructions, allow_new_variable=allow_new_variable)
            ...
            self._compile_variable_function_call_expression(expression, scope, instructions)
            ...
            self._error(f"Unknown expression type {expression.expr_type} @ {expression.line}:{expression.column}")

    def _compile_binary_expression(self, expression: AstNodeBinaryExpression, scope: NewScope, instructions: list, allow_new_variable: bool = False):
        if expression.expr_type != AstNodetype.BINARY_EXPRESSION:
            self._error(f"Expected binary expression, got {expression.expr_type} @ {expression.line}:{expression.column}")
        lhs = []
        self._compile_expression(expression.left, scope, lhs, allow_new_variable=allow_new_variable)
        rhs = []
        self._compile_expression(expression.right, scope, rhs, allow_new_variable=allow_new_variable)
        ...
        if op == "+=" or op == "+":
            ...
            self._error(f"Unknown binary operator {op} @ {expression.line}:{expression.column}")

    def _compile_literal_expression(self, expression: AstNodeLiteralExpression, scope: NewScope, instructions: list):
        if expression.expr_type != AstNodetype.LITERAL_EXPRESSION:
            self._error(f"Expected literal expression, got {expression.expr_type} @ {expression.line}:{expression.column}")
        instructions.append((Opcode_LOAD_CONSTANT, expression.value))

    def _compile_variable_expression(self, expression: AstNodeVariableExpression, scope: NewScope, instructions: list, allow_new_variable: bool = False):
        if expression.expr_type != AstNodetype.VARIABLE_EXPRESSION:
            self._error(f"Expected variable expression, got {expression.expr_type} @ {expression.line}:{expression.column}")
        var = scope.get_variable(expression.name)
        if var is None and allow_new_variable:
            var = scope.new_variable(expression.name)
        if var is None:
            self._error(f"Unknown variable {expression.name} @ {expression.line}:{expression.column}")
        else:
            instructions.append((Opcode_LOAD_VARIABLE, expression.name))

    def _compile_indexed_variable_expression(self, expression: AstNodeIndexedVariableExpression, scope: NewScope, instructions: list):
        if expression.expr_type != AstNodetype.INDEXED_VARIABLE_EXPRESSION:
            self._error(f"Expected indexed variable expression, got {expression.expr_type} @ {expression.line}:{expression.column}")
        var = scope.get_variable(expression.name)
        if var is None:
            self._error(f"Unknown variable {expression.name} @ {expression.line}:{expression.column}")
        var_index = []
    ...
    def _compile_variable_function_call_expression(self, expression: AstNodeVariableFunctionCallExpression, scope: NewScope, instructions: list):
        print("compile_variable_function_call_expression")
        if expression.expr_type != AstNodetype.VARIABLE_FUNCTION_CALL_EXPRESSION:
            self._error(f"Expected variable function call expression, got {expression.expr_type} @ {expression.line}:{expression.column}")
        print(expression.name)
        for arg in expression.args:
            self._compile_expression(arg, scope, instructions)
        self._error("variable function call expression Not implemented")

    def _compile_unary_expression(self, expression: AstNodeUnaryExpression, scope: NewScope, instructions: list):
        if self.verbose:
            print("compile_unary_expression")
        if expression.expr_type != AstNodetype.UNARY_EXPRESSION:
            self._error(f"Expected unary expression, got {expression.expr_type} @ {expression.line}:{expression.column}")
        if self.verbose:
        ...
        if op == "-":
            ...
            self._error(f"Unknown unary operator {op} @ {expression.line}:{expression.column}")
