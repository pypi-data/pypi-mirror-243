...
def usage():
    print(
        f"Usage: python -m to_pip -n <package_name> -v <package_version> [-r <README.md path> -e <requirements.txt path> -u <pypi_username> -p <pypi_password>] <python_files>"
    )
    sys.exit(1)
...
def parse_args():
    ...
    parser.add_argument("-r", "--readme_path", help="README.md", default="README.md")
    parser.add_argument("-e", "--requirements_path", help="requirements.txt", default="requirements.txt")
    parser.add_argument("python_files", nargs="*", help="Python files to include")
    return parser.parse_args()


def create_package_dir(package_name, package_version, python_files, requirements_path):
    ...
    if os.path.exists(requirements_path):
        shutil.copy(requirements_path, os.path.join(package_dir, "requirements.txt"))

    return package_dir


def handle_readme(package_dir, package_name, readme_path):
    if os.path.exists(readme_path):
        shutil.copy(readme_path, os.path.join(package_dir, "README.md"))
    else:
...
# 修改 to_pip 函數，以接收新的路徑參數
def to_pip(python_files, package_name, package_version, readme_path, requirements_path, pypi_username=None,
           pypi_password=None):
    if not python_files:
        usage()

    package_dir = create_package_dir(package_name, package_version, python_files, requirements_path)
    write_setup_py(package_dir, package_name, package_version, python_files)
    handle_readme(package_dir, package_name, readme_path)
...
def to_pip_args():
    ...
    to_pip(args.python_files, args.package_name, args.package_version, args.readme_path, args.requirements_path,
           args.pypi_username, args.pypi_password)
