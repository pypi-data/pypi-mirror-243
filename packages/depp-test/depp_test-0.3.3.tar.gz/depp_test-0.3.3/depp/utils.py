...
def process_seq(self_seq, args, isbackbone, need_mask=False):
    ...
    if (isbackbone or args.query_dist):
        ...
        if args.replicate_seq:
            df = df.groupby(by=lambda x: x.split('_')[0].split('splithere')[0]).sum(numeric_only=False)
        else:
            df = df.groupby(by=lambda x: x.split('splithere')[0]).sum(numeric_only=False)
...
def combine_identical_seq_intree(tree, seq, thr=None, outdir=None, cluster=None, replicate=False):
    ...
    for node in best_node_group:
        ...
        edge_length = np.mean([tree.distance_between(name_to_node[i], node) for i in best_node_group[node]])
