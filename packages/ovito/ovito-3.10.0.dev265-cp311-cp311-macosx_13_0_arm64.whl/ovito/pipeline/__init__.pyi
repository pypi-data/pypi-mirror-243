...
Data modification steps in a pipeline are instances of the :py:class:`ModificationNode` class, which is a specialization :py:class:`PipelineNode`
...
Each pipeline consists of exactly one *head* node, which can be accessed through the :py:attr:`Pipeline.head` property. It represents the output of the pipeline as
it is the last processing step in the pipeline. It's the head node of the linked-list structure formed by the pipeline steps.

Each pipeline has a data *source* node, which is the one forming the tail of the chain of nodes. That tail node is accessible as the pipeline's :py:attr:`~Pipeline.source` property."""
...
Each :py:class:`ModificationNode` has an :py:attr:`.input` property, which is a reference to its preceding node in the pipeline,
...
form a linked list structure. The linked list is always termined by a *source* :py:class:`PipelineNode`, i.e., a node that is *not* a :py:class:`ModificationNode`.

Each :py:class:`ModificationNode` has a :py:attr:`modifier` field, which is a reference to a :py:class:`Modifier` object
implementing the actual data modification algorithm that gets executed during pipeline evaluation. The *modifier* is also where the control parameters
of the data modification step are stored. The :py:class:`ModificationNode` itself simply represents the *use* of a modifier in
