...
class MysqlPlatForm(object):
    ...
    def updateDesAndRefer(self,caseid,testname,scripturl,caseurl):
        ...
        script = _dict['script']
        if _dict['desc'].strip() == scripturl.strip():
            logger.info(f'无需保存Jira用例和脚本关系成功【脚本未改变】：用例编号：{caseid} 用例名称:{testname} 当前脚本路径:{script} 原脚本路径:{caseurl}')
        else:
            ...
            logger.info(f'保存Jira用例和脚本关系成功【脚本有改变】：用例编号：{caseid} 用例名称:{testname} 当前脚本路径:{script} 原脚本路径:{caseurl}')
