...
"""
import os
from . import sg,get_event_key_js,make_component,text_to_key
...
class AbstractBrowser:
    ...
    def get_scan_browser_layout(section=None, extra_buttons=[]):
        ...
        checkboxes_layout = [
            [sg.Checkbox('All', default=True, key=f'-ALL-{section}-')],
            [sg.Checkbox('Files', key=f'-FILES-{section}-')],
            [sg.Checkbox('Folders', key=f'-FOLDERS-{section}-')]
        ]

        browser_buttons_layout = [
            sg.FolderBrowse('Folders', key=f'-FOLDER-BROWSE-{section}-'),
            sg.FileBrowse('Files', key=f'-FILE-BROWSE-{section}-')
        ]

        listboxes_layout =sg.Listbox(values=[], size=(50, 10), key=f'-BROWSER LIST-{section}-', enable_events=True)

        control_buttons_layout = [
            sg.Button('Scan', key=f'-SCAN-{section}-'),
            sg.Button('<-', key=f'-BACK-{section}-'),
            sg.Button('All Scan Mode', key=f'-MODE-{section}-'),
            sg.Button('->', key=f'-FORWARD-{section}-'),
            sg.Button('Add', key=f'-ADD-{section}-'),
            sg.Button('Clear', key=f'-CLEAR-{section}-')
        ] + extra_buttons

        return [
            [sg.Text('Directory to scan:'), sg.InputText(), sg.Column([browser_buttons_layout])],
            [listboxes_layout, sg.Column(checkboxes_layout)],
            control_buttons_layout
        ]
