...
class SSHExecutor(object):
    ...
    def watch_dog(self, remote_file_path, poll_interval=5):
        while True:
            try:
                stdin, stdout, stderr = self._client.exec_command(
                    f"test -f {remote_file_path} && echo exists || echo not_exists")  # NOQA
                output = stdout.read().decode().strip()
                if "exists" == output:
                    print(f"File {remote_file_path} exists on remote server.")
                    break
                else:
                    print(f"Waiting for file {remote_file_path} to be created on remote server.")  # NOQA
