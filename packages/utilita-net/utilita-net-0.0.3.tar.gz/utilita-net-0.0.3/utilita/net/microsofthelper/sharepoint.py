...
class SharepointHelper:
    ...
    def upload_file_from_bytes(self, upload_path: str, file_bytes: bytes, if_exists='fail'):
        ...
        preferred_chunk_size = 1024*1024*16 # 16mb # This is for the future when we support more than 64mb of data.
        ...
        r = requests.post(url=initial_full_url, headers=initial_headers, data=json.dumps(initial_body))

        if r.status_code != 200:
            raise APIException(f'Got an exception while creating upload session: {r.status_code}. Response string is in extra_data.', extra_data=r.text)
        try:
            ...
            if file_size < max_single_request_size:
                ...
                rq: requests.Response = requests.put(url=upload_url, headers=headers, data=file_bytes.read())
                # pass
                if rq.status_code != 202:
                    ex = requests.delete(url=upload_url)
                    raise APIException(f'Unable to upload file. Code is {rq.status_code} Cancelling upload. response is in extra_data', extra_data=rq.text)
                
            else:
                raise APIException(f'Currently we are unable to upload data of larger than 64mb. This needs to be implemented. Sorry.')
            
            # Finalize the file.
            headers_finalize = {
            'Content-Length': '0'
            }
            r_finalize: requests.Response = requests.post(url=upload_url, headers=headers_finalize)
            ...
            rq_del = requests.delete(url=upload_url)
    ...
    def delete_file_by_path(self, file_path) -> bool:
        ...
        del_req = requests.delete(url=full_url, headers=headers)
    ...
    def create_access_link_from_path(self, file_path, file_permissions):
        ...
        rq_share_link = requests.post(url=full_url, headers=headers, data=json.dumps(request_body))
        ...
        if rq_share_link.status_code not in [200, 201]:
            raise APIException(f'We got code {rq_share_link.status_code} when attempting to create an upload link. Exception was: {rq_share_link.text}', extra_data=rq_upload_link.text)
        ...
        if len(access_emails) > 0:
            ...
            rq_invite_link = requests.post(url=grant_url, headers=headers, data=json.dumps(grant_request_body))
