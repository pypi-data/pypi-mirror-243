...
class MixedDataTypes(DataMetricBase):
    ...
    def compute(self, data, **kwargs):
        ...
        for column in data_columns:
            unique_types = data[column].apply(
                lambda x: type(x).__name__).unique()

            if len(unique_types) > 1:
                string_ratio = data[column].apply(
                    lambda x: isinstance(x, str)).mean()
