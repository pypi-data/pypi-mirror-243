...
class ConflictingLabels(DataMetricBase):
    ...
    def compute(self, data, **kwargs):
        ...
        duplicate_samples = data[data.duplicated(
            subset=[data.columns[0]], keep=False)]
        if not duplicate_samples.empty:
            grouped_data = duplicate_samples.groupby(data.columns[0])
            for group_value, group in grouped_data:
                conflicting_samples[group_value] = group.iloc[:,
                                                              1:].values.tolist()

        percent_of_conflicting_samples = len(
            conflicting_samples) / data.shape[0]
