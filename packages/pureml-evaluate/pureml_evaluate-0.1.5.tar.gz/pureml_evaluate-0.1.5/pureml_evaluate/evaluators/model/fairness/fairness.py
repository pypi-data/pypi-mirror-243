...
from pureml_evaluate.schema.metric import MetricTemplate, ColumnTemplate, MetricDictEnum
...
class Fairness:
    ...
    def parse_metrics(self):
        ...
        for metric in self.metrics:
            ...
            if metric_key in self.fairness_metrics_default.keys():
                metrics_temp_fairness[metric_key] = self.fairness_metrics_default[metric_key]
            elif metric_key in self.demography_metrics_default.keys():
                metrics_temp_demography[metric_key] = self.demography_metrics_default[metric_key]
            else:
    ...
    def compute(self):
        ...
        if s_feat_dim > 1:
            ...
            metric_dict_list = self.compute_metrics_column(
                s_feat=s_feat_df, column_name=column_name)
    ...
    def compute_metrics_column(self, s_feat, column_name):
        ...
        columns_sensitive = [ColumnTemplate(name=column_name,
                                            value=MetricDictEnum.all.value)]

        if s_feat.nunique() == 1:
            columns_sensitive = [ColumnTemplate(name=column_name,
                                                value=s_feat.unique()[0])]
        ...
        for metric_name, metric_func in self.demography_metrics.items():
            try:
                ...
                print("Unable to compute", metric_name,
                      'for sensitive column', column_name)
                print(e)
