...
class OutlierSampleDetection(DataMetricBase):
    name = 'outlier_sample_detection'
    ...
    def detect_outliers(self, distance_matrix, nearest_neighbors_percent, extent):
        num_neighbors = int(
            distance_matrix.shape[0] * nearest_neighbors_percent)
        outlier_indices = []
        ...
        for i in range(distance_matrix.shape[0]):
            ...
            loop = np.mean(
                sorted_distances[:num_neighbors]) / np.mean(sorted_distances[num_neighbors:])
