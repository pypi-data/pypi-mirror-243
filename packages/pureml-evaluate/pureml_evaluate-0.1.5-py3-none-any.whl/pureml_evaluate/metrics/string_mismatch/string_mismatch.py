...
class StringMismatch(DataMetricBase):
        ...
        for column in string_columns:
            ...
            if len(unique_values) > 1:
                ...
                for base_form, variants in base_form_to_variants.items():
                    ...
                    mismatch_percentage = (
                        len(variants) / len(unique_values)) * 100
                    string_mismatch_info.append(
                        [column, base_form, variants, mismatch_count, mismatch_percentage])

        columns = ['Column', 'Base String',
                   'Actual Strings', 'Count', 'Percentage']
        string_mismatch_df = pd.DataFrame(
            string_mismatch_info, columns=columns)

        return {
            self.name: string_mismatch_df
        }
    ...
    def get_base_string(self, string_value):
        ...
        # Remove punctuations
        base_string = base_string.translate(
