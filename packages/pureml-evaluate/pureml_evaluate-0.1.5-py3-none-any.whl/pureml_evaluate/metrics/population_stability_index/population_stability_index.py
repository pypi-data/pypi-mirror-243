import numpy as np
from pureml_evaluate.metrics.drift_base import DriftMetricBase
from typing import Any


class PopulationStabilityIndex(DriftMetricBase):
    ...
    def compute(self, reference, production=None, columns=None, bins=10, **kwargs):
        ...
        small_value = 1e-10
        psi_bins = (prop_ref - prop_prod) * \
            np.log((prop_ref + small_value) / (prop_prod + small_value))

        # Handle division by zero and log of zero issues by replacing NaN and inf values with zero
        psi_bins = np.where(np.isnan(psi_bins) |
                            np.isinf(psi_bins), 0, psi_bins)
