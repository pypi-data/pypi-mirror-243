...
fairness_metrics = ['true_positive_parity','false_positive_parity','predictive_value_parity','equalized_opportunity']
...
class RiskEvaluator:
    ...
    def update_metric_info(self, category, metric_name, original_value, threshold_precent, description, matters):
        if metric_name in fairness_metrics:
            severity = get_severity_fairness(original_value=original_value, threshold_precent=threshold_precent)
            ...
            severity = get_severity(original_value=original_value, threshold_precent=threshold_precent)
        ...
        for i in range(len(categories)):
            ...
            if categories[i] in self.values_all and self.values_subset_all != None:
                ...
                for metric_name,metric_values in category_keys.items():
                    ...
                    threshold_precent = get_threshold_percentage(config_file=config_file,category=categories[i],metric_name=metric_name)
                    
                    description,matters = get_description_matters(category=categories[i],metric_name=metric_name,description_file=description_file)
                    
                    self.values_all[category][metric_name].update(self.update_metric_info(category, metric_name, data_from_values_all, threshold_precent, description, matters))

                    for subset_key in keys_in_subset:
                        data_from_subset = self.values_subset_all[subset_key][categories[i]][metric_name]['value']
                        
                        self.values_subset_all[subset_key][category][metric_name].update(self.update_metric_info(category, metric_name, data_from_subset, threshold_precent, description, matters))
...
def get_severity_fairness(original_value,threshold_precent): #For Fairness Metrics Defined Above.
