...
class KineticForms():
    ...
    # be specified as a string, or the connection information must be passed as a dict.
    def __init__(self, connection):
        ...
        if 'master_key' in self.connection_dict:
            self.master_key = self.connection_dict['master_key']
        else:
            # Use another key.  This is only here to prevent 500 errors during testing.
            self.master_key = "b'kpF5AaCEumhPUMxEmK4AZjnyIMakF4nVAKE6jg6_htY='"

    def connect(self):
        if 'host' not in self.connection_dict:
            {"error_code": "4001", "error_msg": "'host' must be in connection dictionary. ", "data": {}}
        if 'user' not in self.connection_dict:
            {"error_code": "4001", "error_msg": "'user' must be in connection dictionary. ", "data": {}}
        if 'database' not in self.connection_dict:
            {"error_code": "4001", "error_msg": "'dataabase' must be in connection dictionary. ", "data": {}}
        if 'password' not in self.connection_dict:
            {"error_code": "4001", "password": "'dataabase' must be in connection dictionary. ", "data": {}}
    ...
    def column_encrypt(self, id, column_name, value):
        sha256_hash = hashlib.sha256()
        s = self.master_key + str(column_name) + str(id)
        sha256_hash.update(s.encode())
        key = sha256_hash.hexdigest()
        key = key[:44]
        cipher_suite = Fernet(key)
        encrypted_value = cipher_suite.encrypt(message.encode())
        return encrypted_value

    def column_decrypt(self, id, column_name, value):
        sha256_hash = hashlib.sha256()
        s = self.master_key + str(column_name) + str(id)
        sha256_hash.update(s.encode())
        key = sha256_hash.hexdigest()
    ...
    def sql(self, s, encrypted_columns=None, errors=True):
        ...
        if len(encrypted_columns) > 0:
            for i in records:
                if 'id' not in i:
                    return {"error_code": "7000", "error_msg": "Cannot decrypt query without selecting 'id' in the query!",
                            "data": []}
                else:
                    tmp = copy.deepcopy(i)
                    for key in i:
                        if key in encrypted_columns:
                            tmp[key] = self.column_decrypt(tmp['id'],key, i[key])
                        else:
                            tmp[key] = i[key]
                output.append(tmp)
    ...
    def sql0(self, s, encrypted_colums=None, errors=True):
        ...
        if len(encrypted_columns) > 0:
            for key in records:
                if 'id' not in records:
                    ...
                    for key in i:
                        if key in encrypted_columns:
                            records[key] = self.column_decrypt(records['id'],key, records[key])
    ...
    def save(self, my_dict, table_name, encrypted_columns=None):
    ...
    # provided.
    def merge(self, my_dict, table_name, keys, encrypted_columns=None):
        ...
        if len(existing) == 0:
            my_dict['id'] = ""
            id_list.append(self.post(my_dict, encrypted_columns))
        else:
            for l in existing:
                my_dict['id'] = l['id']
                my_dict['table_name'] = table_name
    ...
    def post(self, my_dict, encryped_fields=None):
        ...
        if encryped_fields is not None:
            pass
        else:
            encrypted_fields = []
                ...
                if key != 'table_name' and key != 'id' and key != 'action':
                    ...
                    if column_exists:
                        ...
                        if key in encrypted_fields:
                            my_dict[key] = self.column_encrypt(my_id, key, my_dict[key])
                        v = (my_dict[key], my_id)
