...
class Settings(Singleton):
    ...
    def is_assess_enabled(self):
        ...
        # TODO: PYT-3073
        #  determine if we need the Assess/Protect guard. this was removed a while ago, but this is what it would be.
        # return not self.is_protect_enabled() and self.config.get_value("assess.enable")
    ...
    def generate_effective_config(self):
        ...
        user_config = config["config"]["user_configuration_file"]
        ...
        for key in self.config.keys():
            ...
            if option.source() == USER_CONFIGURATION_FILE_SRC:
                config_entry["filename"] = option.file_source
            ...
            if option.file_value is not None:
                config_entry = {
                    "canonical_name": option.canonical_name,
                    # provided name is only for env vars
                    "name": option.canonical_name,
                    "value": option.to_string(option.file_value),
                }
                appended = False
                for file in user_config:
                    if file.get("path", None) == option.file_source:
                        file.get("values").append(config_entry)
                        appended = True
                if not appended:
                    file_mapping = {
                        "path": option.file_source,
                        "values": [config_entry],
                    }
                    user_config.append(file_mapping)
