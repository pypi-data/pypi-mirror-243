...
def parse_buf_temp_conv_block(block, depthwise=False):
    ...
    Args:
        block (list(:obj:`tf.keras.Layer`)): the buffer_temp_conv block layers.
    ...
    block_params = dict(
        kernel_size=btc_layer.kernel_size,
...
def set_buffer_temp_conv_block_variables(layer_ak, block, depthwise=False):
    ...
    Args:
        ...
        block (list(:obj:`tf.keras.Layer`)): the buffer temporal convolution block layers.
        depthwise (bool, optional): boolean to declare the main layer as a depthwise layer.
            Defaults to False.
    """
    btc_layer = block[0]
