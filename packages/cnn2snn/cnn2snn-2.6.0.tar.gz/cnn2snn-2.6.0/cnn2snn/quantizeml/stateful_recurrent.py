...
def _set_intermediate_scaleout_variables(variables_ak, out_quantizer, scale_name, shift_name):
    """Computes and sets the scaleout intermediate variables.

    Args:
        variables_ak (:obj:`akida.Variables`): the targeted akida layer variables.
        out_quantizer (:obj:`quantizeml.OutputQuantizer`): the source output quantizer.
        scale_name (str): scales variable name.
        shift_name (str): shift variable name.
    """
    ...
    if hasattr(out_quantizer, 'qscales'):
        ...
        broadcast_and_set_variable(variables_ak, scale_name,
                                   output_scales)

    shift = out_quantizer.shift.value
    broadcast_and_set_variable(variables_ak, shift_name,
...
def _set_stateful_recurrent_variables(ak_layer, block):
    ...
    # Set intermediate input_proj scale_out variable
    _set_intermediate_scaleout_variables(variables_ak, in_proj_dense.out_quantizer,
                                         "in_proj_outputs_scales", "in_proj_outputs_shift")

    # Set StatefulRecurrent stateful_inputs_shift
    ...
    if stateful_inputs_shift is not None:
        stateful_inputs_shift_ak = stateful_inputs_shift.value.numpy().astype(np.uint8)
    stateful_inputs_shift_ak += stateful_rec.input_add_shift.value.numpy().astype(np.uint8)
    broadcast_and_set_variable(variables_ak, "stateful_inputs_shift",
                               stateful_inputs_shift_ak)
