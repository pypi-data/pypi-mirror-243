...
class SuggestionsTypes(str, Enum):
    options = 'options'
    option_groups = 'option_groups'
    no_suggestions_reason = 'no_suggestions_reason'


class NoSuggestionsReason(SwitBaseModel):
    message: str
...
class SuggestionsResult(SwitBaseModel):
    ...
    no_suggestions_reason: NoSuggestionsReason | None = None
    ...
    def check_suggestions_result(cls, values):
        ...
        suggestion_type = values.get('type')

        if options:
            if suggestion_type != SuggestionsTypes.options:
                raise ValueError('If options are set, type must be options')
        ...
        if option_groups:
            if suggestion_type != SuggestionsTypes.option_groups:
                raise ValueError('If option_groups are set, type must be option_groups')
        ...
        if no_suggestions_reason:
            if suggestion_type != SuggestionsTypes.no_suggestions_reason:
                raise ValueError('If no_suggestions_reason is set, type must be no_suggestions_reason')
