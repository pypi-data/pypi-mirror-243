...
@shared_task(typing=False, aio_variant=True)
def add_not_typed(x, y):
    """Add two numbers, but don't check arguments"""
    return x + y


@shared_task(ignore_result=True, aio_variant=True)
...
@shared_task(bind=True, aio_variant=True)
...
@shared_task(bind=True, default_retry_delay=1, aio_variant=True)
...
@shared_task(bind=True, expires=60.0, max_retries=1, aio_variant=True)
