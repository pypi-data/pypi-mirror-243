...
from .func_single_api import calculate_preseason_single_for_parallel

def calculate_preseason_parallel(y:np.ndarray, x:np.ndarray, **kwargs):
    """
    ...
    - x: np.ndarray, shape (rows, cols, months, vars)
    ...
    - result: np.ndarray, shape (rows, cols, 6, len(target_vars))
      In the 3rd dimension of result, the 6 elements are ['n', 'r', 'ci95lower', 'ci95upper', 'p-val', 'ops'], where ops is the optimal pre-season length.
    """
    ...
    target_vars = kwargs.get('target_vars', [f'x{i}' for i in range(vars)])
    ...
    with mp.Pool(processes=n_jobs) as pool:
        for res in tqdm(pool.imap_unordered(calculate_preseason_single_for_parallel, args_list), ncols=100, total=len(args_list)):
            result.append(res)
    ...
    result = np.stack([group_data.sort_values('index')[['n', 'r', 'ci95lower',	'ci95upper', 'p-val', 'ops']].values
                       .reshape(rows, cols, -1)
                       .astype(np.float32)
