...
class Translator:
    ...
    def __init__(self, filename: Optional[str] = None, suffix: Optional[str] = "_lang") -> None:
        ...
        """
        self.filename = filename or os.path.basename(__file__)
        try:
            with open(os.path.dirname(__file__) + "/" + self.filename + suffix + ".json", "r") as f:
                self.file = json.load(f)
        except:
            raise ValueError("No translation file found for {}.py".format(os.path.basename(self.filename)))
