...
libsocks5 lets you send traffic through SOCKS proxy servers. It is a modern fork of [SocksiPy](http://socksipy.sourceforge.net/) with bug fixes and extra features.
...
We highly recommend using the [requests](https://2.python-requests.org/en/master/) library for proxying HTTP traffic with SOCKS or HTTP proxies. It uses libsocks5 under the hood.

```python
requests.get(url, proxies={"http": "socks5://proxyhostname:9050", "https": "socks5://proxyhostname:9050"})
```

libsocks5 has an option for HTTP proxies, but it only supports CONNECT-based HTTP proxies, and in general we recommend using your HTTP client's native proxy support (such as requests' `proxies` keyword argument) rather than libsocks5'.
...
and you're ready to go. [Editor's note: it is better to use `python setup.py install` for libsocks5]
...
Please open a GitHub issue at https://github.com/Anorov/libsocks5
