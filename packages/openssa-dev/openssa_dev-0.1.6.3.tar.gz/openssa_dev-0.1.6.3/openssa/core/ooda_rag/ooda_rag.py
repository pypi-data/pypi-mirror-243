...
class Planner:
    ...
    def __init__(self, heuristics: Heuristic, max_subtasks: int = 3) -> None:
        self.heuristics = heuristics
        self.max_subtasks = max_subtasks

    def formulate_task(self, model: Model, history: History) -> str:
        response = model.get_response(OODAPrompts.FORMULATE_TASK, history)
        response = model.parse_output(response)
    ...
    def generative_decompose_task(self, model: Model, history: History) -> list[str]:
        response = model.get_response(OODAPrompts.DECOMPOSE_INTO_SUBTASKS, history)
        response = model.parse_output(response)
...
class Solver:
    def __init__(
        ...
        self.planner = Planner(task_heuristics)
        self.model = Model()
    ...
    def run(self, input_message: str, tools: dict) -> None:
        ...
        tool_message = OODAPrompts.PROVIDE_TOOLS.format(
    ...
    def synthesize_result(self) -> None:
        response = self.model.get_response(OODAPrompts.SYNTHESIZE_RESULT, self.history)
