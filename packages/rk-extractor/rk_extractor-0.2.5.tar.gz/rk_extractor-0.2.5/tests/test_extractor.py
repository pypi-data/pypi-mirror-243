...
def test_real_const_dset():
    ...
    rdr.cache_dir= 'tests/rk_extractor/real_const_dset/v4_csrdr'
    d_val, d_var = rdr.get_constraints()

    rdr          = np_rdr(sys='v65', sta='v63', yld='v24')
    rdr.cache    = True
    rdr.cache_dir= 'tests/rk_extractor/real_const_dset/v65_v63_v24'
    cv_sys       = rdr.get_cov(kind='sys')
    cv_sta       = rdr.get_cov(kind='sta')
    d_eff        = rdr.get_eff()
    d_rjpsi      = rdr.get_rjpsi()
    ...
    rdr_mc.cache    = False
    rdr_mc.cache_dir= 'tests/rk_extractor/real_const_dset/v4_mcrdr'
    d_mcmu          = rdr_mc.get_parameter(name='mu')
    d_mcsg          = rdr_mc.get_parameter(name='sg')

    rdr_dt          = mc_rdr(version='v4', real_data=True)
    rdr_dt.cache    = False
    rdr_dt.cache_dir= 'tests/rk_extractor/real_const_dset/v4_dtrdr'
