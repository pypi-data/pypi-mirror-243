...
class HealthCheck:
    ...
    def getEmbeddingFunction():
        functions ={
            "text-embedding-ada-002": embedding_functions.OpenAIEmbeddingFunction(api_key=config.openaiApiKey, model_name="text-embedding-ada-002"),
            "all-MiniLM-L6-v2": embedding_functions.SentenceTransformerEmbeddingFunction(model_name="all-MiniLM-L6-v2"),
            "all-mpnet-base-v2": embedding_functions.SentenceTransformerEmbeddingFunction(model_name="all-mpnet-base-v2"),
        }
        return functions.get(config.embeddingModel, embedding_functions.SentenceTransformerEmbeddingFunction(model_name=config.embeddingModel)) # support custom Sentence Transformer Embedding models by modifying config.embeddingModel
