...
class PageView(MethodView):
    ...
    def get(self, uri=""):
        ...
        headers = {
            "Cache-Control": f"public,max-age:{current_app.config.get('CACHE_DEFAULT_TIMEOUT', 0)},must-revalidate",
            "ETag": hashlib.md5(bytes(str(rendered), encoding="utf-8"), usedforsecurity=False).hexdigest(),
        }
        if rendered:
            if not isinstance(rendered, (str, int, float)):
                # return a json string
                return make_response((serialize_sqlite3_results(rendered), 200, headers))
            return make_response((rendered, 200, headers))
