...
class StateVectorSimulator(AbstractBackend):
    ...
    def measure(
        ...
        """The method of sampling random numbers measures a target qubit.

        Args:
            state: State vector before measuring.
            qnum: Number of qubits.
            target: Target qubit for measurement.

        Returns:
            res: Measurement result, 0 or 1.
            state: State vector after measuring.
    ...
    def sample(
        ...
        **kwargs,
        ...
        for _ in range(shots):
            qvec = self.copy(state)
            res = ""
            for qidx in mq:
                mres, qvec = self.measure(qvec, qnum, qidx)
                res += str(mres)
            answer[res] += 1
        return dict(answer)
