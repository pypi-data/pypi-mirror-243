...
- requests_filepath : str
    ...
    - e.g., {"model": "texts-embedding-ada-002", "input": "embed me"}
...
- token_encoding_name : str, optional
    ...
    - if omitted, will default to "cl100k_base" (used by `texts-embedding-ada-002`)
...
async def process_api_requests_from_session(
    ...
    requests = session.api.openai[request_type][job_id]["requests"].__iter__()
    ...
    logging.info(f"""Parallel processing complete. Results saved to {session._path if session._path else 'session'}""")
...
The example requests file at openai-cookbook/examples/data/example_requests_to_parallel_process.jsonl contains 10,000 requests to texts-embedding-ada-002.
...
n_requests = 10
jobs = [{"model": "texts-embedding-ada-002", "input": str(x) + "\n"} for x in range(n_requests)]
