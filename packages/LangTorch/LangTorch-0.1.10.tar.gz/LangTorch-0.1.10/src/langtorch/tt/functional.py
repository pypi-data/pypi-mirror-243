...
class MulTextTensor(Function):
    ...
    def forward(ctx, input1, input2):
        ...
        assert isinstance(input1, langtorch.TextTensor) and hasattr(input1, "content")
        assert isinstance(input2, langtorch.TextTensor) and hasattr(input2, "content")
    ...
    def backward(ctx, grad_output):
        ...
        print(grad_input1.shape, grad_input2.shape, input1.shape, input2.shape)
...
class JoinTextTensor(Function):
    ...
    def forward(ctx, input, on="", dim=None):
        ...
        if dim is not None and input.shape[dim] == 1: return input.squeeze(dim=dim)

        if dim is None:
            result = input.text_class()
