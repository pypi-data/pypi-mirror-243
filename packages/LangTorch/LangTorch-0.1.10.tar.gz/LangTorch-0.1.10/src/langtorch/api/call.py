...
def chat_strings(prompts, system_messages, model="gpt-3.5-turbo-0613", temperature=1, top_p=1, n=1,
    ...
    default_values = {"temperature": 1, "top_p": 1, "n": 1, "stop": None, "max_tokens": None, "presence_penalty": 0,
                      "frequency_penalty": 0, "functions": None, "function_call": "none"}
...
def chat(prompts, system_messages, model="gpt-3.5-turbo-0613", api_key=None, verbose=False, as_str=False, **kwargs):
    ...
    session = Session()
    if not hasattr(session._config, "api") or not hasattr(session._config.api, "openai"):
        session.api = {"openai": {"limits": dict(), "embeddings": dict(), "chat": dict()}}
    if "T" in kwargs:
    ...
    if not isinstance(prompts, list):  # If no save_filepath we assume we can skip saving
        if session._path is None and False:  # TODO: Fix this
            return call(prompts, system_messages[0], model=model)
        else:
            prompts = [prompts]
    ...
    return session.completions(job_id, "chat", "openai")


@override
def embed(texts, to_csv=True, as_np=False, api_key=None, verbose=False):
    """
    ...
    session = Session()
    session.api = {"openai": {"limits": {"embeddings": 0, "gpt-3.5": 0, "gpt4": 0}, "embeddings": [], "chat": []}}
    ...
    if isinstance(texts, TextTensor):
        ...
        shape = (len(texts),)
    if len(texts) == 1:
        output = openai.Embedding.create(input=str(texts[0]), model=model)['data'][0]['embedding']
        return pd.DataFrame({'texts': texts, 'embedding': [output]}) if to_csv else output

    # Request strings for jobs (I assume you have a list of jobs you want to process)
    requests = [{"model": model, "input": str(x).strip() + "\n"} for x in texts]

    job_id = hashlib.sha256(str(time.time()).encode()).hexdigest()
    session.add_requests("openai", "embedding", job_id, requests)
    ...
    return torch.vstack(session.completions(job_id, "embedding", "openai")).reshape(shape + (-1,))
...
if __name__ == "__main__":
    ...
    """]
    system_messages = [
        "You are a participant in a psychology study, your behaviour has been encoded in function inputs"]
