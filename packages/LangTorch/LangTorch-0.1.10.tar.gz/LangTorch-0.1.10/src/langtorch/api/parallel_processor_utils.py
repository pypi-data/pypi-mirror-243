    ...
    async def call_api(
        ...
        if error:
            ...
            if self.attempts_left:
                ...
                langtorch_session.add_responses("openai", request_type, job_id, [[self.request_json, response]])
            ...
            logging.debug(f"Request {self.task_id} saved to {job_id}.responses")
...
#                 status_tracker.num_tasks_failed += 1
#         else:
#             session.add_responses("gcp", type, job_id,[[self.request_json, response]])
#             status_tracker.num_tasks_in_progress -= 1
#             status_tracker.num_tasks_succeeded += 1
#             logging.debug(f"Request {self.task_id} saved to {job_id}")
