...
def get(table: str, value: str, where: str = None, and_where: list = None, fetchone: bool = True, cog_file: bool = True) -> str | None | list:
...
def edit(table: str, parameter: str, value: str, where: str = None, cog_file: bool = True):
    ...
    if where is None:
        ...
        cursor.execute(f'UPDATE {table} SET {parameter} = ? WHERE ? = ?', (value, where.split(' = ')[0], where.split(' = ')[1]))
...
def delete(table: str, value: str = None, where: str = None, and_where: list = None, cog_file: bool = True):
    # Get database file path
    path = inspect.getmodule(inspect.currentframe().f_back).__file__.replace('\\', '/').split('/')
    path.pop(-1)
    if cog_file is True:
