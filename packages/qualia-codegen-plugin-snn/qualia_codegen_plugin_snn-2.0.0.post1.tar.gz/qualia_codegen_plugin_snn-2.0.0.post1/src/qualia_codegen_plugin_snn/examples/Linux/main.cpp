...
float evaluate(const std::vector<std::array<float, InputDims>> &inputs, const std::vector<std::array<float, OutputDims>> &labels) {
	...
	for (size_t i = 0;  i < inputs.size() ; i++){ //&& i < labels.size()-1000; i++) {
		...
		for (int t = 0; t < MODEL_INPUT_TIMESTEPS; t++){
			...
			if (t==0) {
				for (size_t j=0; j < MODEL_OUTPUT_SAMPLES; j++){
					sum_outputs[j] = outputs[j] ;
				}
			}
			else {
				for (size_t j=0; j < MODEL_OUTPUT_SAMPLES; j++){
					sum_outputs[j] = sum_outputs[j] + outputs[j] ;
				}
			}
		}
		...
		// Mean Spikes
		for (size_t j=0; j < MODEL_OUTPUT_SAMPLES; j++){
			sum_outputs[j] = sum_outputs[j] / MODEL_INPUT_TIMESTEPS;
		}
#else
		...
		float max_val = sum_outputs[0];
		for (unsigned int i = 1; i < MODEL_OUTPUT_SAMPLES; i++) {
			if (max_val < sum_outputs[i]) {
				max_val = sum_outputs[i];
				label = i;
			}
		}
