...
class ExecuteProtocol:
    ...
    async def fetch_var(self, query: str, vars: dict = None, row_class: Type[T] = None):
        return await self._exec(
            method=fetch_var, query=query, vars=vars, row_class=row_class
        )

    async def fetch_one(self, query: str, vars: dict = None, row_class: Type[T] = None):
        return await self._exec(
            method=fetch_one, query=query, vars=vars, row_class=row_class
        )

    async def fetch_all(self, query: str, vars: dict = None, row_class: Type[T] = None):
        return await self._exec(
