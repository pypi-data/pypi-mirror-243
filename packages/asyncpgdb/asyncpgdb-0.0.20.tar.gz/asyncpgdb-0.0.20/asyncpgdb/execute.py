...
def _row_parser(
    __row_class: Type[T], attrs: Iterable[str] = ("parse_obj", "model_validate")
) -> Callable[..., T]:
...
async def fetch_one(
    ...
    return row if row is None else _row_parser(row_class or dict)(row)
...
async def fetch_all(
    ...
    if rows and isinstance(rows, list):
        ...
        result: List[row_class] = list(map(parse_row, rows))
    else:
        result = []
...
class ExecuteProtocol:
    ...
    async def fetch_var(self, query: str, vars: dict = None, row_class: Type[T] = None) -> T:
        return await self._exec(
            method=fetch_var, query=query, vars=vars, row_class=row_class
        )

    async def fetch_one(self, query: str, vars: dict = None, row_class: Type[T] = None) -> T:
        return await self._exec(
            method=fetch_one, query=query, vars=vars, row_class=row_class
        )

    async def fetch_all(self, query: str, vars: dict = None, row_class: Type[T] = None) -> List[T]:
        return await self._exec(
