...
def abnormal_detection_clean(
    ...
    for i in range(len(device_types)):
        if device_types[i] < -0.5:
            ...
            sensor_mask[i] = np.std(data_res[-device_checktimes[i]:, i]) / (np.abs(np.mean(data_res[-device_checktimes[i]:, i]))+0.001) < 0.05
    ...
    # 对3种异常检测结果整合
    abnormal_mask = np.logical_or(np.logical_or(sigma_mask[-1, :], actuator_mask), sensor_mask)
    alarm_res = []

    for i, tag in enumerate(tags):
        tmp = DetectionResult(name=tag.name, value=raw_data[-1, i], time=times[i].strftime("%Y/%m/%d %H:%M:%S"),direction=directions[i],
                              alarm=0, processed_res=-1, device_type= tag.device_type)
        ...
        if abnormal_mask[i] and not confirmed_flag:
            # 发现异常且60分钟内人工都未处理，告警
            tmp.alarm = 1 * sigma_mask[-1, i] + 2 * sensor_mask[i] * (not sigma_mask[-1, i]) + 3 * actuator_mask[i] * (not sensor_mask[i]) * (not sigma_mask[-1, i])
        else:
            # 未发现异常或者人工已经处理过, 不告警
            tmp.alarm = 0
