...
class EntryPoint:
            ...
            try:
                ...
                if not self.stop_flag:
                    ...
                    try:
                        ...
                        elif len(sig.parameters) == 7 and 'user_confirms' in sig.parameters:
                            tc = datetime.datetime.now()
                            clean_data, ab, _ = self.get_abnormal(tc, self.confirms)
                            func(self.storage, algo_cfg, clean_data,
                                timestamps, names, self, ab)
                            logging.info("called func with get_abnormal")
    ...
    def get_abnormal(self, ts:datetime.datetime, confirms:list):
        try:
            ...
            if not confirms:
                ...
                if siz != self.ab_detect_num_local_last and siz != 0:
                    _u = self.storage.local.io.lindex(self.ab_detect_key, siz - 1)
                    u = json.loads(_u)
            ...
            a,b = abnormal_detection_clean(d, tags, t, _confirms)
            return a,b,hist_len
    ...
    def _data_analysis_clean(self):
        ...
        while True:
            ...
            try:
                # TODO:
                _, ab, hist_len = self.get_abnormal(ts,self.confirms)
                print(ab, hist_len)
                if hist_len is None:
                ...
                siz = self.storage.local.io.llen(self.ab_detect_key)
                if siz > hist_len:
                    self.storage.local.io.lpop(self.ab_detect_key, siz - hist_len)
