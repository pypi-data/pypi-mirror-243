    ...
    for key in dict1.keys() & dict2.keys():
        if isinstance(dict1[key], dict) and isinstance(dict2[key], dict):
            ...
            if len(dict1[key]) != len(dict2[key]):
                diff[key] = (get_value_type(dict1[key]), get_value_type(dict2[key]))
            else:
                for i in range(len(dict1[key])):
                    if isinstance(dict1[key][i], dict) and isinstance(dict2[key][i], dict):
                        ...
                        if len(dict1[key][i]) != len(dict2[key][i]):
                            if key not in diff:
                                diff[key] = {}
                            diff[key][i] = (get_value_type(dict1[key][i]), get_value_type(dict2[key][i]))
                        else:
                            ...
                            for x in range(len(types1)):
                                if types1[x] != types2[x]:
                                    diff[key][i][x] = (get_value_type(dict1[key][i][x]), get_value_type(dict2[key][i][x]))
