...
class EntryPoint:
            ...
            if ab:
                ...
                if need_report:
                    ...
                    if res:
                        ...
                        send_notification(self.storage, 'ab_detect', msg)

                        fault = f"点位数值发生异常"
                        smartcall(self.storage, 'ab_detect', fault, 60)
                        logging.info(
                            "ab_detect notification and smartcall sent")

                    return d, ab_m, hist_len
                else:
                    return d, ab, hist_len
            else:
                logging.error(
                    "abnormal_detection_clean return List of ab None")
                return None, None, None
        except Exception as e:
            errmsg = traceback.format_exc()
    ...
    def _data_analysis_clean(self):
        ...
        while True:
            ...
            try:
                _, ab, hist_len = self.get_abnormal(ts, self.confirms, True)
                print(ab, hist_len)
                if hist_len is None:
