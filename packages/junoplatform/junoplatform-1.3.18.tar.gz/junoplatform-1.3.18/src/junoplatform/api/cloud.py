...
def send_notification(storage: Storage, msg: str):
    message = {
        "name": "junoplatform",
        "message": msg
    }
...
def smartcall(storage: Storage, func_key: str, fault: str, ban_minutes: int = 30):
    ...
    logging.info(
        f"smartcall key: {key}, smartcall configs: {junoconfig['smartcall']}")
    scr = storage.local.read(key, cast=dict)
    if scr and datetime.datetime.now().timestamp() - scr["params"]["ts"] < ban_minutes:
        logging.warn(
