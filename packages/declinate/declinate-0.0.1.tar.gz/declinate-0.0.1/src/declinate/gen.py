...
def inspect_command(command: Callable[P, list[str]]) -> CommandInfo:
    ...
    Raises:
        Exception: If command does not adhere to expectations.
    ...
    for argname, parameter in signature.parameters.items():
        ...
        if 'help' not in metadata:
            if annotation != bool and has_default and metadata.get('action') != 'count' \
...
def generate_code(package: str, *, write: bool) -> str:  # noqa: C901
    ...
    for key in defargs:
        assert key in root_commandinfo.args, \
    ...
    if subcommands := getattr(module, 'SUBCOMMANDS', None):
        ...
        for commandinfo in subinfos:
            ...
            if commandinfo.returntype is NoReturn:
                helpstrs = [] if commandinfo.shortdesc.startswith('SUPPRESS.') else [
    ...
    imports = [x for x in imports if 'Sequence' not in x]
    imports = isort.code('\n'.join(imports)).split('\n')
    res = [*HEADER, *imports, *HEADER_TYPE_CHECKING, *res, *completer_lines, *COMPLETE, *MAIN]
    ...
    if write:
        ...
        testcode = [
            ...
            f'    assert not (res := check_package({package!r})), res',
            '',
            '',
            'def test_cli() -> None:',
            '    """Cli."""',
