...
class ActivityPub:
    ...
    async def determine_instance_type(self: ActivityPubClass) -> None:
        ...
        if (
            response_dict["configuration"]
            and response_dict["configuration"]["statuses"]
            and (max_attachments := response_dict["configuration"]["statuses"]["max_media_attachments"])
        ):
            self.max_attachments = max_attachments
        elif max_attachments := response_dict["max_media_attachments"]:
            self.max_attachments = max_attachments
    ...
    async def __check_exception(response: aiohttp.ClientResponse) -> None:
        ...
        logger.debug("ActivityPub.__check_exception - response.status = %s", response.status)
