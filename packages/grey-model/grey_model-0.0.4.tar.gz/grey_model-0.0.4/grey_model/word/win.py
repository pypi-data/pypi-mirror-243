...
def plot(file,p_name):
    ...
    if if_drop.get():
        ...
        file = f"grey_model/word/ass/{an}_out.txt"
    try:
    ...
    plt.axis('off')
    error.set('运行成功~')
    with open('grey_model/word/ass/Read_must.txt', 'w') as f:
...
def drop_unless(filename):
    # 创建停用词列表
    def stopwordslist():
        stopwords = [line.strip() for line in open('grey_model/need/stop.txt',encoding='UTF-8').readlines()]
        return stopwords
    ...
    outfilename = f"grey_model/word/ass/{an}_out.txt"
    try:
        print('2')
