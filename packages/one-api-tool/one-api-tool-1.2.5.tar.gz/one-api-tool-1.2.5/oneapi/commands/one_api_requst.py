    ...
    if args.prompt and args.config_file:
        ...
        if f'{key_word}_API_KEY' in os.environ:
            ...
            if api_type in ["huggingface", "vllm"]:
                ...
                inquirer.Text("api_version", message=f"{key_word.title()} API version, Enter to use the default {key_word.title()} version"),
        ...
        tool = OneAPITool.from_config(api_key=api_key, api_base=api_base, api_type=api_type, api_version=api_version, chat_template=chat_template)
