...
from ...types.eval_dataset_create import EvalDatasetCreate
...
class ProjectClient:
    ...
    def update_existing_project(self, project_id: str, *, name: str) -> Project:
        ...
        Parameters:
            ...
            - name: str. <span style="white-space: nowrap">`non-empty`</span> <span style="white-space: nowrap">`<= 70 characters`</span>
        """
        _response = self._client_wrapper.httpx_client.request(
            "PUT",
            urllib.parse.urljoin(
    ...
    def create_eval_dataset_for_project(
        self, project_id: str, *, request: EvalDatasetCreate
    ) -> EvalDataset:
        """
    ...
    def upsert_eval_dataset_for_project(
        self, project_id: str, *, request: EvalDatasetCreate
    ) -> EvalDataset:
        """
        Upsert an eval dataset for a project.
        Updates if an eval dataset with the same name and project_id already exists. Otherwise, creates a new eval dataset.
...
class AsyncProjectClient:
    ...
    async def upsert_eval_dataset_for_project(
        self, project_id: str, *, request: EvalDatasetCreate
    ) -> EvalDataset:
        """
        Upsert an eval dataset for a project.
