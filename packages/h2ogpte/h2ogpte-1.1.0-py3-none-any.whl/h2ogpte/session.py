...
class Session:
    ...
    def query(
        ...
        Args:
            message: Query or instruction from the end user to the LLM.
            ...
            pre_prompt_query: Text that is prepended before the contextual document chunks. The default can be
            customized per environment, but the standard default is "Pay attention and remember the information
            below, which will help to answer the question or imperative after the context ends.\n"
            prompt_query: Text that is appended to the beginning of the user's message. The default can be customized
            ...
            pre_prompt_summary: Not yet used, use H2OGPTE.summarize_content
            prompt_summary: Not yet used, use H2OGPTE.summarize_content
            ...
            self_reflection_config: Dictionary of arguments for self-reflection, can contain the following string:string mappings:
                llm_reflection: str = \"gpt-4-0613\"  # \"\" to disable reflection
                pre_prompt_reflection: str = \"Pay attention to the following context. You will need to evaluate the quality of a response for a given prompt.\"
                prompt_reflection: str = 'Given the above context, here\'s the prompt and the response:\n\"\"\"Prompt:\n%s\n\"\"\"\n\n\"\"\"Response:\n%s\n\"\"\"\n\nWhat is the quality of the response for the given prompt? Respond with a score ranging from Score: 0/10 (worst) to Score: 10/10 (best), and give a brief explanation why.'
                system_prompt_reflection: str = \"\"
                llm_args_reflection: str = \"{}\"
            rag_config: Dictionary of arguments to control RAG (retrieval-augmented-generation) types. Can contain the following key/value pairs:
                rag_type: str = one of [
                            "llm_only",  # No RAG (LLM only)
                            "rag",       # RAG (embed query)
                            "hyde1",     # HyDE RAG (embed LLM response and query)
                            "hyde2",     # HyDE RAG+ (embed HyDE RAG response and query)
                        ]
                no_rag_llm_prompt_extension: str = '\nKeep the answer brief, and list the 5 most relevant key words at the end.'
            timeout: Amount of time in seconds to allow the request to run. The default is 1000 seconds.
            callback: Function for processing partial messages, used for streaming responses
            to an end user.
