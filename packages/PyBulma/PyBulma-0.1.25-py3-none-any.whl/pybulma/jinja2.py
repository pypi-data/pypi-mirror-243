...
class Jinja2Block(BaseComponent):
    ...
    def __init__(self, block: str, load_static: bool = True) -> None:
        super().__init__(item_id=None, styles=None, attributes=None, classes=None)
        self.block = block
    ...
    def _pre_template(self) -> str:
        result = []
        if self.load_static:
            result.append("{% load static %}")
        result.append("{% block " + self.block + " %}")
        return " ".join(result)
...
class Jinja2Extends(BaseComponent):
    name = "jinja2-extends"
    base_classes = []

    def __init__(self, base: str, load_static: bool = True) -> None:
        super().__init__(item_id=None, styles=None, attributes=None, classes=None)
        self.base = base
        self.load_static = load_static
