...
class AbstractHook:
    ...
    def deregister(self, interpreter):
        if interpreter is not self.interpreter:
            raise em.ConsistencyError("hook not associated with this interpreter")
        self.interpreter = None
...
class Hook(AbstractHook):
    ...
    def beforeExpand(self, string, locals, name): pass
    def afterExpand(self, result): pass

    def beforeTokens(self, tokens, locals): pass
    def afterTokens(self, result): pass

    def beforeFileLines(self, file, locals): pass
    def afterFileLines(self): pass

    def beforeFileChunks(self, file, bufferSize, locals): pass
    def afterFileChunks(self): pass

    def beforeFileFull(self, file, locals): pass
    def afterFileFull(self): pass

    def beforeString(self, string, locals): pass
