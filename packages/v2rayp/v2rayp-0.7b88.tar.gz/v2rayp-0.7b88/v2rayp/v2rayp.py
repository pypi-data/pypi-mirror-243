...
class MainGUI:
    ...
    def _read_gui_config(self):
        ...
        with open(
            ...
            if "chisel_address" not in self.gui_data:
                self.gui_data["chisel_address"] = ""
            if "chisel_port" not in self.gui_data:
                self.gui_data["chisel_port"] = ""
    ...
    def save_gui(self):
        ...
        with open(
            ...
            try:
                ...
                self.gui_data["chisel_address"] = self.settings["chisel_address"].get()
                self.gui_data["chisel_port"] = self.settings["chisel_port"].get()

            except Exception as e:
                print(f"Cannot save gui: {e}")
    ...
    def run_Chisel(self):
        ...
        self.chisel_interface = Chisel_Interface(
            self.GFW_port,
            self.settings["chisel_address"],
            self.cloudflare_port,
            self.settings["chisel_port"],
        )
