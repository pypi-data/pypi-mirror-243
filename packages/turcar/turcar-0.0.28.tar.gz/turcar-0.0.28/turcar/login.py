...
class UserLogin:
    ...
    def login(self):
        ...
        if existUser > 0:
            query_sql = f"SELECT COUNT(*) FROM {user_table} WHERE SUBSTRING(id_card_num, LENGTH(id_card_num)-5) = {account} AND SUBSTRING(phone, LENGTH(phone)-3) = {password} AND state = 1"
            cursor.execute(query_sql)
            fetchone = cursor.fetchone()
            existUser = fetchone['COUNT(*)']
            if existUser > 0:
