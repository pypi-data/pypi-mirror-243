    ...
    def check(self, env):
        ...
        for group, score in env.score.items():
            if score >= 20000.0:
                ...
                return True
        try:
            env.screen_manager.screen_state.set_text(f'L:{np.round(env.score["Left"], 1)} vs R:{np.round(env.score["Right"], 1)}')
        except:
            pass
        return False
    ...
    def check(self, env):
        ...
        if carrier is None:
            ...
            if env.r_t - dur > 0:
                ...
                return True
        try :
            env.screen_manager.screen_state.set_text(f'L:{np.round(dur - env.l_t, 2)} vs R:{np.round(dur - env.r_t, 2)}')
        except:
            pass
        return False
...
class CaptureFlagCondition:
    ...
    def check(self, env):
        ...
        if l_dst < 0:
            ...
            return True
        try:
            env.screen_manager.screen_state.set_text(f'L:{l_dst} vs R:{r_dst}')
        except:
            pass
        return False
