...
def match_expr(
    ...
    for line in map(str.strip, filter(None, pattern.splitlines())):
        if line.startswith(pattern_prefix):
            capture, expression = splittwo(
                line.removeprefix(pattern_prefix), return_partition
            )
...
def c_func_def(
    ...
    python_body = (
        f"def {func_name}({', '.join(parsed_args)}) -> {ret_type}:\n{' '*4}"
        + parse_clike_body(body, indent_level=1)
    )
