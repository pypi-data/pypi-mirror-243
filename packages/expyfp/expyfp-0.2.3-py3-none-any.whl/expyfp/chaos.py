...
class Inspector:
    obj: object
    limit: int = 2
    name_rule: Callable[[int, str], bool] = lambda limit, name: True  # noqa ; fmt: skip
            ...
            if self.name_rule(self.limit, _attrname):
                try:
                    self._memory[_attrname] = (
                        Inspector(getattr(obj, _attrname), self.limit - 1, self.name_rule)
                        if (self.limit - 1) != 0
...
def haskell(src, g=globals(), l=locals()): # noqa
    return eval(src.replace("\\", "lambda ", 1).replace("->", ":"), g, l)


def match(pattern: str, glob: dict[str, Any] = globals(), loc: dict[str, Any] = locals()) -> FunctionType:
...
def tryexcept(
    ...
    try:
        ...
        return exceptblock() if callable(exceptblock) else exceptblock if exceptblock is not None else None
...
def builder(pattern="with_"):
    return type(
        ...
        (),
        {
            "__getattribute__": lambda self, __name: (lambda value: [setattr(self, __name[len(pattern) :], value), self][-1])
