...
class ProviderSwitchHandler(object):
    ...
    def _prepare(self) -> None:
        ...
        providers = ProviderCollection(
            ...
            retry_when_not_found=self.retry_when_not_found, root_path=self.root_path, boto3_session=self.boto3_session).get_providers()
        if self.allowed_providers is not None:
            providers = [
                provider for provider in providers if provider.name in self.allowed_providers]
    ...
    def _request(self, method: str, url: str, headers: dict, params: dict, timeout: int = None) -> dict:  # noqa: C901
        ...
        content_type = response_headers.get(
            'content-type') if response_headers else None
        provider_data = {}
        rate_limit_keys = self.current_provider.config.get(
        ...
        if rate_limit_keys is not None and response_headers is not None:
            rate_limit_quotas_key = rate_limit_keys.get(
                'rate_limit_quotas_key', 0)
            rate_limit_remaining_key = rate_limit_keys.get(
                'rate_limit_remaining_key', 0)
            rate_limit_refresh_key = rate_limit_keys.get(
                'rate_limit_refresh_key', 0)

            quotas = int(response_headers.get(rate_limit_quotas_key, 0))
            quotas_remaining = int(
                response_headers.get(rate_limit_remaining_key, 0))
            quota_refresh_time = int(
                response_headers.get(rate_limit_refresh_key, 0))
    ...
    def _make_request(self, variables: dict) -> dict:  # noqa: C901
        ...
        try:
            ...
            timeout = int(provider.get_attr('timeout')
                          ) if provider.get_attr('timeout') else None
