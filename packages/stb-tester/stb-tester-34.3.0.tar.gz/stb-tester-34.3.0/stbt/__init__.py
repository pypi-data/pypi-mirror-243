...
def wait_for_content_start(
        ...
        timeout_secs: float = 30) -> list[tuple[float, str, Any]]:
...
class Learning(enum.Enum):
    ...
    #: Don't learn the menu structure for future calls to `navigate_1d`.
    NONE = 0

    #: Learn the menu structure to speed up future calls to `navigate_1d`
    #: during this test-run (that is, during the lifetime of the Python
    #: process).
    TEMPORARY = 1
    ...
    #: `navigate_1d` when running tests on the same Stb-tester Node.
    PERSISTENT = 2
...
class NavigationFailed(AssertionError):
    """Raised by `navigate_1d` if it couldn't find the requested target."""
...
def navigate_1d(
    ...
    """
    Navigate through a 1-dimensional menu, attempting to find a target.
