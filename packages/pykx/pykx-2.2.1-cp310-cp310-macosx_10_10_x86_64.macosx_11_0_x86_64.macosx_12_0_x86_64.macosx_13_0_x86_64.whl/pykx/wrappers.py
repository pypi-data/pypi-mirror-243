...
def _idx_to_k(key, n):
    ...
    if isinstance(key, Integral):
        ...
        key_start = _key_preprocess(key.start, n, slice=True)
        key_stop = _key_preprocess(key.stop, n, slice=True)
        key = slice(key_start if (key.start is not None) else 0,
                    key_stop if (key.stop is not None) else n,
                    key.step)
    return K(key)
