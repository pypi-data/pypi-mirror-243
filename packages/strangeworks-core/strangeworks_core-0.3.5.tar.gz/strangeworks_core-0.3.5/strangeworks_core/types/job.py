...
class Job(BaseModel, RemoteObject):
    ...
    is_platform_terminal_state: Optional[bool] = Field(
        default=None, alias=AliasChoices("is_terminal_state", "isTerminalState")
    )
    remote_job_status: Optional[str] = Field(
        default=None, alias=AliasChoices("remote_status", "remoteStatus")
    )
    job_data: Optional[Dict[str, Any]] = Field(
        default=None, alias=AliasChoices("job_data", "jobData", "data")
    )
    ...
    def remote_id(self) -> str:
        ...
        May return None if job is not being run remotely or is a batch job.
        """
    ...
    def remote_status(self) -> str:
        ...
        May return None if job is not running on a remote system or is a
        batch job.
        """
        return self.remote_job_status
