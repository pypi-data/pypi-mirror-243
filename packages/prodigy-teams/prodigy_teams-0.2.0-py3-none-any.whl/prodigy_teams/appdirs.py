...
Mostly taken from the `appdirs` package, with small modifications for testability
...
def user_data_dir(
    appname: ty.Optional[str] = None,
    appauthor: ty.Optional[ty.Union[str, bool]] = None,
    ...
    if system == "win32":
        ...
        if appname:
            if appauthor is not False:
                assert isinstance(appauthor, str)
                path = os.path.join(path, appauthor.encode(), appname.encode())
            else:
...
def user_log_dir(
    appname: ty.Optional[str] = None,
    appauthor: ty.Optional[ty.Union[str, ty.Literal[False]]] = None,
    version: ty.Optional[ty.Union[str, ty.Literal[False]]] = None,
    opinion: bool = True,
):
    ...
    """
    if system == "darwin":
        path = os.path.join(str(os.path.expanduser("~/Library/Logs")), appname or "")
    elif system == "win32":
        path = user_data_dir(appname, appauthor, version or "")
        version = False
        if opinion:
            path = str(path)
...
def _get_win_folder_from_registry(csidl_name):
    ...
    dir, _ = winreg.QueryValueEx(key, shell_folder_name)  # type: ignore
    return dir
...
def _get_win_folder_with_jna(csidl_name):
    ...
    buf = array.zeros("c", buf_size)  # type: ignore
...
if system == "win32":
    try:
        from ctypes import windll as _  # type: ignore
    except ImportError:
        try:
            import com.sun.jna as _  # type: ignore
        except ImportError:
            try:
                import winreg as _  # noqa
            except ImportError:
