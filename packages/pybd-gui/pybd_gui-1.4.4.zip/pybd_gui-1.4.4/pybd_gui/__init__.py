...
from pybd_gui.placement_viewer import placement_viewer
from pybd_gui.input_viewer import input_viewer
...
class pybd_gui(tk.Tk):
    def __init__(self):
        ...
        self.menu_debug.add_command(label="check block placement", \
                command=self.check_placement)
        self.menu_debug.add_command(label="refresh block placement", \
                command=self.refresh_block_placement)
        self.menu_debug.add_command(label="show all inputs", \
                command=self.show_all_inputs)
        self.menu_debug.add_command(label="colorful wires", \
                command=self.toggle_colorful_wires)
    ...
    def toggle_colorful_wires(self, *args, **kwargs):
        if self.colorful_wires:
            self.colorful_wires = False
            print("colorful_wires off")
        else:
            self.colorful_wires = True
            print("colorful_wires on")


    def show_version(self, *args, **kwargs):
        msg1 = "pydb_gui version: %s" % version
        backend_version = pybd.version
        msg2 = "backend version: %s" % backend_version
        msg = msg1 + '\n' + msg2
        showinfo(title="Version", message=msg)


    def check_placement(self, *args, **kwargs):
        #msg = "In check_placement"
        #showinfo(title="Trying to help....", message=msg)
        any_abs = self.bd.any_absolute()
        if not any_abs:
            msg = "Your block diagram does not have\nany blocks with absolute placement."
            showinfo(title="Trying to help....", message=msg)
            return None


        placed_blocks, unplaced_blocks = self.bd.find_placement_order()
        if len(unplaced_blocks) > 0:
            mydialog = placement_viewer(title="Block Placement Viewer", \
                    parent=self)
            mydialog.list_unplaced_blocks(unplaced_blocks)
            mydialog.grab_set()
        else:
            # say you think averything is fine
            msg = "All of your blocks seem to have a valid placement.\n\n" + \
                    "If you are still having problems, try 'refresh block placement'"
            showinfo(title="Trying to help....", message=msg)
            return None


        # if there are unplaced_blocks, pop up a dialog with a csv view


    def refresh_block_placement(self):
        self.bd.refresh_block_placements()
        self.on_draw_btn()


    def show_all_inputs(self, *args, **kwargs):
        mydialog = input_viewer(title="Input Viewer", \
                    parent=self)
        mydialog.list_inputs()
        mydialog.grab_set()
