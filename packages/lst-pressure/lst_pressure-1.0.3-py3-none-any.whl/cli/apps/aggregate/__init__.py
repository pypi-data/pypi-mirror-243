...
class Aggregate(AppInterface):
    ...
    def build(self) -> Self:
        ...
        self.parser_sub.add_argument(
            ...
            default=False,
            action="store_true",
            help=f"Echo the aggregation query back - useful for evaluating .sql file contents that would otherwise be opaque. For example: --echo {existing_reports[0]}",
        )
    ...
    def exe(self) -> Self:
        ...
        if not self.input:
            if not sys.stdin.isatty():
                ...
                error(
                    "No input provided. Please either provide a filepath (--input), or pipe from stdin"
                )
                exit(1)
        else:
