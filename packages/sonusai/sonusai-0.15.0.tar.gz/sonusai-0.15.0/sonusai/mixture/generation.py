...
def update_mixture(mixdb: MixtureDatabase,
    ...
    mixture, targets_audio = _initialize_target_audios(mixdb, mixture)
...
def _augmented_noise_audio(mixdb: MixtureDatabase, mixture: Mixture) -> AudioT:
    ...
    if noise_augmentation.ir is not None:
        audio = apply_ir(audio, read_ir(mixdb.impulse_response_file(noise_augmentation.ir)))

    return audio


def _initialize_target_audios(mixdb: MixtureDatabase, mixture: Mixture) -> tuple[Mixture, AudiosT]:
    from .augmentation import apply_augmentation
...
def _initialize_mixture_gains(mixdb: MixtureDatabase,
    ...
    target_audio = np.sum(targets_audio, axis=0)
