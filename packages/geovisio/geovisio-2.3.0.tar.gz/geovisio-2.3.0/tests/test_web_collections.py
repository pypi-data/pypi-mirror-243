...
def test_collections_filter(datafiles, initSequence):
    ...
    response = client.get("/api/collections?filter=updated >= '2023-12-31'")
    assert response.status_code == 200
    assert len(response.json["collections"]) == 0
    ...
    response = client.get("/api/collections?filter=updated BETWEEN '2018-01-01' AND '2023-12-31'")
    assert response.status_code == 200
    assert len(response.json["collections"]) == 1
...
def test_getCollectionImportStatus_empty_collection(client):
    ...
    # the empty item in the response is not pretty, but a bother to remove
...
def test_delete_sequence(datafiles, initSequenceApp, dburl, bobAccountToken, bobAccountID):
    ...
    # nor in the collection thumbnail route
...
def test_delete_1_sequence_over_2(datafiles, initSequenceApp, dburl, bobAccountToken, bobAccountID):
    ...
    assert len(pagination_links) == 3  # first page, so no prev
    for _rel, href in pagination_links.items():
        assert "filter=status+IN+('deleted','ready')" in href
...
@conftest.SEQ_IMGS
def test_user_collection_many_filters_sortby(datafiles, initSequence, defaultAccountID, dburl):
    client = initSequence(datafiles, preprocess=False)
    conftest.createManySequences(dburl)
    response = client.get(f"/api/users/{defaultAccountID}/collection?limit=50&sortby=created,updated&filter=updated > '2020-01-01'")
...
def test_user_collection_pagination_first2last(datafiles, initSequence, dburl, defaultAccountID, sortby):
    client = initSequence(datafiles, preprocess=False)
    conftest.createManySequences(dburl)
    ...
    while nextLink:
        ...
        for l in response.json["links"]:
            if l["rel"] == "child":
                receivedChildren.append(l)
                receivedSeqIds.append(l["id"])
            elif l["rel"] == "next":
                nextLink = l["href"]
    ...
    if sortby in ["updated", "-updated"]:
        ...
        with psycopg.connect(dburl) as conn:
            nb = conn.execute("SELECT COUNT(*) FROM sequences WHERE updated_at is not null").fetchone()[0]
        assert len(receivedSeqIds) == nb
        assert len(set(receivedSeqIds)) == nb
        ...
        assert len(receivedSeqIds) >= 1020 and len(receivedSeqIds) <= 1024
        assert len(set(receivedSeqIds)) >= 1020 and len(set(receivedSeqIds)) <= 1024
    else:
        assert len(receivedSeqIds) == 1024
        assert len(set(receivedSeqIds)) == 1024
...
def test_user_collection_pagination_last2first(datafiles, initSequence, dburl, defaultAccountID, sortby):
    client = initSequence(datafiles, preprocess=False)
    conftest.createManySequences(dburl)

    # Call first page to get last page URL
    response = client.get(f"/api/users/{defaultAccountID}/collection?limit=200&sortby={sortby}")
    assert response.status_code == 200
...
@pytest.mark.parametrize(
    ("min", "max", "dmin", "dmax", "direction", "expected"),
    (
        ##############################################
        # In ascending order
        #
        ...
        [
            ...
            "ASC",
            {
                ...
                "prev": "sortby=%2Bcreated&filter=created+%3C+'2021-01-01'",
                "next": "sortby=%2Bcreated&filter=created+%3E+'2021-01-15'",
                "last": "sortby=%2Bcreated&filter=created+%3C%3D+'2024-12-31'",
            },
        ],
        # Matches dataset bounds
        ["2020-01-01", "2024-12-31", None, None, "ASC", {}],
        ...
        [
            ...
            None,
            "ASC",
            {"next": "sortby=%2Bcreated&filter=created+%3E+'2021-01-15'", "last": "sortby=%2Bcreated&filter=created+%3C%3D+'2024-12-31'"},
        ],
        ...
        [
            ...
            "DESC",
            {
                ...
                "prev": "sortby=-created&filter=created+%3E+'2021-01-15'",
                "next": "sortby=-created&filter=created+%3C+'2021-01-01'",
                "last": "sortby=-created&filter=created+%3E%3D+'2020-01-01'",
            },
        ],
        # Matches dataset bounds
        ["2020-01-01", "2024-12-31", None, None, "DESC", {}],
        # Starting on dataset bounds
        [
            "2020-01-01",
            "2021-01-15",
        ...
        [
            ...
            None,
            "DESC",
            {"next": "sortby=-created&filter=created+%3C+'2021-01-01'", "last": "sortby=-created&filter=created+%3E%3D+'2020-01-01'"},
        ],
    ),
)
def test_get_pagination_links(dburl, tmp_path, min, max, dmin, dmax, direction, expected):
    app = create_app(
        {
            "TESTING": True,
            "PICTURE_PROCESS_DERIVATES_STRATEGY": "ON_DEMAND",
    ...
    with app.app_context():
        res = geovisio.web.collections.get_pagination_links(
            "stac_collections.getUserCollection",
            {"limit": 50, "userId": "1234"},
            "created",
            direction,
            (dmin or "2020-01-01", dmax or "2024-12-31"),
            (min, max),
        )
