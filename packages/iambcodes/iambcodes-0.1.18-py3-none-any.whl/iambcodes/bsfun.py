...
#
############################
def GP_PlateData(FileName, GrowthThresh=3, GVcorrect={'GVexp':1, 'eexp':0, 'Background':0}, GPsource='GP'):
    if GPsource == 'GP-de':
        skiprows = 23
        decimal = ','
        TimeUnit = 60
    ...
    myGr = df.loc[:, df.columns != 'Time (min)'].apply(lambda x: CorrectedOD(x, GVexp, eexp, Background))
