...
def fetch_from_dpaste(dpaste_link):
    try:
        response = requests.get(dpaste_link)
...
def import_build(input_string, offset_x=0, offset_y=0, offset_z=0):
    ...
    for i in range(len(import_parts)):
        ...
        colors=[]
        if items[5]:
            colors=items[5].split('+')
        if len(colors)>2:
            parts.append({"x": float(items[2])+offset_x, "y": float(items[3])+offset_y, "z": float(items[4])+offset_z, "index": index_counter, "part": items[0], 'active': active,'r':colors[0],'g':colors[1],'b':colors[2]})
        else:
            parts.append({"x": float(items[2])+offset_x, "y": float(items[3])+offset_y, "z": float(items[4])+offset_z, "index": index_counter, "part": items[0], 'active': active})
        index_counter+=1
        j+=1
    if(len(import_connections)>1):
        for i in range(len(import_connections)):
            wires = import_connections[i].split(",")
            createWireIndex(int(wires[0])+index_counter-j,int(wires[1])+index_counter-j)
...
def render():
    ...
    def draw_3d_parts():
        for part in parts:
            ...
            if 'r' in part:
                props = [float(part['r']), float(part['g']), float(part['b'])]
            else:
                props = []
            
            if part['active'] == True:
                blockColour = tuple((np.array(blockColours[part['part']]) + np.array([64, 64, 64])).tolist())
            else:
                blockColour = blockColours[int(part['part'])]
            ...
            normalized_color = (blockColour[0] / 255.0+transparency, blockColour[1] / 255.0+transparency, blockColour[2] / 255.0+transparency)
