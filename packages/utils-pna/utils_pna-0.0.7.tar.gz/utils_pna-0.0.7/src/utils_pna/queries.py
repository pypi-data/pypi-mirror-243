...
def get_lims_aoi(aoi):
    if (aoi is None) or (aoi.lower() == "areas pna"):
...
class QueryAPI:
    def __init__(self, gis, api, step_dates=5):
        self.gis = gis
    ...
    def query(self, dates=None, unpack=True, **args):
        if dates is None:
            end_date = datetime.now()
            start_date = end_date - timedelta(minutes=self.step_dates)
        ...
        dates_intervals = get_dates_in_intervals(start_date, end_date, interval_minutes=self.step_dates)
