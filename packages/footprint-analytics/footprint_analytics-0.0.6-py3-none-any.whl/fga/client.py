...
class FGA:
    ...
    def user_set(self, user_id: str, user_properties: Optional[dict] = None):
        if user_properties is None:
            self.configuration.logger.error("Empty identify properties")
        else:
            event = User(user_id=user_id, user_properties=user_properties)
            self.track(event)

    def user_set_once(self, user_id: str, key: str, value):
        user_properties = {}
        user_properties[key] = value
        event = User(user_id=user_id, user_properties=user_properties, event_properties={'user_operation': constants.IDENTITY_OP_SET_ONCE })
        self.track(event)
