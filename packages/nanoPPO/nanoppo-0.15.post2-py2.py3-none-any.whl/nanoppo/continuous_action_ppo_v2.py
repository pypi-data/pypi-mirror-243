...
def rescale_action(predicted_action, action_min, action_max):
    rescaled_action = action_min + (predicted_action - (-1)) / 2 * (
    ...
    for sgd_iter in range(sgd_iters):
        ...
        if verbose > 1 and iter_num % 1000 == 0:
            print(
                ...
                "value lr",
                value_lr,
                "policy param magnitude",
                policy_param_magnitude,
                "value param magnitude",
                value_param_magnitude,
                "policy gradient magnitude",
                policy_gradient_magnitude,
...
def train_env(env_name):
    if env_name == "MountainCarContinuous-v0":
        ...
        best_config = {
            ...
            "policy_lr": 6.965606472889233e-05,
            "value_lr": 5.337021407693641e-05,
            "weight_decay": 0.005615599464308048,
            "sgd_iters": 2,
            "use_gae": True,
            "batch_size": 256,
            "vf_coef": 0.4407130713151379,
            "entropy_coef": 4.796586082455764e-05,
        }
...
@click.option(
    ...
    type=click.Choice(
        ["PointMass1D-v0", "PointMass2D-v0", "MountainCarContinuous-v0", "Pendulum-v1"]
    ),
)
def cli(env_name):
