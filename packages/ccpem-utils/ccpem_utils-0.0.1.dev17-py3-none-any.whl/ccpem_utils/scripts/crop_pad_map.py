...
def main():
    ...
    if args.mask:
        ...
        mask_mapobj(input_mapobj, maskobj)
    elif args.contour:
        suffix += "_contoured"
        if not args.threshold:
            raise ValueError("Provide mask threshold")
        threshold_mapobj(input_mapobj, args.threshold)
    if args.crop:
        suffix += "_cropped"
        crop_dim = None
    ...
    if args.outfile:
        ...
        outfile += suffix
    if args.odir:
        map_output = os.path.join(args.odir, outfile)
    else:
        map_output = outfile
    write_newmapobj(input_mapobj, map_output=map_output)
