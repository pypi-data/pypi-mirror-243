...
def pad_array(arr, nx, ny, nz, fill_padding=None):
    ...
    gridshape = (arr.shape[0] + 2 * nz, arr.shape[1] + 2 * ny, arr.shape[2] + 2 * nx)
...
def apply_real_space_filter(
    ...
    for i in range(iter):
        ...
        if inplace:  # if filtered_array is returned
            filtered_array = arr
    if normzero_one:
...
def apply_tanh_filter(arr: np.ndarray, kernel_size: int = 5, inplace: bool = False):
    ...
    if inplace:
        arr[:] = generic_filter(
            arr, tanh_convolve, footprint=footprint_sph, mode="constant", cval=0.0
        )
