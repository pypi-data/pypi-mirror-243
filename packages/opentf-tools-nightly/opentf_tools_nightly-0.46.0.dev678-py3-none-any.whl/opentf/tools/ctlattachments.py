...
def _add_attachment_metadata(verbose, event, data):
    ...
    if verbose and (event.get('status', 0) != 0):
        data.update(
            {
                'status': event['status'],
                'message': ' '.join(event.get('logs', [])),
            }
        )
    return data
...
def _get_attachments_dict(workflow_id: str, verbose: bool) -> Dict[str, Any]:
    ...
    for event in results:
        for attachment in event['attachments']:
            ...
            _add_attachment_metadata(verbose, event, data)
