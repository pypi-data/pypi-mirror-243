...
from src.data.clickhouse_api import (create_clickhouse_database,
                                     create_clickhouse_table,
                                     drop_click_table, insert_clickhouse,
                                     query_exist_date,
                                     query_exist_max_datetime)
from src.data.data_trans import pd_to_tuplelist
from src.data.get_data import (get_jq_bars, get_jq_trade_days,
                               get_trade_days)
from src.data.mongodb_api import save_mongodb
    ...
    try:
        ...
        if len(pd_data) > 1000:
            ...
            pd_data = pd_data.assign(
                ...
                date=pd_data['datetime'].apply(lambda x: str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(x)))[0:10]),
...
if __name__ == '__main__':
    ...
    # save_trade_days()
    current_filename = './json/' + str(datetime.date.today()) + '.json'
    current_filename = './json/2023-10-27.json'
    print(current_filename)
    save_bars_from_json(current_filename)
