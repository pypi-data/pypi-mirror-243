...
from kraken_thing.kraken_actions import kraken_actions
...
class Thing:
    ...
    def api_get(self, url=None):
        ...
        content = thing_api.get(url, self.record_type, self.record_id)
    ...
    def api_post(self, url=None):
        API_URL = os.getenv('API_URL')
        url = url if url else API_URL
        record = self._db_dump()
        content = kr.json.dumps(record)
        return thing_api.post(url, content)

    def api_delete(self, url = None):
        API_URL = os.getenv('API_URL')
        url = url if url else API_URL
        return thing_api.delete(url, self.record_type, self.record_id)

    async def api_get_async(self, url=None):
        API_URL = os.getenv('API_URL')
        url = url if url else API_URL
        content = await thing_api.get_async(url, self.record_type, self.record_id)
        if not content or content is False:
            return False
        record = kr.json.loads(content)
        return self._db_load(record)

    async def api_post_async(self, url=None):
        API_URL = os.getenv('API_URL')
        url = url if url else API_URL
        record = self._db_dump()
        content = kr.json.dumps(record)
        return await thing_api.post_async(url, content)
    
    async def api_delete_async(self, url=None):
        API_URL = os.getenv('API_URL')
        url = url if url else API_URL
        return await thing_api.delete_async(url, self.record_type, self.record_id)
    ...
    def potential_actions(self):
        return kraken_actions.get(self.dump())
