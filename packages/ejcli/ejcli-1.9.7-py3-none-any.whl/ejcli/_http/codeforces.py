...
from ejcli._http.ejudge import get, post
...
class CodeForces(Backend):
    ...
    def _get(self, path, codes=set()):
        ...
        if code == 200 and not codes:
            ...
            if headers['Location'] == 'https://%s/enter?back=%2F'%self.host:
                raise EJError("Login required (contest not started?)")
            raise EJError("Got unexpected redirect %d (%s -> %s)" %(code, path, headers['Location']))
    ...
    def submissions(self, all=False):
        if all:
            ...
            if data['status'] != 'OK':
                raise EJError('Failed to load submissions.')
    ...
    def submit_solution(self, task, lang, text):
        ...
        if tasks is None:
            raise EJError("Submissions are closed.")
