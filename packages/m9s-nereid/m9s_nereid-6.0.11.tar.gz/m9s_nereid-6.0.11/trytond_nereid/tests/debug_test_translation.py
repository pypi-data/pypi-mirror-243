...
class TestTranslation(NereidTestCase):
    ...
    def test_0300_translation_update(self):
        ...
        after = IrTranslation.search([])
        b = set([b.id for b in before])
        a = set([a.id for a in after])
        #print('b - a', b - a)
        #print('a - b', a - b)
        for item in before:
            corr = IrTranslation.search([
                    ('src', '=', item.src),
                    ('lang', '=', new_lang.code),
                    ])
            if not corr:
                print(item.rec_name, item.src, item.module)
