...
class FugleAccount(Account):
    ...
    def __init__(self, config_path='./config.ini.example', market_api_key=None):
        ...
        if 'FUGLE_ACCOUNT' in os.environ:
            setup_keyring(os.environ["FUGLE_ACCOUNT"])

        if 'FUGLE_ACCOUNT_PASSWORD' in os.environ and 'FUGLE_ACCOUNT' in os.environ:

            keyring.set_password("fugle_trade_sdk:account", 
                                 os.environ['FUGLE_ACCOUNT'],
                                 os.environ['FUGLE_ACCOUNT_PASSWORD'])

        if 'FUGLE_CERT_PASSWORD' in os.environ and 'FUGLE_CERT' in os.environ:
            keyring.set_password("fugle_trade_sdk:cert", 
                                 os.environ['FUGLE_ACCOUNT'],
                                 os.environ['FUGLE_CERT_PASSWORD'])
