...
from sqlalchemy import String, or_, inspect
...
class PaginationUtils:
    ...
    def search_all_data(
        ...
        for column in where_columns:
            where_clauses.append(cast_if(getattr(entity, column), String).ilike(f"%{search_all}%"))
    ...
    def validate_required_search_filter(
        ...
        for field in query_dto_fields:
            if query_dto_fields[field].required and field not in search_fields:
                return False
    ...
    def generating_selected_relationships_and_columns(
        ...
        for field in query_dto_fields:
            if field in entity_relationships:
                if query_dto_fields[field].required or field in selected_columns:
            ...
            elif query_dto_fields[field].required and field not in selected_columns:
