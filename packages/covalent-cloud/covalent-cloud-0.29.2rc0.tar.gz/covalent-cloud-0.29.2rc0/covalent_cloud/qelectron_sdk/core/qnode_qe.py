...
from .qinfo import QElectronInfo, QNodeSpecs
from .qresult_utils import get_original_shots, re_execute
...
class QNodeQE(qml.QNode):
    ...
    def run_later(self, *args, **kwargs) -> QNodeFutureResult:
        ...
        return future_result(args, kwargs, temp_result)
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.device._async_run:
            # Return here during call from `run_later()`.
            return retval

        interface = self.device.qnode_specs.interface

        # Skip re-execution if interface for the following.
        if interface in {self.interface, "numpy", "autograd"}:
            return retval

        # Run through an overloaded circuit/device to loop `retval` into interfaces.
        return re_execute(retval, self.original_qnode, self.tape)(interface, *args, **kwargs)
    ...
    @contextmanager
    def override_gradient_fn(self, _gradient_fn):
        """
        Override the private `self._gradient_fn` to override the `gradient_fn`
        attribute (property).
        """
        self._gradient_fn, _tmp_gradient_fn = _gradient_fn, self._gradient_fn
