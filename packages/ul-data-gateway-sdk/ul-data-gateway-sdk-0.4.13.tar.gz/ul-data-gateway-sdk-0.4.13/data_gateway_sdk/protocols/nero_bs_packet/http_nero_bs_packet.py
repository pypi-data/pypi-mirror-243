...
class DevicePackage(BaseModel):
    ...
    def validate_protocol(cls, value: Union[str, DevicePackageProtocolTypeEnum], values: Dict[str, Any]) -> DevicePackageProtocolTypeEnum:
        if value == DevicePackageProtocolTypeEnum.unbp.value or value is DevicePackageProtocolTypeEnum.unbp:
            ...
            if values.get("data_unbp2", None) is not None:
                raise ValueError(f'data_unbp2 for protocol "{value}" must be None. "{type(values.get("data_unbp2", None)).__name__}" was given')
        ...
        if value == DevicePackageProtocolTypeEnum.unbp2.value or value is DevicePackageProtocolTypeEnum.unbp2:
            ...
            if not isinstance(values.get('data_unbp2', None), (dict, DataUnbp2)):
                raise ValueError(f'data_unbp2 for protocol "{value}" must be dict. "{type(values.get("data_unbp2", None)).__name__}" was given')
            if isinstance(value, str):
                return DevicePackageProtocolTypeEnum(value)
            return value
