...
from svn2git import GitMigrationHelper, SVN2GitOptions, svn_setup_authentication_cache, svn_setup_authentication_plain
...
def parse(to_parse_args: list[str]) -> SVN2GitOptions:
    ...
    if "allow_store_plaintext_password" in parsed_args and (not parsed_args.username or not parsed_args.password):
    ...
    if "allow_store_plaintext_password" in parsed_args and not parsed_args.realm:
