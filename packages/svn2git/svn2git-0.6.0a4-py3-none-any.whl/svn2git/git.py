...
class GitMigrationHelper:
    ...
    def clone(
        ...
        run_command(
            f"GIT_TERMINAL_PROMPT=0 git config --global init.defaultBranch {self._initial_branch_name}", cwd=self._cwd
        )

        # Initialize repository
        logger.info("Initializing repository...")
        init_command = f"GIT_TERMINAL_PROMPT=0 git svn init --prefix={self._svn_remote_prefix}/ "
        if username:
        ...
        run_command(f"GIT_TERMINAL_PROMPT=0 git config --global --add safe.directory {os.getcwd()}", cwd=self._cwd)
    ...
    def get_branches(self, tags_base: str = "tags") -> GitBranches:
        ...
        result_local = run_command(
            "GIT_TERMINAL_PROMPT=0branch -l --no-color --format='%(refname:short)'",
            exit_on_error=True,
            return_stdout=True,
            cwd=self._cwd,
        )
    ...
    def fix_tags(self, branches: GitBranches, tags_base: str = "tags") -> None:
        ...
        for tag in branches.tags:
            ...
            run_command(
                f'GIT_TERMINAL_PROMPT=0tag -a -m "{escape_quotes(subject)}"'
                f'"{escape_quotes(tag_id)}" "{escape_quotes(tag)}"',
                cwd=self._cwd,
            )
    ...
    def fix_branches(self, branches: GitBranches, rebase: bool = False, trunk_branch: str = "trunk") -> None:
        ...
        if rebase:
            ...
            run_command("GIT_TERMINAL_PROMPT=0svn fetch", cwd=self._cwd)
        ...
        for svn_branch in svn_branches:
            ...
            if rebase and (branch in branches.locals or branch == trunk_branch):
                ...
                run_command(f'GIT_TERMINAL_PROMPT=0rebase "remotes/{self._svn_remote_prefix}/{branch}"', cwd=self._cwd)
                continue
    ...
    def push_branches(
        ...
        if large_repository_mode:
            ...
            for idx, commit in enumerate(commits):
                ...
                stdout = run_command(
                    f'GIT_TERMINAL_PROMPT=0push origin "{commit}:refs/heads/{self._initial_branch_name}"',
                    exit_on_error=False,
                    return_stdout=True,
                    cwd=self._cwd,
                )
                if stdout.find("GIT_TERMINAL_PROMPT=0pull") != -1:
            ...
            for branch in branches.locals:
                ...
                for idx, commit in enumerate(commits):
                    logger.debug(f"Pushing commit {commit} ({idx + 1}/{commits_len})...")
                    stdout = run_command(
                        f'GIT_TERMINAL_PROMPT=0push origin "{commit}:refs/heads/{branch}"',
                        exit_on_error=False,
                        return_stdout=True,
                        cwd=self._cwd,
                    )
        ...
        run_command(f"GIT_TERMINAL_PROMPT=0push -u {self._git_remote} --all", cwd=self._cwd)
    ...
    def push_tags(self) -> None:
        ...
        run_command(f"GIT_TERMINAL_PROMPT=0push -u {self._git_remote} --tags", cwd=self._cwd)
