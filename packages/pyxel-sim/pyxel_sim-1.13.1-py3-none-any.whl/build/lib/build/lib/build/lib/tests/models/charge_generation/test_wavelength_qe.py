...
def valid_qe_dataset(
    ...
    valid_qe = np.array([
        [4.00000e+02, 4.19078e-01],
        [4.20000e+02, 5.25519e-01],
        [4.40000e+02, 6.16757e-01],
        [4.60000e+02, 6.96008e-01],
        [4.80000e+02, 7.65085e-01],
        [5.00000e+02, 8.21701e-01],
       ])
...
def test_conversion_with_qe_valid(ccd_5x5: CCD, valid_qe_map_path: Union[str, Path]):
    ...
    load_qe_curve(detector=detector, filename=valid_qe_map_path)
...
def test_simple_conversion_invalid(ccd_5x5: CCD, invalid_qe_map_path: Union[str, Path]):
    with pytest.raises(
        ...
        load_qe_curve(detector=ccd_5x5, filename=invalid_qe_map_path, wavelength_col_name="wave"
...
def sample_qe_dataset():
    ...
    return xr.Dataset({"QE": ("wavelength", qe_values)}, coords={"wavelength": wavelengths})
