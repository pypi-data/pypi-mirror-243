...
class IOTests(BaseTestCase):
    ...
    def test_Directories_callbackFiltering_1(self):
        nestedInfo = self._tmpNestedFolder()
        tmpList = []
        sw.io.directory.List(nestedInfo.entryPath, tmpList.append, filter = lambda x: x.endswith(".txt") or x.endswith(".unkown"))
    ...
    def test_Directories_regexFiltering_1(self):
        ...
        tmpList = []
        sw.io.directory.List(nestedInfo.entryPath, tmpList.append, filter=r"/\.(unkown|txt)/i")
        self.assertEqual(len(tmpList), nestedInfo.FileOfTypeTextCount)
        for i in tmpList:
            pi = PathInfo(i)
            self.assertEqual(pi.FileExtension, "txt")
    ...
    def test_Directories_regexFiltering_2(self):
        ...
        tmpList = []
        sw.io.directory.List(nestedInfo.entryPath, tmpList.append, filter=r"/\.(bin)$/i")
        for path in tmpList:
            self.assertEqual(
    ...
    def test_Directories_regexFiltering_3(self):
        nestedInfo = self._tmpNestedFolder()
        tmpList = []
        sw.io.directory.List(nestedInfo.entryPath, tmpList.append, filter=r"/\.(unkown)$/")
        self.assertEqual(len(tmpList),  0)
        return
    ...
    def test_Directories_regexFiltering_AllFiles_1(self):
        nestedInfo = self._tmpNestedFolder()
        tmpList = []
        sw.io.directory.List(nestedInfo.entryPath, tmpList.append, filter=r"/\.(bin|txt)$/")
        self.assertEqual(len(tmpList),  nestedInfo.FileCount)
        return
    ...
    def test_Directories_regexFiltering_AllFiles_2(self):
        nestedInfo = self._tmpNestedFolder()
        tmpList = []
        sw.io.directory.List(nestedInfo.entryPath, tmpList.append, filter=r"/.*/i")
        self.assertEqual(len(tmpList),  nestedInfo.SubEntriesCount)
    ...
    def test_Directories_include_filter(self):
        ...
        files = []
        sw.io.directory.List(tmpdir, lambda x: files.append(x), filter=r'/\.txt$/')
