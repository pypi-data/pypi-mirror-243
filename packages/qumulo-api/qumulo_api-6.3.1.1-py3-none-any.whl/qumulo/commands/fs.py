...
class UserMetadataSet(qumulo.lib.opts.Subcommand):
    ...
    def options(parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            '--binary',
            help='Metadata value is hex-encoded binary',
            dest='binary',
            action='store_true',
        )
        parser.add_argument('--value', help='Metadata value', type=str_decode, required=True)
...
def make_metadata_table(metadata: Sequence[Dict[str, bytes]], display_headers: bool = True) -> str:
    TRIM = 50
    ...
    for m in metadata:
        ...
        try:
            ...
            value = 'Hex: ' + binascii.hexlify(m['value']).decode('utf-8')
    ...
    def main(rest_client: RestClient, args: argparse.Namespace) -> None:
        ...
        if not args.json:
            ...
            for _, result in enumerate(iterator):
                print(make_metadata_table(result.data['entries'], print_header))
                print_header = False
        else:
            vals = []
            for _, result in enumerate(iterator):
                ...
                for entry in entries:
                    ...
                    vals.append({'key': key, 'value': value, 'val_format': val_format})
