...
def select_installed_catalogs(api: Api):
    """
    ...
    """
    response = api.get(
        api.config["domain"] + "api/catalogues/getList"
    ...
    print(
        "The following catalogs are currently available on your RegScale installation:\n"
    )
    ...
    while True:
        ...
        catalog_number_to_update = input(
            "\nEnter the # of the catalog you wish to update on your target system,"
            " or type STOP to exit: "
        )
        if catalog_number_to_update.isdigit() and int(catalog_number_to_update) in ids:
            return catalog_number_to_update
...
def get_update_file(api: Api, catalog_number_to_update: int) -> Union[str, bytes]:
    ...
    """
    response = api.get(
        f"{api.config['domain']}{API_CATALOGUES_}{catalog_number_to_update}"
    )
...
def read_update_from_disk(update_sourcefile: str) -> bytes:
    ...
    try:
        ...
        error_and_exit(
            f"Error encountered when trying to read {update_sourcefile}. Unable to continue: {e}"
        )
...
def load_updated_catalog(update_source: Union[str, bytes]) -> dict:
    ...
    try:
        ...
        # TODO: Go back and reformat all the legacy catalogs so I can get rid of this hacky stuff
            ...
            for key in updated_catalog["catalogue"].keys():
                new_format_updated_catalog[key] = updated_catalog["catalogue"][key]
...
def load_existing_catalog(api: Api, catalog_number_to_update: int) -> dict:
    ...
    logger.info(
        "Loading data from existing version of catalog on RegScale installation."
    )
    existing_catalog = api.get(
        f"{api.config['domain']}{API_CATALOGUES_}{catalog_number_to_update}"
    ).json()
    # Controls
    # returns a list of abbreviated records
    controls = api.get(
        f"{api.config['domain']}/api/SecurityControls/getList/{catalog_number_to_update}"
    ).json()
    ...
    for control in track(
        ...
        control = api.get(
            f"{api.config['domain']}{API_SECURITY_CONTROLS_}{control['id']}"
        ).json()
        if (
            "objectives" in control
        ):  # currently is a bug where api get endpoint returning empty lists for these
    ...
    existing_catalog["objectives"] = api.get(
        f"{api.config['domain']}api/controlObjectives/getByCatalog/{catalog_number_to_update}"
    ).json()
    logger.info(
        "Received %i objective(s) from RegScale...", len(existing_catalog["objectives"])
    )
    ...
    for control in track(
        ...
        more_parameters = api.get(
            f"{api.config['domain']}api/controlParameters/getByControl/{control['id']}"
        ).json()
        control_parameters = control_parameters + more_parameters
        # Tests
        more_tests = api.get(
            f"{api.config['domain']}api/controlTestPlans/getByControl/{control['id']}"
        ).json()
        control_test_plans = control_test_plans + more_tests
        # CCIs
        more_ccis = api.get(
            f"{api.config['domain']}api/cci/getByControl/{control['id']}"
        ).json()
...
def process_catalog_update(
    api: Api, new_version_catalog: dict, existing_catalog: dict, dryrun: bool
) -> None:
    """
...
def check_controls(
    ...
    (
        ...
        id_key_name="controlId", old_records=existing_controls, new_records=new_controls
    )
    ...
    check_controls_do_updates(
        api, dryrun, existing_map, ignore_keys, new_map, track_changes, update_ids_set
    )

    # CREATE NEW
    archived = []  # don't upload a control if it's already in archived status
    check_controls_do_create(
        api, archived, create_ids_set, dryrun, existing_controls, new_map, track_changes
    )

    # PROCESS ARCHIVED
    check_controls_do_archived(
        api, archive_ids_set, dryrun, existing_map, track_changes
    )
    ...
    for change in track_changes:
        if change["field"] == "archived" and change["new_value"] is True:
            archived_controls.append(
                existing_map[change["id"]]["id"]
            )  # note id of control w/ archived updated to true
    return archived_controls


def check_controls_do_archived(
    api: Api, archive_ids_set: set, dryrun: bool, existing_map, track_changes: list
) -> None:
    """
    ...
    :param existing_map: hashmap of identifiers and complete records
    ...
    if len(archive_ids_set) > 0:
        ...
        for control_id in archive_ids_set:
            if (
                existing_map[control_id]["archived"] is False
            ):  # skip if already archived status
                archive_record(
                ...
                if dryrun is False:  # but ONLY if this is NOT a dry run
                    ...
                    response = api.put(
                        f"{api.config['domain']}{API_SECURITY_CONTROLS_}{existing_map[control_id]['id']}",
                        json=existing_map[control_id],
                    )
                    if not response.ok:
                        logger.error(
                            f"Response {response.status_code} - Trouble archiving with URL: {response.request.url}"
                        )
                    else:
                        logger.info(
                            f'Archived Control #{existing_map[control_id]["id"]}: '
                            f'{existing_map[control_id]["controlId"]}'
                        )
    ...
    if len(create_ids_set) > 0:
        ...
        for control_id in create_ids_set:
            ...
            if dryrun is False:  # only post updates if this is not a dry run
                ...
                response = api.post(
                    url=api.config["domain"] + API_SECURITY_CONTROLS_,
                    json=new_map[control_id],
                )
                if not response.ok:
                    logger.error(
                        f"Response {response.status_code} - Trouble posting to URL: {response.request.url}"
                    )
                else:
                    response_id = json.loads(response.content)["id"]
                    logger.info(
                        f'Created Control {new_map[control_id]["controlId"]} (ID# {response_id})'
                    )
...
def check_controls_do_updates(
    ...
    for control_id in update_ids_set:
        current_changes_count = len(
            track_changes
        )  # note size of track changes before updates
        update_record(
            existing_record=existing_map[control_id],
        ...
        if current_changes_count < len(
            track_changes
        ):  # if any changes were recorded for this control
            if dryrun is False:  # but ONLY if this is NOT a dry run
                response = api.put(
                    f"{api.config['domain']}{API_SECURITY_CONTROLS_}{existing_map[control_id]['id']}",
                    json=existing_map[control_id],
...
def check_child_records(
    ...
    # PROCESS ARCHIVES
    check_child_do_archives(
        api, archive_ids_set, dryrun, endpoint, existing_map, record_type, track_changes
    )
...
def check_child_do_create(
    ...
    if len(create_ids_set) > 0:
        ...
        for identifier in create_ids_set:
            ...
            control_mapped = hacky_fix_for_catalog_data_structure(
                existing_controls, identifier, new_controls, new_map
            )
...
def do_create(
    ...
    if control_mapped is True and new_map[identifier]["archived"] is False:
        ...
        if dryrun is False:
            response = api.post(
                url=api.config["domain"] + endpoint, json=new_map[identifier]
            )
            if not response.ok:
                logger.error(
                    f"{response.status_code} - Trouble creating new record with: {response.request.url}"
                )
            else:
                logger.info(
                    f'Created {record_type} {identifier} ID {json.loads(response.content)["id"]})'
                )
    else:
        logger.warning(
...
def check_child_do_archives(
    ...
    if len(archive_ids_set) > 0:
        ...
        for identifier in archive_ids_set:
            if (
                existing_map[identifier]["archived"] is False
            ):  # skip if already archived status
                archive_record(
                ...
                if dryrun is False:  # but ONLY if this is NOT a dry run
                    ...
                    response = api.put(
                        f"{api.config['domain']}{endpoint}/{existing_map[identifier]['id']}",
                        json=existing_map[identifier],
                    )
                    if not response.ok:
                        logger.error(
                            f"Response {response.status_code} - Trouble archiving with: {response.request.url}"
                        )
                    else:
                        logger.info(
                            f'Archived {record_type} #{existing_map[identifier]["id"]}: {identifier}'
                        )
...
def check_child_do_updates(
    ...
    for identifier in update_ids_set:
        ...
        handle_archived(
            archived_controls, existing_map, identifier, record_type, track_changes
        )
        #
        if current_changes_count < len(
            track_changes
        ):  # if changes recorded for this objective
            if dryrun is False:  # but ONLY if this is NOT a dry run
                response = api.put(
                    f"{api.config['domain']}{endpoint}/{existing_map[identifier]['id']}",
                    json=existing_map[identifier],
                ...
                if not response.ok:
                    logger.error(
                        f"Response {response.status_code} - Trouble updating to URL: {response.request.url}"
                    )
                else:
                    logger.info(
                        f'Updated {record_type} #{existing_map[identifier]["id"]}: {identifier}'
                    )
...
def handle_archived(
    ...
    for control_id in archived_controls:
        if (
            ...
            logger.info(
                f"Inheriting archived status of parent control for {record_type}: {identifier}"
            )
            archive_record(
                existing_record=existing_map[identifier],
                track_changes=track_changes,
...
def check_catalog_metadata(
    ...
    if current_changes_count < len(
        track_changes
    ):  # if changes recorded in this section
        ...
        if dryrun is False:  # ONLY if this is NOT a dry run
            response = api.put(
                ...
                + API_CATALOGUES_
                + str(existing_catalog["id"]),
                json=existing_catalog,
            )
            if not response.ok:
                logger.error(
                    f"Response {response.status_code} - 424 Trouble updating catalog: {response.request.url}"
                )
            else:
                logger.info(
                    f'Updated catalog metadata for #{existing_catalog["id"]}: {existing_catalog["title"]}'
                )
...
def define_operations(
    id_key_name: str, old_records: list, new_records: list
    ...
    archive_ids_set = (
        existing_id_set - new_id_set
    )  # archive objects found in old version of catalog but not in the new
    create_ids_set = (
        new_id_set - existing_id_set
    )  # create as new objects found in new but not old
    update_ids_set = (
        existing_id_set & new_id_set
    )  # update existing if found in both old and new
...
def update_record_fields_removed(
    ...
    ) - ignore_keys:  # fields that were in old version but not new are set to null
        if (
            existing_record[key] != "" and existing_record[key] is not None
        ):  # would ignore if it field already empty
            track_changes.append(
                {
                    "operation": "update",
                    "record_type": record_type,
...
def update_record_new_fields(
    ...
    ) - ignore_keys:  # are any new fields present that were not in the old version?
        # ----
        track_changes.append(
            {
                "operation": "update",
...
def update_record_latest_field_data(
    ...
    for key in (
        existing_keys & new_keys
    ) - ignore_keys:  # where same keys in both, check each field for new data
        if (
            ...
            existing_record[key] = new_record[
                key
            ]  # and overwrite existing with new data for this field
