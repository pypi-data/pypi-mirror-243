...
def confirm(prompt=None, default="y"):
    ...
    prompt += " [y,n]?".replace(default.lower(), default.upper())
    while True:
        ln = input(prompt)
        if not ln:
            ln = default
        if (lnl := ln.lower()) in ('y', 'j', 'o'):
            return True
        elif lnl == 'n':
            return False
        print("Please answer Y or N")

def confirm_else(prompt=None, default="e", otherwise=None, okwargs={}):
    prompt += " [y/n/e]?".replace(default.lower(), default.upper())
    while True:
        ln = input(prompt)
        if not ln:
            ln = default
        if (lnl := ln.lower()) in ('y', 'j', 'o'):
            ...
            if otherwise is not None:
                return otherwise(**okwargs)
            else:
                return confirm("Parameter otherwise (else) is None, defaulting to 'Y'")
        if otherwise is not None:
            print("Please answer Y or N")
        else:
            print("Please answer with Y, N or E")
