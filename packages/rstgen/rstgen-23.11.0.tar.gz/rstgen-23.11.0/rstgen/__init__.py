...
SETUP_INFO = {'url': 'https://gitlab.com/lino-framework/rstgen'}
...
class Column(object):
    ...
    def adjust_width(self, row):
        ...
        for ln in s.splitlines():
            width = column_width(ln)
            if self.width is None or self.width < width:
                self.width = width
                # if self.width > 500:
...
def _write_header(writeln, level, s):
    width = column_width(s)  # Avoid warning "Title overline too short" for
                             # headings with wide unicode chars
    if level == 1:
        writeln('=' * width)
    elif level == 2:
        writeln('-' * width)
    ...
    def format_row(self, row):
        ...
        for c in self.cols:
            ...
            for ln in cell.splitlines():
                ...
                lines[c.index].append(ln + (" " * (c.width - column_width(ln))))
        height = 1
        for c in self.cols:
