...
class Course(BaseModel):
    RP : str | None                  = Field(description='Prerequisites of the course.')
    seats: int | str              = Field(description='```"Inact"``` means registration isn\'t open yet. \n\n```"Cancel"``` means that the course is cancelled.')
    waitlist: int | str | None = Field(description='```null``` means that the course has no waitlist (ie MATH 1183 & MATH 1283). \n\n```"N/A"``` means the course does not have a waitlist.')
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "rpt_limit" : None,
                "notes" : None,
                "schedule" : [ScheduleEntry.Config.json_schema_extra["example"]],
            }
        }
...
class Semester(BaseModel):
    ...
    year: int                     = Field(description='Year of semester.')
    term: int             = Field(description='Term of semester')
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "courses" : [Course.Config.json_schema_extra["example"]]
            }
        }
