...
def pk_id_validation(source_query: str, destination_query: str, source: Source, destination: Destination, pk_id:str):
    try:
        source_tbl_name, dest_tbl_name = create_query_result_table(source, destination, source_query, destination_query)
        max_min_id = source.execute_sql(f"select min({pk_id}), max({pk_id}) from {source_tbl_name}")[0]
        min_id = max_min_id[0]
        max_id = max_min_id[1]
        ...
        while cur_id <= max_id:
            ...
            ids_source = set()
            source_result = source.execute_sql(f'select {pk_id} from {source_tbl_name} where {pk_id} >= {cur_id} order by {pk_id} asc limit 10000;')
            ...
            ids_dest = set()
            dest_result = destination.execute_sql(f'select {pk_id} from {dest_tbl_name} where {pk_id} >= {cur_id} and {pk_id} <={cur_max_id} order by {pk_id} asc;')
        ...
        logger.error(f"pk_id_validation error: {e}")
        raise Exception(e)