...
def process_partition_filter(source:Source, dest:Destination,
                             source_table: str, destination_table: str, partition_columns: list,
                             partition_values: list, is_source_partition: bool, is_dest_partition: bool):

    if is_source_partition:
        assert len(partition_columns) == len(partition_values), "Partition columns and partition values should have the same length"
        cols = source.get_table_columns(source_table.split('.')[0], source_table.split('.')[1])
        for partition in partition_columns:
            if partition not in [col.name for col in cols]:
                logger.error(f"Partition column {partition} not in source table {source_table}")
                return source_table, destination_table
        partition_sql = " where "
        for index, column in enumerate(partition_columns):
            partition_sql += f"{column} = '{partition_values[index]}' and "
        source_table = f"{source_table}{partition_sql[:-4]}"
    if is_dest_partition:
        assert len(partition_columns) == len(partition_values), "Partition columns and partition values should have the same length"
        cols = dest.get_table_columns(destination_table.split('.')[0], destination_table.split('.')[1])
        for partition in partition_columns:
            if partition not in [col.name for col in cols]:
                logger.error(f"Partition column {partition} not in destination table {destination_table}")
                return source_table, destination_table
        partition_sql = " where "
        for index, column in enumerate(partition_columns):
            partition_sql += f"{column} = '{partition_values[index]}' and "
...
def count_validation(source_table: str, destination_table: str, source: Source, destination: Destination, is_source_partition: bool, is_dest_partition: bool, partition_columns: list, partition_values: list):
    try:
        source_table, destination_table = process_partition_filter(source, destination,source_table, destination_table, partition_columns, partition_values, is_source_partition, is_dest_partition)
...
def pk_id_column_validation(source_table: str, destination_table: str, source: Source, destination: Destination, pk_id: str, is_source_partition: bool, is_dest_partition: bool, partition_columns: list, partition_values: list):
    try:
        source_table, destination_table = process_partition_filter(source, destination, source_table, destination_table, partition_columns, partition_values, is_source_partition, is_dest_partition)
