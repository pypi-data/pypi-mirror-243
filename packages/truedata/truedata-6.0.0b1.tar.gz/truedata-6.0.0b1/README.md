...
```python
from truedata.websocket import TD_live
...
```python
td_obj.live_data[symbol_1].__dict__
...
In version v6 we have seperated live and history modules. so please import according to your configuration.

please note that TD_hist is imported from truedata's history module which is truedata.history. all historical success call return pandas dataframe. 
