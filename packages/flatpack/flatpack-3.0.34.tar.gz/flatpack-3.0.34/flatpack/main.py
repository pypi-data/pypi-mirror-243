...
def fpk_install(directory_name: str, session, verbose: bool = False):
    ...
    # Fetch and validate flatpack TOML
    ...
    # Write to a temporary file with secure permissions
    temp_toml_path = Path('temp_flatpack.toml')
    with open(temp_toml_path, 'w') as f:
        f.write(toml_content)
    fpk_set_secure_file_permissions(temp_toml_path)

    # Generate and execute the installation script
    bash_script_content = parse_toml_to_venv_script(temp_toml_path, '3.10.12', directory_name)
    bash_script_path = Path('flatpack.sh')
    with open(bash_script_path, 'w') as f:
        f.write(bash_script_content)
    fpk_set_secure_file_permissions(bash_script_path)

    # Execute the script with minimal privileges
    command = ["bash", str(bash_script_path)]
    try:
        process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = process.communicate(timeout=30)  # Set a timeout for safety
        if process.returncode != 0:
            print(f"❌ Error in execution: {stderr.decode()}")
        else:
            fpk_cache_last_flatpack(directory_name)
            print(f"🎉 Installation of '{directory_name}' completed successfully!")
    ...
    bash_script_path.unlink(missing_ok=True)
