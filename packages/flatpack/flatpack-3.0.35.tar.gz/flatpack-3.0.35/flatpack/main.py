...
def fpk_install(directory_name: str, session, verbose: bool = False):
    ...
    bash_script_path = 'flatpack.sh'
    command = ["bash", bash_script_path]

    try:
        if verbose:
            # Avoid using shell=True and properly handle subprocess
            process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            stdout, stderr = process.communicate(timeout=30)  # Set a timeout for the subprocess
            if process.returncode != 0:
                print("❌ Error in execution:", stderr.decode())
        else:
            # For non-verbose mode, subprocess handling remains similar
            process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            process.communicate(timeout=30)  # Set a timeout for the subprocess
