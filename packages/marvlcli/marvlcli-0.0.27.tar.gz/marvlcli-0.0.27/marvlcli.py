...
def load_user():
    if os.path.isfile(pkg_resources.resource_filename(__name__, 'package_data/user.json')) is True:
        ...
        click.echo(click.style("Warn:", fg="yellow")+"You are not logged in! Please login with "+click.style("marvl init", fg="magenta") )
...
def check(name):
    ...
    # do something
        click.echo(click.style("Error", fg='red')+": Reason: "+ "No internet connection or "+e.reason)
    else:
...
if __name__=="marvlcli":
    ...
    if check(__name__)==False:
        click.echo(click.style("Warn", fg="yellow") + ":Please update the cli now with the command "+ click.style("pip install --upgrade marvlcli", fg='magenta')+" to avoid this warning!\n")
