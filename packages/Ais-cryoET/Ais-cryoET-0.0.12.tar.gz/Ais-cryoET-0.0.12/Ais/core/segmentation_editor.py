...
class SegmentationEditor:
    if True:
        # scNodes related vars:
        EMBEDDED = False  # is set to True in scNodes+Ais to access some hidden functionality.
    ...
    def __init__(self, window, imgui_context, imgui_impl):
        ...
        # scNodes+Ais related stuff:
        if SegmentationEditor.EMBEDDED:
            import scNodes.core.config as scnodes_cfg
        ...
        if SegmentationEditor.EMBEDDED:
            if not imgui.get_io().want_capture_keyboard and imgui.is_key_pressed(glfw.KEY_TAB):
                if imgui.is_key_down(glfw.KEY_LEFT_SHIFT):
                    scnodes_cfg.active_editor = (scnodes_cfg.active_editor - 1) % len(scnodes_cfg.editors)
                else:
                    scnodes_cfg.active_editor = (scnodes_cfg.active_editor + 1) % len(scnodes_cfg.editors)
