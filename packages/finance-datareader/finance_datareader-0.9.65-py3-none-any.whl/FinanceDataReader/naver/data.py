...
class NaverDailyReader:
    ...
    def read(self):
        # single code
        if ',' not in self.code: 
            df = _naver_data_reader(self.code, self.start, self.end)
            df.attrs = {'source':'NAVER', 'exchange':'KRX', 'code':self.code}
            return df
        ...
        merged.attrs = {'source':'NAVER', 'exchange':'KRX', 'code':self.code }
        return merged
