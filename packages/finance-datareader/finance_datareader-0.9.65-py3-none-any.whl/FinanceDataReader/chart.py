...
# KRX holidays 1975 ~ 2026
cal_url_base = 'https://raw.githubusercontent.com/FinanceData/FinanceDataReader/master/calendars'
holidays_krx, holidays_nyse = None, None
...
def plot(df, tools=None, layout=None):
    ...
    * updates: additional chart configurations
    '''
    global holidays_krx, holidays_nyse
    if holidays_krx is None:
        holidays_krx = pd.read_csv(f'{cal_url_base}/holidays-krx.csv', dtype={'date':str})['date'].values 
    if holidays_nyse is None:
        holidays_nyse = pd.read_csv(f'{cal_url_base}/holidays-nyse.csv', dtype={'date':str})['date'].values
