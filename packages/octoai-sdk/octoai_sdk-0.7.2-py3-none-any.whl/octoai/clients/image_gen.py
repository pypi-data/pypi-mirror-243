...
class ImageGenerator(BaseModel):
    ...
    def _validate_inputs(
        ...
        if cfg_scale is not None and ((0 > cfg_scale) or (50 < cfg_scale)):
            raise OctoAIValidationError(
                f"cfg_scale set to: {cfg_scale}. Allowable range is > 0 and <= 50."
            )
        ...
        if num_images is not None and (0 >= num_images or num_images > 16):
            raise OctoAIValidationError(
                f"num_images set to: {num_images}.  Allowable range is > 0 and <= 16."
            )
