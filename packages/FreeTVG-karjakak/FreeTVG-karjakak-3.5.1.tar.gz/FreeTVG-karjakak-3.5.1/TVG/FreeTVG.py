...
class TreeViewGui:
    ...
    def _save_bible_record(self, record: dict = None, wr: bool = True):
        if wr:
            if record:
                with open(f"{self.glop.parent.joinpath('bible')}.json", "w") as wr:
                    json.dump(record, wr)
        else:
            with open(f"{self.glop.parent.joinpath('bible')}.json") as rd:
    ...
    def bible_reading(self, event=None):
        ...
        if self.lock is False:
            ...
            self.lock = True
            rec = None
            if self.glop.parent.joinpath("bible.json").exists():
