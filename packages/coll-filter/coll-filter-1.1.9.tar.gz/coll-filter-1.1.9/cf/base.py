...
class BaseCollFilter(Generic[U, T]):
    ...
    def __init__(self, data: Iterable[Tuple[U, T, float]], similar_func=default_similar_func):
        ...
        self.user_item_ratings, self.item_user_ratings = {}, {}
        ...
        print_cost_time(f"数据处理, 当前进程: {os.getpid()}, 处理 {cnt} 条记录, "
                        f"user数: {len(self.user_item_ratings)}, item数: {len(self.item_user_ratings)}, 耗时", start_time)
    ...
    def _data_process(self, user_id: U, item_id: T, score: float):
        ...
        item_user_rating = self.item_user_ratings.get(item_id)
        if item_user_rating:
            item_user_rating[user_id] = item_user_rating.get(user_id, 0) + score
        else:
            self.item_user_ratings[item_id] = {user_id: score}
    ...
    def _get_cal_similar_inputs(self, cf_type: CFType):
        if cf_type == CFType.UCF:
            return self.user_item_ratings, self.item_user_ratings.values()
        else:
            return self.item_user_ratings, self.user_item_ratings.values()
