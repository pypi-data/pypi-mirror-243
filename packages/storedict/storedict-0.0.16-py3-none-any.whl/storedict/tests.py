...
@pytest.fixture(params=[DATASTORE_PROXY_URL])
...
def test_iter(store):
    store["foo"] = "bar"
    store["baz"] = "qux"
    assert sorted(list(iter(store))) == sorted(["foo", "baz"])
...
def test_get(store):
    ...
    store["foo"] = None
    assert store.get("foo", "baz") is None
...
def test_validate_value(store):
    ...
    # Refer to JSON-serializables.md in the docs repo.
    store["foo"] = {1: 2}
    assert store["foo"] == {"1": 2}
    store["foo"] = (1, 2, 3)
    assert store["foo"] == [1, 2, 3]
