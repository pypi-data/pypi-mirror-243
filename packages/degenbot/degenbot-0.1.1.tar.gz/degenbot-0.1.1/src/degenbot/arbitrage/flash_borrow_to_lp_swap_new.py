        ...
        if override_future:
            if any(
                [
                    ...
                    override_future_borrow_pool_reserves_token1 is None,
                ]
            ):
            ...
            if TYPE_CHECKING:
                assert reserves_token0 is not None
                assert reserves_token1 is not None
        else:
            if any(
                [
                    override_future_borrow_pool_reserves_token0 is not None,
                    override_future_borrow_pool_reserves_token1 is not None,
                ]
            ):
                raise ValueError(
        ...
        opt = minimize_scalar(
            lambda x: -float(
                ...
                - self.borrow_pool.calculate_tokens_in_from_tokens_out(
                    ...
                    if override_future_borrow_pool_reserves_token0 is not None
                    and override_future_borrow_pool_reserves_token1 is not None
                    else None,
                )
            ),
            method="bounded",
