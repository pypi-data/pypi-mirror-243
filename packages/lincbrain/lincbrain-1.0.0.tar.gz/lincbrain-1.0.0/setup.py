...
try:
    ...
    # Strip off any additional information after the main version number
    # For example, "0+untagged.3407.g7eb75fe.dirty" becomes "0"
    version_parts = version_config.split('+')
    main_version = version_parts[0]
    if main_version.endswith('.dirty'):
        main_version = main_version.replace('.dirty', '')
    version_config = main_version
except Exception:
    print("WARNING: failed to import versioneer, falling back to no version for now")
    version_config = "0.13.0"  # Fallback version
    cmdclass = {}
