...
class LoggingFormatter(logging.Formatter):
    ...
    verbose = " [ {exechain}: {lineno} ] ".format(exechain=skriba.messenger.SystemMessage.blue("%(filename)s: %(module)s.%(funcName)s"), lineno="%(lineno)s")
    ...
    '''
    FORMATS = {
        logging.DEBUG: start_msg + skriba.messenger.SystemMessage.green(
            middle_msg) + execution_msg + "[%(lineno)-4s] %(message)s",
        logging.INFO: start_msg + skriba.messenger.SystemMessage.blue(middle_msg) + execution_msg + "%(message)s",
        logging.WARNING: start_msg + skriba.messenger.SystemMessage.yellow(middle_msg) + execution_msg + "%(message)s",
        logging.ERROR: start_msg + skriba.messenger.SystemMessage.red(
            middle_msg) + execution_msg + "[ %(lineno)-4s] %(message)s",
        logging.CRITICAL: start_msg + skriba.messenger.SystemMessage.red_highlight(
            middle_msg) + execution_msg + "[%(lineno)-4s] %(message)s"
    }
    '''
