...
class LoggingFormatter(logging.Formatter):
    start_msg = "[ {time} ] ".format(time=skriba.messenger.SystemMessage.formatted_string("%(asctime)s", style=["magenta"]))
    middle_msg = "{level}".format(level="%(levelname)8s")
    execution_msg = " {name} [ {filename} ]: {exec_info}: ".format(
        name="%(name)10s",
        filename="%(filename)-20s",
        exec_info=skriba.messenger.SystemMessage.formatted_string("%(callchain)-45s", style="[blue]"))

    FORMATS = {
        logging.DEBUG: start_msg + skriba.messenger.SystemMessage.formatted_string(
            middle_msg, style="[green]") + execution_msg + "[ln: %(lineno)-4s] %(message)s",
        logging.INFO: start_msg + skriba.messenger.SystemMessage.formatted_string(middle_msg, style="[blue]") + execution_msg + "%(message)s",
        logging.WARNING: start_msg + skriba.messenger.SystemMessage.formatted_string(middle_msg, style="[yellow]") + execution_msg + "%(message)s",
        logging.ERROR: start_msg + skriba.messenger.SystemMessage.formatted_string(
            middle_msg, style="[red]") + execution_msg + "[ln: %(lineno)-4s] %(message)s",
        logging.CRITICAL: start_msg + skriba.messenger.SystemMessage.formatted_string(
            middle_msg, style="[bold red]") + execution_msg + "[ln: %(lineno)-4s] %(message)s"
    }
...
def get_logger(logger_name=None):
    ...
    extra = {'callchain': "{module}:{function}".format(module=module, function=function)}
