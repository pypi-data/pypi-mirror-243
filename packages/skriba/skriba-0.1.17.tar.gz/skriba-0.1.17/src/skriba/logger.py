...
class LoggingFormatter(logging.Formatter):
    ...
    message_format["normal"] += "[ {time} ] ".format(time=skriba.messenger.SystemMessage.magenta_dim("%(asctime)s"))
    message_format["normal"] += "%(levelname)8s"
    message_format["normal"] += " %(name)10s"
    message_format["normal"] += "%(message)s"

    message_format["verbose"] += "[ {time} ] ".format(time=skriba.messenger.SystemMessage.magenta_dim("%(asctime)s"))
    message_format["verbose"] += "%(levelname)8s"
    message_format["verbose"] += " {name} [ {filename} ]: {exec_info}: ".format(
        name="%(name)10s",
        filename="%(filename)-20s",
        exec_info=skriba.messenger.SystemMessage.blue_dim("%(callchain)-45s"))
    message_format["verbose"] += "[ln:%(lineno)-4s] %(message)s"

    FORMATS = {
        logging.DEBUG: message_format["verbose"],
        logging.INFO: message_format["normal"],
        logging.WARNING: message_format["normal"],
        logging.ERROR: message_format["verbose"],
        logging.CRITICAL: message_format["verbose"]
    }
