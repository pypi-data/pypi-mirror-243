...
@LastEditTime: 2023-11-23 14:24:11
...
def heart_beat_monitor(work_name, interval_time = 30):
    """
    :description: 作业心跳监控
    ...
    if is_heart_beat_monitor == True:
        try:
            ...
            if abs(TimeHelper.difference_seconds(process_date, now_date)) > interval_time:
                RedisExHelper.init().set(f"heart_beat_monitor:{work_name}", now_date, 30*24*3600)
                work_process_date_dict[work_name] = now_date
    ...
    while True:
        try:
            ...
            for item in match_result:
                work_name = item
                process_date = init.get(work_name)
                if process_date:
                    now_date = TimeHelper.get_now_format_time()
