...
@LastEditTime: 2023-11-28 09:39:44
...
def heart_beat_monitor(work_name, interval_time = 30, data={}, redis_config_dict=None):
    """
    :description: 作业心跳监控
    ...
    if is_heart_beat_monitor == True:
        try:
            ...
            if abs(TimeHelper.difference_seconds(process_date, now_date)) > interval_time:
                RedisExHelper.init(config_dict=redis_config_dict).set(f"heart_beat_monitor:{work_name}", JsonHelper.json_dumps({"process_date":now_date,"data":data}), 30*24*3600)
                work_process_date_dict[work_name] = now_date
        except Exception as ex:
            logger_error.error(f"{work_name}-作业心跳监控异常,ex:{traceback.format_exc()}")

def heart_beat_check(interval_time = 60,  redis_config_dict=None):
    """
    :description: 作业心跳检测
    ...
    while True:
        try:
            time.sleep(60)
            init = RedisExHelper.init(config_dict=redis_config_dict)
