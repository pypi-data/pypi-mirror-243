...
def makedirec(path):
    if not os.path.exists(path):#判断如果文件路径不存在，就新建文件夹名字
        os.makedirs(path)#多层级文件路径
    else:#如果已存在就不用重复建了
        return
...
def STTR(word_list,n):
    '''
    Parameters
    ----------
    st : TYPE
        DESCRIPTION word_list=['more', 'than', 'ever', 'we', 'must', 'do', 'these', 'things', 'together', 'as', 'one', 'nation', 'and', 'one', 'people', 'this', 'generation', 'of', 'americans', 'has', 'been']
    n : TYPE
        DESCRIPTION 计算每个文本每n个词 n=100

    Returns
    -------
    None 0.7095238095238096.

    Standardised TTR: 
        One of four measures developed by Mick Perkins and used for investigating repetitiveness in language disorders.
    ...
    # n=100 #每100词
    if len(l)<n:
        STTR=0 # Texts with less than 1,000 words (or whatever n is set to) will get a standardised type/token ratio of 0.
    else:
        rs=[l[i:i+n] for i in range(0, len(l), n)]
        ttrs=[]
        valid_n_list=0
        for cm in rs:
            if len(cm)>990:
                TTR_grade=len(set(cm))/len(cm)    #[0,1]
                valid_n_list+=1
            else:
                TTR_grade=0.0
            ttrs.append(TTR_grade)
        STTR=sum(ttrs)/valid_n_list
    return STTR