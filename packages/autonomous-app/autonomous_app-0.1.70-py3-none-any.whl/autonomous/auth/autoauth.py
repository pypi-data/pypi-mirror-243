...
class AutoAuth:
    ...
    def current_user(self):
        ...
        """
        if not self.__class__.user:
            self.__class__.user = AutoAuth.user_class(**session.get("user"))
    ...
    def auth_required(func):
        ...
        def decorated_view(*args, **kwargs):
            if current_app:  # with current_app.app_context():
                # log(AutoAuth.user, session.get("user"))
                if session.get("user") and session["user"]["state"] == "authenticated":
                    AutoAuth.user = AutoAuth.user_class(**session.get("user"))
                    AutoAuth.user.last_login = datetime.now()
                    AutoAuth.user.save()
                    session["user"] = AutoAuth.user.serialize()
