...
def to_small_data(
    ...
    return Transform(
        sp.Transform(
            name='ToSmallData',
            spec=sp.Transform.Spec(
                to_small_data=sp.Transform.ToSmallData(size=size)
            ),
            inversible=False,
            schema_preserving=True,
        )
...
def transform_to_small_data(
    dataspec: st.DataSpec, code_name: str
) -> st.DataSpec:
    """
    ...
    if manager.is_big_data(dataspec) and not handle_big_data_from_name(
        code_name
    ):
        ...
        small_dataspec = t.cast(
            st.DataSpec, to_small_data(size=big_data_threshold)(dataspec)
        )
    else:
