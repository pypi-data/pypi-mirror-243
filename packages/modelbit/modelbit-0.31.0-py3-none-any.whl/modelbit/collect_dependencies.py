...
def collectedSpecialObj(obj: Any, name: str, collection: NamespaceCollection) -> bool:
  ...
  if _inTypeStr("_FastText", obj):
    ...
    return True
  if _isKerasModel(obj):
    _collectKeras(obj, name, collection)
    return True
  return False


def _isKerasModel(obj: Any):
...
def _collectKeras(obj: Any, name: str, collection: NamespaceCollection):
  import os
  import tempfile
  saveFormat = "h5"  # maybe "keras" will work soon. That might also remove the need for this custom code
  tmpFilePath = os.path.join(tempfile.gettempdir(), f"tmp.{saveFormat}")
  obj.save(tmpFilePath, overwrite=True, save_format=saveFormat)
  with open(tmpFilePath, "rb") as f:
    collection.extraDataFiles[f"data/{name}.{saveFormat}"] = (obj, f.read())
  os.unlink(tmpFilePath)
