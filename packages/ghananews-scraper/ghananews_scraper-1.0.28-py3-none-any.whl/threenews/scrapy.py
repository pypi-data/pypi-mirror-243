...
class ThreeNews:
    def __init__(self, url: str, limit_pages: int = None):
        ...
        # self.url = f"https://3news.com/news/{self.author}/"
        response = requests.get(self.url, headers=HEADERS)
        soup = BeautifulSoup(response.content, "html.parser")

        # Find the element with the "pages" class
        pages_element = soup.find('span', class_='pages')
        print(pages_element)

        # # Extract the text content and split it to get the total number of pages
        # total_pages_text = pages_element.get_text(strip=True)
        # total_pages = int(total_pages_text.split()[-1])
        # print(total_pages)

        self.last_page = (
            soup.find("a", class_="last")
        )

        # Extract the title attribute and convert it to an integer
        if self.last_page and 'title' in self.last_page.attrs:
            self.last_page = self.last_page['title']
            try:
                self.last_page = int(self.last_page)
            except ValueError:
                logger.error(f'Error: Unable to convert "{self.last_page}" to an integer.')
        else:
            logger.error('Error: Unable to find the "a" element with class "last" or missing title attribute.')
...
if __name__ == "__main__":
    three = ThreeNews(url="https://3news.com/category/news/", limit_pages=40)
    three.download()
