...
def save_diverse_examples(experiment, experiment_type, ks, count_of_samples_per_cluster,  topic_counts = None, seed =None, logger=None):
    if topic_counts:
    ...
    all_diverse_samples = []
    if topic_counts:
        log_message(logger, f"performing clustering on a dataset with {topic_counts}")
        assert (len(ks)==1)
        assert(seed != None)
        for topic_count in topic_counts:
            log_message(logger, f"performing clustering on a dataset with {topic_count}", logging.INFO)
            df_diverse = find_diverse_examples(experiment, experiment_type, ks[0],count_of_samples_per_cluster, topic_count, logger=logger)
