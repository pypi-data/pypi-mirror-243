...
def main():
    ...
    if args['action'] == Actions.DELETE.value:
        ...
        if args['place_to_mask'] == Actions.PLACE_TO_MASK.value:
            confirm = input(
                "All columns of the {} table will be masked, do you confirm? (Y/N): ".format(args['table'])).lower()
        else:
            confirm = input(
                "The following columns will be masked: {}. There are {} rows in the table. Do you confirm? (Y/N): ".format(
                    ", ".join(args['columns']), utils.get_table_rows_count(args['table']))
            ).lower()
