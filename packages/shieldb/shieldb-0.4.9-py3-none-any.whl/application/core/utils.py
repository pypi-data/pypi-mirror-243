...
class Utils:
    ...
    def read_user_args(self):
        ...
        mask_types = {
            ...
            'regex': 'regex (Regex works for phone number, credit card number, email, password and masking is done '
                     'accordingly.)',
            'reverse': 'reverse (shieldb --> bdleihs)',
            'random_char': 'random mask char (shieldb --> sh*e*d*)',
            'random_character': 'middle random character(shieldb --> shabcdb)',
            'nltk': 'random word (shieldb --> apple)',
            'start': 'start (shieldb --> ***eldb)',
            'end': 'end (shieldb --> shie***)',
            'middle': 'middle mask char(shieldb --> sh***db)',
        }
        ...
        if action == Actions.DELETE.value:
            ...
            while True:
                columns = inquirer.text(
                    message="Enter the columns to be masked, separated by spaces:").execute()

                if not columns.strip():
                    print("You must enter at least one column name.")
                    continue
