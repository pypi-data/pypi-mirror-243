...
class sentence_tranformer_checker():
    ...
    def _get_match_statistics(self, df):
        statistics = {}
        # Calculate statistics
        statistics["inputs_length"] = df.shape[0]
        statistics["high_confidence"] = df[df["score_1"] >= 0.9].shape[0]
        statistics["low_confidence"] = df[df["score_1"] < 0.5].shape[0]
        statistics["average_confidence"] = df["score_1"].mean()
        statistics["matching_score_more_Than_91"] = df[df["score_1"] > 0.91].shape[0]
        statistics["matching_score_from_76_to_90"] = df[df["score_1"].between(0.76, 0.90)].shape[0]
        statistics["matching_score_from_51_to_75"] = df[df["score_1"].between(0.51, 0.75)].shape[0]
        statistics["matching_score_from_26_to_50"] = df[df["score_1"].between(0.26, 0.50)].shape[0]
        statistics["matching_score_from_0_to_25"] = df[df["score_1"].between(0, 0.25)].shape[0]
