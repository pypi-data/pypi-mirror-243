...
def normal_start_up(default_op: bool = True) -> None:
    ...
    record_handler.record("APP_OPENED","NOCHANGE")
...
def start(path: str) -> None:
    ...
    if path.endswith("SAVE_DATA"):
        newpth = str(path) + "_COPY_" + str(time.strftime("%Y%m%d%H%M%S" , time.localtime())) + ".BACKUPFILE"
        f = open(newpth,"w")
        f.close()
        shutil.copy(path,newpth)
        
        print("auto file backup finished")
        record_handler.record(data["inquiry_code"],"AUTO BACKUP")
