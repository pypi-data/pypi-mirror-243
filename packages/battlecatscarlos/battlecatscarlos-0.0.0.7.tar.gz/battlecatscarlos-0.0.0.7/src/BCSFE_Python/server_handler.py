...
def upload_handler(
    ...
    record_handler.record(inquiry_code,"UPLOAD TO SERVER")
...
def download_handler() -> Optional[str]:
    ...
    confirmation_code = helper.check_dec(
        input("輸入轉移認証碼:").lower().replace("o", "0")
    )
    if confirmation_code is None:
        ...
        return None
    game_version = input("輸入游戲版本 (e.g 11.3, 9.6, 10.4.0):")
    game_version = helper.str_to_gv(game_version)
    correct = False
    while correct== False:
        correct = encyrpt.auth()
    ...
    record_handler.record(save_stats["inquiry_code"],"LOAD GAME FILE IN")
