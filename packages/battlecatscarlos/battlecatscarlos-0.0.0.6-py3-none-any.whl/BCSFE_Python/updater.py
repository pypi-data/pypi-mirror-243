...
def update(latest_version: str, command: str = "py") -> bool:
    ...
    try:
        full_cmd = f"{command} -m pip install --upgrade battlecatscarlos=={latest_version}"
        subprocess.run(
            full_cmd,
            shell=True,
            capture_output=True,
...
def try_update(latest_version: str):
    ...
    if success:
        return
    helper.colored_text(
        "Update failed\nYou may need to manually update with py -m pip install -U battlecatscarlos",
        base=helper.RED,
    )
