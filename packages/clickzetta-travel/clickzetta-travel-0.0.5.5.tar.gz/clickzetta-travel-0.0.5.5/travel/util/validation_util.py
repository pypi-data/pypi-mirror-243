...
def count_partition_table_validation(source: Source, destination: Destination, source_table: str,
                                     destination_table: str, is_source_partition: bool,
                                     is_dest_partition: bool, partition_columns: list, partition_values: list):
    try:
        result_dict = validation_partition_table_util.count_validation(source_table, destination_table,
                                                             source,
                                                             destination, is_source_partition, is_dest_partition, partition_columns, partition_values)
        source_dict = {}
        destination_dict = {}
        source_keys = []
        destination_keys = []
...
def pk_id_partition_table_validation(source: Source, destination: Destination,
                                     ...
                                     is_source_partition: bool, is_dest_partition: bool,
                                     partition_columns: list, partition_values: list):
    try:
        diff_result = validation_partition_table_util.pk_id_column_validation(source_table,
                                                                              destination_table, source, destination,
                                                                              pk_id, is_source_partition, is_dest_partition,
                                                                              partition_columns, partition_values)
        return diff_result
    except Exception as e:
        raise e
