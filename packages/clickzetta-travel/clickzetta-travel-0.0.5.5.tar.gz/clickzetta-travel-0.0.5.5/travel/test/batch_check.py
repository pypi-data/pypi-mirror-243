...
def check_validation(source, destination, source_table, destination_table, out_path, uid, validation_type,
                     check_schema=0, is_source_partition=False, is_dest_partition=False,
                     partition_columns=None, partition_values=None, pk_column=None):
    ...
    if int(validation_type) == 0:
        ...
        source_df_result, destination_df_result = validation_util.count_partition_table_validation(
            source, destination, source_table, destination_table,is_source_partition, is_dest_partition,partition_columns, partition_values)
        ...
        result = validation_util.pk_id_partition_table_validation(source, destination,
                                                                  ...
                                                                  is_source_partition, is_dest_partition, partition_columns, partition_values)
        only_in_source_table = result['only_in_source_list']
        only_in_destination_table = result['only_in_dest_list']
        if len(only_in_source_table) > 0 or len(only_in_destination_table) > 0:
            with open(f'{out_path}/{uid}_pk_check_result.txt', 'a') as f:
...
def validate(source, destination, source_tables, destination_tables, validation_type, out_path, executor, check_schema=0, is_source_partition=False, is_dest_partition=False,
             partition_columns=None, partition_values=None, pk_column=None):
    ...
    try:
        for source_table, destination_table in zip(source_tables, destination_tables):
            executor.submit(check_validation, source, destination, source_table,
                            ...
                            check_schema, is_source_partition, is_dest_partition,
                            partition_columns, partition_values, pk_column)
    except Exception as e:
        raise e
...
if __name__ == '__main__':
    ...
    check_schema = 0
    is_source_partition = False
    is_dest_partition = False
    partition_columns = None
    partition_values = None
    pk_column = None
    if int(validation_type) == 4:
        assert len(sys.argv) == 12
        is_source_partition = True if int(sys.argv[8]) == 1 else False
        is_dest_partition = True if int(sys.argv[9]) == 1 else False
        partition_columns = sys.argv[10].strip().split(',')
        partition_values = sys.argv[11].strip().split(',')
    ...
    try:
        ...
        validate(source, destination, source_tables, destination_tables, validation_type,
                 out_path, executor, check_schema, is_source_partition, is_dest_partition,
                 partition_columns, partition_values, pk_column)
