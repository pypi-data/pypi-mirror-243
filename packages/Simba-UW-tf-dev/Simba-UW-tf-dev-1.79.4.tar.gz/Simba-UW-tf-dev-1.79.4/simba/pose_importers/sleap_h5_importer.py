...
from simba.utils.errors import BodypartColumnNotFoundError, InvalidFileTypeError
    ...
    def run(self):
        for file_cnt, (video_name, video_data) in enumerate(self.data_and_videos_lk.items()):
            ...
            with h5py.File(video_data['DATA'], "r") as f:
                ...
                if missing_keys:
                    stdout_warning(msg=f'{video_data["DATA"]} is not a valid SLEAP H5 file. Missing keys {missing_keys} Skipping {video_name}...')
                    continue
                tracks = f["tracks"][:].T
