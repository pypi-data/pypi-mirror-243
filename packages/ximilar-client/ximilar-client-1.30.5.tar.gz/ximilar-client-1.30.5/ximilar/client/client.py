...
class RestClient(object):
    ...
    def custom_endpoint_processing(self, records, endpoint, request_params=None):
        """
    ...
    def parallel_records_processing(self, records, method, max_workers=3, batch_size=1, output=False, error_file="", timeout=60):
        """
        ...
        with tqdm(total=len(records), disable=not output) as pbar:
            for brecords in self.batch(records, n=batch_size*max_workers*2):
                futures = []
                for records_to_proc in self.batch(brecords, n=batch_size):
                    futures.append({
                        "future": executor.submit(catch_exception_method, method, records_to_proc),
                        "records": records_to_proc,
                        "size": len(records_to_proc)
                    })

                for future in futures:
                    result = future["future"].result(timeout=timeout)
                    self.update_status(status, result, future["records"], errors)
                    if result is not None:
                        results.append(result)
                    pbar.update(future["size"])

        if error_file and len(errors) > 0:
            with open(error_file, "w") as f:
                json.dump(errors, f, indent=4)

        if output:
            print("Result of parallel: ", status)

        return results

    def update_status(self, status, result, records, errors):
        if result is None:
            return
        ...
        if "skipped_records" in result:
            ...
            for skipped in result["skipped_records"]:
                if skipped["_reason"]["status"]["code"] != 211:
