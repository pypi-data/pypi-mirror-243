...
class CustomSimilarityClient(RecognitionClient):
    ...
    def get_group(self, group_id):
        ...
        return SimilarityGroup(self.token, self.endpoint, group_json["type"]["workspace"], group_json), RESULT_OK
    ...
    def get_groups_url(
        self, page_url=None, search=None, test=None, image=None, group=None, empty=None, type=None, object=None
    ):
        url = (
    ...
    def get_groups(
        ...
        url = self.get_groups_url(page_url, search, test, image, group, empty, type, object)
    ...
    def get_all_groups(self, page_url=None, search=None, test=None, empty=None, type=None):
        url = self.get_groups_url(page_url, search, test, empty=empty, type=type)
    ...
    def get_all_groups_by_name(self, name, sim_type=None, page_url=None, test=None):
        return self.get_all_groups(page_url, "search=" + name, test, type=sim_type)
    ...
    def get_all_groups_by_type(self, sim_type, page_url=None, test=None):
        return self.get_all_groups(page_url, sim_type, test, type=sim_type)

    def get_groups_by_type(self, sim_type, page_url=None, test=None):
        return self.get_groups(page_url, test=test, type=sim_type)
...
class SimilarityGroup(CustomSimilarityClient):
    def __init__(self, token, endpoint, workspace, group_json):
        ...
        self.description = group_json["description"] if "description" in group_json else None
    ...
    def download(self, destination):
        ...
        for image in self.images:
            image = Image(self.token, self.endpoint, image)
            image.download(os.path.join(destination, self.name))

        for object_1 in self.objects:
            image = object_1.image
            image = Image(self.token, self.endpoint, image)
            image.download(os.path.join(destination, self.name))
