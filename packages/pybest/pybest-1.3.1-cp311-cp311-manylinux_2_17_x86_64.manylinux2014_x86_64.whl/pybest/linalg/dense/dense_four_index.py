...
from pybest.linalg.dense.dense_orbital import DenseOrbital
from pybest.linalg.dense.dense_three_index import DenseThreeIndex
from pybest.linalg.dense.dense_two_index import DenseTwoIndex
from pybest.linalg.gpu_contract import PYBEST_CUPY_AVAIL, cupy_helper
from pybest.linalg.pytransform import apply_four_index_transform_cpp
from pybest.log import log
from pybest.utils import check_options, check_type

try:
    from opt_einsum import contract as oe_contract
except ImportError:
    oe_contract = np.einsum
...
class DenseFourIndex(FourIndex):
    ...
    def assign_triu(self, other, begin4=0, end4=None, shape=None, k=0):
        ...
        **Arguments:**
        ...
        """
        check_type("other", other, DenseOneIndex, np.ndarray)
    ...
    def iadd(
        ...
        **Optional arguments:**
        ...
        """
        check_type("other", other, DenseFourIndex, DenseTwoIndex)
    ...
    def iadd_transpose(self, transpose, other=None, factor=1.0):
        ...
        """
        check_type("factor", factor, float, int)
        check_type("other", other, type(None), DenseFourIndex)
        if other is None:
            self.array[:] = (
    ...
    def iadd_expand_one_to_four(self, axis, one, factor=1.0):
        ...
        **Arguments:**
        ...
        one
             An instance of DenseOneIndex.

        **Optional arguments:**

        factor
             See :py:meth:`DenseLinalgFactory.tco`
        ...
        """
        check_options(
            "axis", axis, "0", "1", "2", "3", "b->abcb", "a->abac", "b->abbc"
        )
        if axis == "0":
            for i in range(self.nbasis):
                self.array[i, :, :, :] += one.array[i] * factor
        if axis == "1":
            for i in range(self.nbasis1):
                self.array[:, i, :, :] += one.array[i] * factor
        if axis == "2":
            for i in range(self.nbasis2):
                self.array[:, :, i, :] += one.array[i] * factor
        if axis == "3":
            for i in range(self.nbasis3):
                self.array[:, :, :, i] += one.array[i] * factor
        elif axis == "a->abac":
            for i in range(self.nbasis):
                self.array[i, :, i, :] += one.array[i] * factor
        elif axis == "b->abcb":
            for i in range(self.nbasis1):
                self.array[:, i, :, i] += one.array[i] * factor
        elif axis == "b->abbc":
    ...
    def iadd_expand_two_to_four(
        ...
        **Arguments:**
        ...
        axis
             Any of ``bc->abac`` (abac += bc), ``ac->abcb`` (abcb += ac),
        ...
        **Optional arguments:**
        ...
        end0, end1 = two.fix_ends(end0, end1)
        if axis == "ac->abcb":
            for i in range(self.nbasis1):
                self.array[:, i, :, i] += (
                    two.array[begin0:end0, begin1:end1] * factor
                )
        elif axis == "0-3":
            for i in range(self.nbasis1):
                self.array[:, i, i, :] += (
            ...
            for i in range(self.nbasis):
                for j in range(self.nbasis1):
                    self.array[i, j, :, :] += (
                        two.array[begin0:end0, begin1:end1] * factor
                    )
        elif axis == "ad->abcd":
            for i in range(self.nbasis1):
                for j in range(self.nbasis2):
                    self.array[:, i, j, :] += (
                        two.array[begin0:end0, begin1:end1] * factor
                    )
        elif axis == "ac->abcd":
            ...
            for i in range(self.nbasis1):
                for j in range(self.nbasis3):
                    self.array[:, i, :, j] += (
                        two.array[begin0:end0, begin1:end1].T * factor
                    )
        elif axis == "bd->abcd":
            for i in range(self.nbasis):
                for j in range(self.nbasis2):
            ...
            for i in range(self.nbasis):
                for a in range(self.nbasis1):
                    self.array[i, a, i, a] += two.array[i, a] * factor
        else:
            NIndexObject.contract(
                two1,
                axis,
                two,
                out=self,
    ...
    def iadd_expand_three_to_four(self, axis, three, factor=1.0):
        ...
        **Arguments:**
        ...
        axis
             Any of ``abc->abac`` (abac += abc), ``1-2-1-2`` (abca += abc),
             ``abc->abcb`` (abcb += abc), ``0-3-0-2`` (abbc += abc),
        ...
        **Optional arguments:**
        ...
        if axis == "abc->abac":
            assert self.nbasis == self.nbasis2
            for i in range(self.nbasis):
                self.array[i, :, i, :] += three.array[i, :, :] * factor
        elif axis == "1-2-1-2":
            assert self.nbasis == self.nbasis3
            for i in range(self.nbasis):
                self.array[i, :, :, i] += three.array[i, :, :] * factor
        elif axis == "abc->abcb":
            assert self.nbasis1 == self.nbasis3
            for i in range(self.nbasis1):
                self.array[:, i, :, i] += three.array[:, i, :] * factor
        elif axis == "abc->abbc":  # neu
            assert self.nbasis1 == self.nbasis2
            for i in range(self.nbasis1):
                self.array[:, i, i, :] += three.array[:, i, :] * factor
        elif axis == "0-3-0-2":
            assert self.nbasis1 == self.nbasis2
            ...
            assert self.nbasis == self.nbasis1
            for i in range(self.nbasis):
                self.array[i, i, :, :] += three.array[i, :, :] * factor
        elif axis == "0":
            assert self.shape[1:] == three.shape
            for i in range(self.nbasis):
                self.array[i, :, :, :] += three.array[:, :, :] * factor
        elif axis == "1":
            assert tuple(np.take(self.shape, [0, 2, 3])) == three.shape
            self.array[:] -= np.expand_dims(three.array, int(axis)) * factor
        #           for i in range(self.nbasis1):
        #               self.array[:,i,:,:] += three.array[:,:,:]*factor
        elif axis == "2":
            assert tuple(np.take(self.shape, [0, 1, 3])) == three.shape
            for i in range(self.nbasis2):
                self.array[:, :, i, :] += three.array[:, :, :] * factor
        elif axis == "3":
            assert self.shape[:3] == three.shape
            for i in range(self.nbasis3):
                self.array[:, :, :, i] += three.array[:, :, :] * factor

    def iadd_expand_four_to_three(
        self,
        axis,
        three,
        factor=1.0,
        ...
        """Expand diagonal parts of four-index object along one axes and add it
        to three-index object.
        ...
        **Optional arguments:**
        ...
        begin0, end0, begin1, end1
             ...
             When not given, the full range is used.
        """
        check_options(axis, axis, "bcbc->abc", "acac->abc", "abab->abc")
        end0, end1, end2, end3 = self.fix_ends(end0, end1, end2, end3)
        if axis == "bcbc->abc":
    ...
    def assign_four_index_transform(
        ...
        **Keyword arguments:**
        ...
        if method == "einsum":
            ...
            # only works for restricted orbitals so far
            if exp0 is not exp1 and exp0 is not exp2 and exp0 is not exp3:
                raise NotImplementedError
            apply_four_index_transform_cpp(
                ao_integrals.array, exp0.coeffs, self.array
            )
        else:
            raise UnknownOption(
