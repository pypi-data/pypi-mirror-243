...
from pybest.utils import check_options, check_type, unmask, unmask_orb
...
def print_ao_mo_coeffs(gobasis, orb, begin=1, end=None):
    ...
    **Optional arguments:**
    ...
    def convert_to_fullao(gobasis):
        atom = np.array([], dtype=int)
        shell = np.array([])
        for a, s in zip(gobasis.shell2atom, gobasis.shell_types):
            ls = int(abs(s) * 2.0 + 1)
            for s_ in range(ls):
    ...
    atoms, aos = convert_to_fullao(gobasis)
