...
from pybest.linalg.gpu_contract import PYBEST_CUPY_AVAIL
from pybest.occ_model import AufbauOccModel
from pybest.orbital_utils import (
    print_ao_mo_coeffs,
    split_core_active,
    transform_integrals,
)
...
def prepare_hf(basis_str, mol, linalg):
    ...
    # Input structure
    obasis = get_gobasis(basis_str, mol, print_basis=False)
    lf = linalg(obasis.nbasis)
    ...
    if isinstance(lf, CholeskyLinalgFactory):
        er = compute_cholesky_eri(obasis, threshold=1e-8)
    else:
        er = compute_eri(obasis)
    core = compute_nuclear_repulsion(obasis)
...
def prepare_mol(
    basis, filen="test/h2o_ccdz.xyz", orbfilen="test/h2o_ccdz_hf.txt"
):
    fn_xyz = context.get_fn(filen)
    obasis = get_gobasis(basis, fn_xyz, print_basis=False)
...
if PYBEST_CUPY_AVAIL:
    ...
    test_tco = [
        ...
        pytest.param("opt_einsum", marks=pytest.mark.slow),
        pytest.param("einsum_naive", marks=pytest.mark.slow),
        pytest.param("cpp", marks=pytest.mark.slow),
    ]
...
def test_print_ao_mo_coeff_full():
    obasis, orb_a = prepare_mol("cc-pvdz")

    # Dry run
    print_ao_mo_coeffs(obasis, orb_a)
...
def test_print_ao_mo_coeff_view():
    ...
    # Dry runs
    print_ao_mo_coeffs(obasis, orb_a, 1, 20)
    print_ao_mo_coeffs(obasis, orb_a, 1, 1)
    print_ao_mo_coeffs(obasis, orb_a, 11, 11)
    # first orbital index exceeds
    with pytest.raises(ValueError):
        print_ao_mo_coeffs(obasis, orb_a, 26)
    # last orbital index exceeds
    with pytest.raises(ValueError):
        print_ao_mo_coeffs(obasis, orb_a, end=26)
    # first orbital index larger than last
    with pytest.raises(ValueError):
        print_ao_mo_coeffs(obasis, orb_a, 0)
