...
import pybest.gbasis.gobasis as pybest_basis
            ...
            if filename[-3:] == ".py":
                ...
                with open(path, "r", encoding="utf8") as f:
                    for line in f.readlines():
                        if "select=" in line:
                            line0, line1 = line.split("select=", 1)
                        else:
                            line0 = line
                        if (".contract" in line0) or (".slice" in line0):
                            try:
                                str0, str1, str2 = line0.split("'")
...
def test_contract_begin_end():
    ...
    factor = np.random.rand()
    operands, x = get_operands_dense(subscripts)
...
@pytest.mark.parametrize("min,max,save_memory", parameter_td_cholesky_special)
def test_contract_cholesky_td_special_cases(subscripts, min, max, save_memory):
    """Test if tensordot helper works properly."""
...
def contract_cholesky_blas():
    """Test if C++ operations work properly."""
    ...
    for subscripts in cases:
        ...
        if out is not None:
            ...
            de_result = de_result._array
        error = (
            "NIndexObject.contract(select='blas') returns different "
            "results for Dense and Cholesky representations. "
            "Error occured for subscripts " + subscripts
        )
        assert np.allclose(ch_result, de_result), error
...
if pybest_basis.PYBEST_CHOLESKY_ENABLED:
