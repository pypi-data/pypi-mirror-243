...
def test_three_index_iadd_expand_one_to_three():
    lf = DenseLinalgFactory(5)
    ...
    inp.iadd_expand_one_to_three("a->abc", one, factor=factor1)
    for i in range(5):
        for j in range(5):
            out._array[:, i, j] += one._array[:] * factor1
    assert np.allclose(out._array, inp._array)

    # Blind test on all other cases
    others = ["b->abc", "c->abc"]
    for subscripts in others:
        inp.iadd_expand_one_to_three(subscripts, one, factor=1)
    # with ranges
    one = lf.create_one_index(9)
    one.randomize()
    out = inp.copy()
    inp.iadd_expand_one_to_three(
        "a->abc", one, begin0=1, end0=6, begin3=1, end3=6, factor=factor2
    )

    for i, j in itertools.product(range(5), range(5)):
        out._array[:, i, j] += one._array[1:6] * factor2
    assert np.allclose(out._array, inp._array)
    # Blind test on all other cases
    # "b->abc"
    inp.iadd_expand_one_to_three(
...
def test_four_index_iadd_expand_two_to_four():
    ...
    out = inp.copy()
    two.randomize()
    inp.iadd_expand_two_to_four("ac->abcb", two, factor=1.3)
    for i in range(4):
        for j in range(4):
            for k in range(4):
    ...
    two.randomize()
    out = inp.copy()
    inp.iadd_expand_two_to_four(
        "ac->abcb", two, factor=1.3, begin0=3, end0=7, end1=4
    )
    for i in range(4):
        for j in range(4):
            for k in range(4):
    ...
    for subscripts in others:
        inp.iadd_expand_two_to_four(
            subscripts, two, factor=1.7, begin0=3, end0=7, end1=4
        )


def test_four_index_iadd_expand_three_to_four():
    # test in detail for 'abc->abac'
    lf = DenseLinalgFactory(4)
    inp = lf.create_four_index()
    three = lf.create_three_index()
    inp.randomize()
    out = inp.copy()
    three.randomize()
    inp.iadd_expand_three_to_four("abc->abac", three, factor=1.3)
    for i in range(4):
        for j in range(4):
            for k in range(4):
                out._array[i, j, i, k] += three._array[i, j, k] * 1.3
    # Blind test on all other cases
    others = ["0-2-0-1", "1-2-1-2", "abc->abcb", "abc->abbc"]
    for subscripts in others:
