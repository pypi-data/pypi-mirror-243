...
from pybest.linalg.base import FourIndex, parse_four_index_transform_exps
from pybest.linalg.gpu_contract import PYBEST_CUPY_AVAIL, cupy_helper
from pybest.linalg.pytransform import apply_four_index_transform_cpp_chol
from pybest.log import log, timer
from pybest.utils import check_options, check_type, doc_inherit
...
class CholeskyFourIndex(FourIndex):
    ...
    def einsum_index(script):
        ...
        if len(script) != 4 and isinstance(script, str):
            raise ValueError
        char = [char for char in "xijkl" if char not in script][0]
        ...
        if method == "einsum":
            ...
            if ao_integrals.is_decoupled or not (
                ...
                apply_four_index_transform_cpp_chol(
                    ao_integrals.array2, exp1.coeffs, self._array2
                )
            apply_four_index_transform_cpp_chol(
                ao_integrals.array, exp0.coeffs, self._array
            )
        else:
            raise UnknownOption(
                "The method must either be 'einsum' or 'tensordot' or 'blas'."
            )
