...
test_raises = [
    ...
    ("cc-pvddddz", None, None, GOBasisError),
    ("cc-pvdz", [0], [0, 1, 3, 4], ValueError),
    ("cc-pvdz", [0], [2], GOBasisError),
    ("cc-pvdz", [5], None, GOBasisError),
    ("cc-pvdz", [5], [5], ValueError),
]


@pytest.mark.parametrize("basis, dummy, af", test_cases)
def test_gobasis_dry_run(basis, dummy, af):
...
def test_gobasis_raises(basis, dummy, af, error):
    mol_xyz = context.get_fn("test/h2o.xyz")
...
def test_gobasis_consistency():
    fn = context.get_fn("test/lih.xyz")
    ...
    gobasis = Basis(fn, nprims, shell_map, shell_types, alphas, con_coeffs)
    assert gobasis.nbasis == 25
    assert max(gobasis.shell_types) == 3

    shell_types = np.array([1, 1, 0, 2, 2, 0, 1])
    gobasis = Basis(fn, nprims, shell_map, shell_types, alphas, con_coeffs)
    assert gobasis.nbasis == 21
    assert max(gobasis.shell_types) == 2
...
def test_gob_normalization():
    ...
    obasis = Basis(fn, nprim, shell2atom, shell_types, alpha, contraction)
    # l, s_obs, n_alpha
    assert abs(obasis.get_renorm_pure(0, 1, 0) - 0.601575009051) < 1e-5
    assert abs(obasis.get_renorm_pure(0, 3, 0) - 0.117002092145) < 1e-5
    assert abs(obasis.get_renorm_pure(1, 4, 0) - 4.0198361784) < 1e-5
    assert (
        abs(
            obasis.get_renorm_cart(2, 7, 0, np.array([2, 0, 0]))
            - 5.84002374414
        )
        < 1e-8
    )
    assert (
        abs(
            obasis.get_renorm_cart(2, 7, 0, np.array([1, 1, 0]))
            - 10.1152178423
        )
        < 1e-8
    )
    assert (
        abs(
            obasis.get_renorm_cart(3, 9, 0, np.array([3, 0, 0]))
            - 3.12353490477
        )
        < 1e-8
    )
    assert (
        abs(
            obasis.get_renorm_cart(3, 9, 0, np.array([2, 1, 0]))
            - 6.98443637716
        )
        < 1e-8
    )
    assert (
        abs(
            obasis.get_renorm_cart(3, 9, 0, np.array([1, 1, 1]))
            - 12.0973986675
        )
        < 1e-8
    )
    assert (
        abs(
            obasis.get_renorm_pure(0, 0, 0)
            - obasis.get_renorm_cart(0, 0, 0, np.array([0, 0, 0]))
        )
        < 1e-10
    )
    assert (
        abs(
            obasis.get_renorm_pure(0, 1, 0)
            - obasis.get_renorm_cart(0, 1, 0, np.array([0, 0, 0]))
        )
        < 1e-10
    )
    assert (
        abs(
            obasis.get_renorm_pure(0, 2, 0)
            - obasis.get_renorm_cart(0, 2, 0, np.array([0, 0, 0]))
        )
        < 1e-10
    )
    assert (
        abs(
            obasis.get_renorm_pure(1, 4, 0)
            - obasis.get_renorm_cart(1, 4, 0, np.array([1, 0, 0]))
        )
        < 1e-10
    )
    assert (
        abs(
            obasis.get_renorm_pure(1, 5, 0)
            - obasis.get_renorm_cart(1, 5, 0, np.array([0, 1, 0]))
        )
        < 1e-10
    )
    assert (
        abs(
            obasis.get_renorm_pure(1, 6, 0)
            - obasis.get_renorm_cart(1, 6, 0, np.array([0, 0, 1]))
        )
        < 1e-10
    )
...
def test_write_read_basis():
    ...
    # reading
    obs2 = IOData.from_file(
        f"{filemanager.temp_path('checkpoint.h5')}"
    ).gobasis
...
def test_gobasis_dry_run_v2(basis, mol, dummy, af, expected):
...
def test_gobasis_star_basis_reader(basis, molecule, nbasis):
    ...
    obasis = get_gobasis(basis, mol_xyz, print_basis=False)

    assert obasis.nbasis == nbasis, "Wrong number of basis functios"
