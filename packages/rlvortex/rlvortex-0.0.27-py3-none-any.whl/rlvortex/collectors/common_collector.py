...
class TransitionCollector(BaseCollector):
    def __init__(
        ...
        self.observations, _ = self.env_wrapper.reset()
    ...
    def collect(self) -> None:
        assert self.observations is not None, "observations must not be None"
        for _ in range(self.replay_buffer.num_transitions_per_env):
            if self.policy is None:
                actions, policy_info = self.env_wrapper.sample_action(), {}
            else:
                actions, policy_info = self.policy.step(self.observations)
