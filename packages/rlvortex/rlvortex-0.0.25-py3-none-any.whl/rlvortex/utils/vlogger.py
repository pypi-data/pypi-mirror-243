...
class BaseFlowMeter:
    ...
    def eposide_return(self) -> float:
        assert "returns" in self._eposide_data_dict, "returns must be in eposide_data_dict"
        return np.mean(self._eposide_data_dict["returns"])
    ...
    def log(self):
        ...
        self.logger.info(f"{self._epoch_counter} - [Duration]: {duration}")
