...
from wuji.tools.plot_parallel_data import plot_multiple_datasets
...
if __name__ == '__main__':
    ...
    for file in sorted(os.listdir('/Users/cxs/project/OSAPillow/data/SHHS/annotations')):
        if file.endswith('.xml'):
            ...
            edf_fp = os.path.join('/Users/cxs/project/OSAPillow/data/SHHS/edfs', file.split('.')[0][:-5]+'.edf')
            reader = NSRRAnnotationReader(fp)
            edf_reader = NSRREDFReader(edf_fp)
            flow = edf_reader.get_signal(type='flow')
            flow_fs = edf_reader.get_sample_frequency(type='flow')
            spo2 = edf_reader.get_signal(type='spo2')
            spo2_fs = edf_reader.get_sample_frequency(type='spo2')
            stages = reader.get_standard_sleep_stages()
            rsp_events = reader.get_respiratory_events()
            # print('rsp_events')
            # print(rsp_events)
            od_events = reader.get_OD_events(OD_level=4)
            # print('od_events')
            # print(od_events)
            ...
            # print('AH_events')
            # print(AH_events)
            # plot_multiple_datasets(flow, spo2, sampling_rates=[flow_fs, spo2_fs], labels=['flow', 'spo2'])
