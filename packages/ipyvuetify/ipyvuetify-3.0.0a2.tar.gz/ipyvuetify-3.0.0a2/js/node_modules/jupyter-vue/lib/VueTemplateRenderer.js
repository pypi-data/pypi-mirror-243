...
function createComponentObject(model, parentView) {
  ...
  return {
    ...
    data() {
      ...
      return _objectSpread({}, data, {}, dataTemplate, {}, createDataMapping(model));
    },
...
function createMethods(model, parentView) {
  return model.get('events').reduce((result, event) => {
    ...
    result[event] = (value, buffers) => {
      if (buffers) {
        const validBuffers = buffers instanceof Array && buffers[0] instanceof ArrayBuffer;
      ...
      }, model.callbacks(parentView), buffers);
    };
...
function createClassComponents(components, containerModel, parentView) {
  return components.reduce((accumulator, [componentName, componentSpec]) => _objectSpread({}, accumulator, {
    [componentName]: {
...
function createFullVueComponents(components) {
  return components.reduce((accumulator, [componentName, vueFile]) => _objectSpread({}, accumulator, {
...
function aliasRefProps(model) {
  return model.keys().filter(key => key.endsWith('_ref')).map(propRef => [propRef, propRef.substring(0, propRef.length - 4)]).filter(([, prop]) => !model.keys().includes(prop)).reduce((accumulator, [propRef, prop]) => _objectSpread({}, accumulator, {
    [prop]() {
      return this[propRef];
    }
