...
export function createObjectForNestedModel(model, parentView) {
  ...
  return {
    mounted() {
      parentView.create_child_view(model).then(view => {
        currentView = view; // since create view is async, the vue component might be destroyed before the view is created
...
function pickSerializable(object, depth = 0, max_depth = 2) {
  ...
  for (let key in object) {
    ...
    if (value instanceof Node) // specify which properties you want to see from the node
      value = {
        id: value.id
      };else if (value instanceof Window) value = 'Window';else if (value instanceof Object) value = pickSerializable(value, depth + 1, max_depth);
    obj[key] = value;
  }
...
export function vueRender(createElement, model, parentView, slotScopes) {
  ...
  const elem = createElement({
    ...
    render(createElement2) {
      const element = createElement2(tag, createContent(createElement2, model, this, parentView, slotScopes), renderChildren(createElement2, model.get('children'), this, parentView, slotScopes));
      updateCache(this);
      return element;
    }
...
function updateCache(vueModel) {
  Object.keys(vueModel.childCache).filter(key => !vueModel.childIds.includes(key)) // eslint-disable-next-line no-param-reassign
