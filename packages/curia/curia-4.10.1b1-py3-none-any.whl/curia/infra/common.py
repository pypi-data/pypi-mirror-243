...
def _create_or_update_workflow_template(
        ...
        task: TaskDefinition,
        task_id: str,
        resource_naming_config: TaskDeployResourceNamingConfig) -> str:
    """
    ...
    :param internal_task_name: name of the workflow template
    ...
    new_workflow_template = WorkflowTemplate(
        ...
        definition={
            ...
            "data": {
                **{
                    task_input.name: "{{parameters." + task_input.name + "}}" for task_input in task.inputs
                }
            },
            "opts": {},
            "children": []
...
def _create_or_update_task(
    ...
    new_task = Task(
        ...
        inputs={
            ...
            **task.build_task_type_specific_input_arguments(task_specific_context)
        },
        outputs={
            task_output.name: {
    ...
    if len(results) == 0:
        ...
        raise ValueError(f"Found multiple tasks with name {internal_task_name}. This should not happen, please clean up the "
