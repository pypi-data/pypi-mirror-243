...
def mapillary_classes_to_vcd(img_name, mapillary_config, vcd_poly_mode, vcd_root_name):
    """
    This function converts an image of labels (classes) of the Mapillary Vistas dataset into a
    VCD file.
...
def mapillary_instances_to_vcd(img_name, mapillary_config, vcd_poly_mode, vcd_root_name):
    ...
    img_ids_array_bgr = cv.cvtColor(img_ids_array, cv.COLOR_GRAY2BGR)
...
def vcd_mapillary_classes_to_png(vcd, mapillary_config):
    """
    This function reads a VCD file containing Mapillary Vistas labels (classes), as generated
    by function mapillary_classes_to_vcd(), and creates a PNG image, equal to the original
    Mapillary Vistas label PNG.

    :param vcd: VCD object
    :param mapillary_config: configuration file
    :return: PNG image as a numpy array
    """
    if vcd.get_metadata()["labels"] != "class":
        raise RuntimeError("The VCD file must contain classes")
...
def vcd_mapillary_instances_to_png(vcd, mapillary_config):
    if vcd.get_metadata()["labels"] != "class":
        raise RuntimeError("The VCD file must contain labels")
