...
def get_similar_session_values_by_name(name):
    ...
    if s:
        ...
        log.warning('find no similar session values by name: %s' % name)
    return d
...
def create_teachers_by_tags(tags):
    from xyz_auth.helper import find_user_ids_by_tag
    from django.contrib.auth.models import User
    uids = set(find_user_ids_by_tag(tags))
    ts = []
    for u in User.objects.filter(id__in=uids):
        teacher, created = models.Teacher.objects.get_or_create(
            user=u,
            defaults=dict(
