...
class Cancer:
    ...
    def get_miRNA(self, source: str=None, tissue_type: str="both", imputed: bool=False) -> pd.DataFrame:
        ...
        """
        return self.get_dataframe('miRNA', source, tissue_type, imputed=imputed)
    ...
    def join_omics_to_mutations(
        ...
        """Joins specified mutations for a gene(s) to a selected omics dataframe using intersection (inner join) of indices.

        The function returns a DataFrame where each location or mutation cell contains a list. This list contains one or more location or mutation values for that sample for the gene in question. If a sample doesn't have a mutation in that gene, the cell will indicate so.

        Parameters:
        - omics_name (str): Name of the omics dataframe to join the mutation data to.
        - mutations_genes (str or list[str]): The gene(s) for which to retrieve mutation data. Pass a str for one gene, or a list of str for multiple genes.
        - omics_source (str): Name of source for the omics data.
        - mutations_source (str): Name of source for the mutations data.
        - omics_genes (str or list[str], optional): Gene(s) to extract from the omics dataframe. Pass a str for one gene, or a list of str for multiple. Default is None, which selects the entire dataframe.
        - mutations_filter (list, optional): List of mutations to prioritize when filtering multiple mutations, in order of priority. Default is None, including all mutation data. Refer to full function docstring for more details.
        - show_location (bool, optional): Whether to include the Location column from the mutation dataframe. Default is True.
        - how (str, optional): Type of join operation to perform. Accepts 'outer', 'inner', 'left', 'right'. Default is 'outer'.
        - quiet (bool, optional): Whether to suppress warnings when inserting NaNs. Default is False.
        - tissue_type (str, optional): Desired tissue type for the dataframe. Accepts 'tumor', 'normal', 'both'. Default is 'both'.

        Returns:
        - pandas.DataFrame: A DataFrame of mutations for the specified gene(s), joined with the selected portion of the omics dataframe. Each cell in the location or mutation column contains a list with location or mutation values for that sample, or an indicator if the sample didn't have a mutation in that gene.
    ...
    def multi_join(self, join_dict: dict, mutations_filter: list = None, flatten: bool = False,
        ...
        Parameters:
        - join_dict (dict): A dictionary defining the dataframes and columns to join. Keys represent the names of dataframes. Values are lists of column names corresponding to each dataframe.
        - mutations_filter (list, optional): List of mutations to prioritize when filtering out multiple mutations, in order of priority. If None, all mutations are included.
        - flatten (bool, optional): If True, multiindexes in the dataframe will be flattened. Defaults to False.
        - levels_to_drop (list, optional): List of level names to drop from the dataframe. Defaults to an empty list (no levels dropped).
        - how (str, optional): Method of join. Accepts 'outer', 'inner', 'left', 'right'. Default is 'outer'.
        - tissue_type (str, optional): Type of tissue data to retrieve. Accepts 'tumor', 'normal', 'both'. Default is 'both'.

        Returns:
        - pandas.DataFrame: The resulting DataFrame after performing all joins specified in the join_dict.

        Example:
        ```python
        multi_join(
            join_dict={
                "source1 dataframe1": ["column1", "column2"],
                "source2 dataframe2": ["column3", "column4"]
            },
            mutations_filter=['nonsynonymous SNV'],
            flatten=True,
            levels_to_drop=["level1", "level2"],
            how="outer",
            tissue_type="tumor"
        )
        ```
    ...
    def list_data_sources(self, source_filter = "all"):
        ...
        Returns:
        - pandas.DataFrame: A DataFrame where each row represents a data type. The columns are 'Data type' and 'Available sources'.

        Raises:
        - InvalidParameterError: If a source specified in the source_filter is not a valid source for the dataset.
        """
        ...
        source_filter = [source_filter] if isinstance(source_filter, str) else source_filter
        if source_filter == ["all"]:
            source_filter = self._sources.keys()

        # Ensure all sources in source_filter are valid
        for src in source_filter:
            if src not in self._sources.keys():
                raise InvalidParameterError(f"{src} is not a valid source for the {self._cancer_type} dataset. Valid sources are: {list(self._sources.keys())}")
    ...
    def get_dataframe(
        ...
        """
        Checks if a given dataframe from a specific source exists and return a copy if it does.

        Parameters:
        data_type (str): The data type of the desired dataframe.
        source (str, optional): The source of the dataframe. If not specified and there's only one source for the data_type, that source will be used. If not specified and there's more than one source for the data_type, DataSourceNotFoundError will be raised. Defaults to None.
        ...
        Raises:
        DataFrameNotIncludedError: If the data_type is not included in the dataset.
        DataSourceNotFoundError: If the source is not found in the dataset or if no source is specified and more than one source provides the data_type.
    ...
    def _get_omics_cols(self, omics_df_name, source, genes, tissue_type="both"):
        """
        Based on a single gene, or a list or array-like of genes, select multiple columns from an omics dataframe, 
        and return a copy of the selected columns as a new dataframe.

        Parameters:
        omics_df_name (str): Name of the omics dataframe to select column(s) from.
        source (str): Source for data to select column(s) from.
        genes (str or list or array-like of str): Gene(s) to use to select columns from omics_df. str if one gene, 
            list or array-like if multiple. If None is passed, the entire omics dataframe will be selected. 
            Defaults to None.
        tissue_type (str): Acceptable values in ["tumor","normal","both"]. Specifies the tissue type desired in 
            the dataframe. Defaults to "both".

        Raises:
        InvalidParameterError: If the `genes` parameter is not a string, list, array-like, or None. A detailed error 
            message is provided indicating the passed value and the acceptable types.

        Warnings:
        ParameterWarning: If certain genes specified were not found in the dataframe. These genes will be inserted into 
            the returned dataframe, but filled with NaN.

        Returns:
        pandas.DataFrame: A copy of the selected columns from the original dataframe.
        """
