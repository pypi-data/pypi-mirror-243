...
__all__ = ['clipsd2', 'clipsd3', 'merge_dfs', 'calculate_contribution', 'calculate_contributions', 'BoundsSpec', 'ClipSpec',
           ...
           'zscore_cells', 'Basis', 'hvgset', 'embed_hvg']
...
from nchr import U1, NIL, SPACE, DOT
from nlit import PHATE_HVG, X_
from nlit import (GENE_SYMBOL, BATCH)
from nlit import X_PCA_HVG, X_PHATE_HVG, EXT_H5AD, HIGHLY_VARIABLE

from chck import isad, isarr, islist, isany, isnone, iscall, isstr, notnone

from pstr import splitjoin, axeschars
...
from .attr import findgenes, subset_genes, genes2str
...
class BoundsSpec(NamedTuple):
    ...
    def apply(self, data, key = None, *args, **kwargs):
        match type(data):
            case ad.AnnData:   arr = getkey(data, key)
...
class Basis:
    def __init__(self, name: str = PHATE_HVG, prepend_X: bool = True) -> None:
        self.name = name
        self.prex = prepend_X

    def __repr__(self): return f'{type(self).__name__}({self.label})' 
    def __call__(self, adata: AnnData) -> NPArray: return adata.obsm[self.basis]
    @property
    def basis(self) -> str: return (X_ if self.prex else NIL) + self.name
    @property
    def label(self) -> str: return self.basis.removeprefix(X_, NIL)
    
    def axis(self, dim: int = None, modify = str.upper, spaces: bool = True) -> str:        
        axis = splitjoin(self.label, SPACE, U1)
        if notnone(dim): axis = U1.join((self.label, str(dim)))
        if callable(modify): axis = modify(axis)
        if spaces: axis = axis.replace(U1, SPACE)
        return axis
   
    def axes(self, dims: int, oneidx: bool = False, useint: bool = True):         
        adims = range(int(oneidx), dims + int(oneidx))
        if not useint: adims = axeschars(dims)
        return tuple(self.axis(dim) for dim in adims)
    
    def xyz(self, *dims) -> tuple:
    ...
    def getdim(self, adata: AnnData, dim: int) -> NPArray: return self(adata)[:, dim]
    def getdims(self, adata: AnnData, *dims) -> tuple: return tuple(self.dim(adata, dim) for dim in dims)
...
def hvgset(adata): return adata[:, adata.var[HIGHLY_VARIABLE]]
