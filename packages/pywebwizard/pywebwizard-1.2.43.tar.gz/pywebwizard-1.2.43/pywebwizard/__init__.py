    ...
    def _request(self, action: dict):
        ...
        try:
            ...
            if _response.status_code >= 400:
                result = dict(status=_response.status_code, error=_response.content)
            else:
                try:
                    result = dict(status=_response.status_code, response=_response.json())
                except AttributeError:
                    result = dict(status=_response.status_code, response=_response.content)
                except Exception as response_error:
                    result = dict(status=_response.status_code, response=_response.text, error=response_error)
        except Exception as e:
            result = dict(status=500, error=str(e))
        if _function_response_variable:
            for _new_var_i, _new_var in enumerate(_function_response_variable):
                if _new_var.strip() not in self.environment:
                    self.environment[_new_var.strip()] = None
        ...
        if _debug:
            logging.debug(f"[REQUEST] Result: {result}")
        return result
