...
class Object:
    ...
    def set_evaporator_parameters(self, params):
        if params:
            self.EVAP.LP_bar = params.get('LP_bar', 2.930154)
            self.EVAP.surchauff = params.get('surchauff', 2)
            self.EVAP.Inlet.F = params.get('F', 1)
            self.EVAP.Inlet.h = CP.PropsSI('H', 'P', self.EVAP.LP_bar * 1e5, 'T', 40 + 273.15, self.fluid)
...
# # Usage example
# chiller_params = {
#     'evap_params': {'LP_bar': 2.930154, 'surchauff': 2},
#     'comp_params': {'Tcond_degC': 40, 'eta_is': 0.8, 'Tdischarge_target': 80, 'Qcomp': 100000},
#     'cond_params': {'subcooling': 2}
# }

# chiller = Object(fluid="R134a", **chiller_params)
