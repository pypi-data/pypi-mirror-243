                ...
                if expires_age < 0:
                    ...
                    if isinstance(expire_threshold, int) and expire_threshold > abs(expires_age):
                        logging.info(f"'{name}' Expiration Date is within the valid specified threshold of "
                                     ...
                                     f"reported in {expire_threshold - abs(expires_age)} days.")
...
if __name__ == '__main__':
    ...
    az_path = f"/opt/homebrew/bin"
    vault = "kv-thelma-dev"
    expire_threshold = None
    ignore_no_expiration = True
    include_all = False

    kv_report = AzureKeyVaultReport(vault)
    kv_report.az_cmd(path=az_path)
    kv_report.parse_results()
    kv_report.set_report_header()
    kv_report.plaintext_report()
