...
from modeltasks.config import (
    ConfigurationValue,
    SecureConfigurationValue,
    StaticConfigurationValue,
    ConfigurationFile,
    ConfigurationFileChecksum,
    ConfigurationFolder,
    ConfigurationFolderChecksum,
    UserInput,
    SecureUserInput,
    UserSelection
)
...
class Model:
    ...
    def __init__(
        ...
        # Set the task registry (Which loads all tasks) and call event handler
    ...
    def render_graph(
        ...
        if output and output.suffix == '.md':
            ...
            for i, g in enumerate(graphs):
                mermaid += f'# Model {i + 1 if len(graphs) > 1 else ""}\n```mermaid\nflowchart TD\n'
                ...
                start_tasks = [t for t in list(g.nodes) if len(self._registry.get_task(t).inputs) == 0]
                ...
                for task in [self._registry.get_task(t).instance for t in list(g.nodes)]:
                    ...
                    groups.setdefault(group, []).append(dict(
                        name=task.name,
                        description='<font size=2>{}</font>'.format(shorten(
                ...
                for group, group_tasks in groups.items():
                    ...
                    for t in group_tasks:
                        mermaid += f'    {t["name"]}[{"<br>".join([t["name"], t["description"]])}]{":::END" if t["name"] == task.name else ":::START" if t["name"] in start_tasks else ":::TASK"}\n'
                    mermaid += '\n'
                    for t1, t2 in relations:
                        ...
                        if group1 != group2 and group2 == group:
                            mermaid += f'    {t1.name} --> {t2.name}\n'
                    if len(groups) > 1:
                        mermaid += '  end\n'
