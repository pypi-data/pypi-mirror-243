...
class PyWV:
    def __init__(self, q, start_ev, exit_ev, loaded, emit_queue, return_queue, html, debug,
                 width, height, x, y, screen, on_top, maximize, my_custom_name: str = 'lw_chart'):
        ...
        self.windows = []
        self.create_window(width, height, x, y, screen, on_top, maximize, my_custom_name)
    ...
    def __init__(self, width: int = 800, height: int = 600, x: int = None, y: int = None, screen: int = None,
        ...
        if self._i == 0:
            ...
            self._process = mp.Process(target=PyWV, args=(
                ...
                width, height, x, y, screen, on_top, maximize, self.my_custom_name
            ), daemon=True)
            self._process.start()
        else:
            window.handlers = Chart._main_window_handlers
            super().__init__(window, inner_width, inner_height, scale_candles_only, toolbox)
            self._q.put(('create_window', (width, height, x, y, screen, on_top, maximize, self.my_custom_name)))
