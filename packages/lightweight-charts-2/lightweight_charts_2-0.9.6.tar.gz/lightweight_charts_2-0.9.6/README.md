...
Original: https://pypi.org/project/lightweight-charts/

This version allows each window to be named if you want multiple charts to be open at the same time. These windows can then be easily distinguished from each other, useful for organization, or accessing individual windows from an OS-level. Example usage:

```
from lightweight_charts_2 import Chart

charts = {ticker: Chart(my_custom_name=ticker) for ticker in tickers}
```
