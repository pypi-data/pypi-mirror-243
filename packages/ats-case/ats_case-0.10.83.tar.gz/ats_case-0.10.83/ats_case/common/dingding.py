...
def send(payload: Payload, project: str, test_sn: str, start_time, end_time, msg=''):
    try:
        url, secret, sign = _link(test_sn)
        dc = DingtalkChatbot(url, secret)
        dc.send_markdown(title=f'{sign}', text=_data(payload, project, test_sn, start_time, end_time, msg),
                         is_at_all=True)
...
def _data(payload, project, test_sn, start_time, end_time, msg=''):
    sn = test_sn.split(':')[0]
    ...
    return TXT.format(project, sn, start_time, end_time, completed_msg, username, msg)
