...
class Executor(object):
    ...
    def _err_msg_notify(self, msg: str):
        project = '{}[{}]'.format(self._context.case.name, self._context.runtime.step)
        if self._context.mode == WorkMode.FORMAL:
            project = '{} - {}[{}]'.format(self._context.project, self._context.case.name, self._context.runtime.step)

        dingding.send(Payload.ERROR, project, self._context.test_sn, self._context.start_time, datetime.now(), msg=msg)
