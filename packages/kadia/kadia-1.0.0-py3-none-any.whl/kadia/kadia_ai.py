...
class KadiaAI(AioThing):
    ...
    async def _get_missing_chunks(
        ...
        for document in documents:
            is_stored = await asyncio.get_running_loop().run_in_executor(
                None,
                lambda: self.vector_storage.exists_by_field_value(
    ...
    def ensure_config(
        ...
        if not os.path.exists(config_path) or force:
            config = {
                ...
                "model": KadiaModel.default_config(
                    llm_name=llm_name,
    ...
    async def generate_chunks_from_document(
        ...
        return await asyncio.get_running_loop().run_in_executor(
            None, lambda: self.document_chunker.to_chunks(document)
        )
