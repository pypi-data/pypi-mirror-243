...
class Memu:
    def __init__(self):
        '''
        To Use:
        - Call the turn_on() method to start the UI.
        ...
        self.main_font_bold_large = ctk.CTkFont(family="Arial", size=54, weight='bold')
    ...
    def turn_on(self):
        self.ui_thread = threading.Thread(self.app.mainloop())
    ...
    def start(self):
        ...
        if not self.emu_running:
            ...
            self.emulator = emu.Backend(self.rom_path.get())

            # Threading
            self.emu_thread = self.emu_thread = threading.Thread(self.emulator.start())
            self.emu_thread.start()
            self.emu_running = True
            self.emu_thread.join()

            # self.screen_img = self.emulator.get_screen()
            # self.screen = ctk.CTkImage(light_image=self.screen_img,
            #                       dark_image=self.screen_img)
            # self.screen.pack()
