...
class _FuzzyMatch(__BaseScoring):
    def search(self, keyword: str, threshold: float = 0.5, normalize_threshold: bool = True) -> List[ScoringResult]:
...
class TextScoring(__BaseScoring):
    ...
    def _prepare_contents(self, contents: List[Content]) -> None:
        ...
        log.info(f"total '{self.conf.collection}' {rep.count_total_base_data()} base data, finished in {round(time.time() - st, 3)} seconds.")
        ...
        log.info(f"total '{self.conf.collection}' {rep.count_total_processed_data()} processed data, finished in {round(time.time() - st, 3)} seconds.")
        self.db_prepared()

    @property
    def _index_ready(self) -> bool:
        return path.exists(self.conf.cosine_index_location) \
