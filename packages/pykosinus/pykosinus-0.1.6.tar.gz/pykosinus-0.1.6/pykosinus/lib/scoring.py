...
class _CosineSimilarity(__BaseScoring):
    ...
    def _create_index(
        ...
        tmp_cosine_index_location = f"{self.conf.tmp_cosine_index_location}_{uuid.uuid4()}"
    ...
    def _prepare_contents(self, contents: List[Content], many: bool = False) -> None:
        ...
        for contents_batch in self._batch_generator(contents, self.conf.batch_size):
            if many:
                ...
                for content in contents_batch:
                    if data := rep.create_base_data(content.content, content.identifier, content.section):
                        is_update = True
                        updated.append(data)
                    pb.update()
