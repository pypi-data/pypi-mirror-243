...
def simulate_conditioning(stimuli, n_trials=10, e=0.5,
    ...
    for t in range(n_trials):
        g = sum(stimuli_values.copy().values())
        for b, d, (start_trial, end_trial), _ in stimuli:
            if t not in range(start_trial, end_trial):
                continue  
            h = a[b] + d * e * (f - g)  
            stimuli_values[name] = h
        trial_data.loc[t+1] = stimuli_values.values()
    return trial_data
...
def test_1b_oshadow(stim_cs, stim_oshadowed, reward):
    ...
    salience_ok = stim_cs[1] > 0 and stim_cs[1] > stim_oshadowed[1]
...
def test_choose_action(student_function):
    np.random.seed(1)
    Q_table = np.random.normal(size=(16,4))
    student_actions = []
    correct_actions = []
    for state in range(15):
      np.random.seed(state)
      random.seed(state)
      student_actions.append(student_function(Q_table, state, 0.5))
      np.random.seed(state)
      random.seed(state)
      correct_actions.append(choose_action(Q_table, state, 0.5))
