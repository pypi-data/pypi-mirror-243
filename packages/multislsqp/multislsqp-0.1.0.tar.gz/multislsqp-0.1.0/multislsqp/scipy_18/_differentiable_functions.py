...
from multislsqp.scipy_18._numdiff import approx_derivative, group_columns
...
class ScalarFunction(object):
    ...
    def __init__(self, fun, x0, args, grad, hess, finite_diff_rel_step,
        ...
        self.ngev = 0
        self.nhev = 0
        self.f_updated = np.array([False]*self.m)
        self.g_updated = np.array([False]*self.m)
        self.H_updated = False
        self._bl_f = np.array([True]*self.m)
        self._bl_g = np.array([True]*self.m)
        ...
        def update_fun():
            f = fun_wrapped(self.x)
            if not hasattr(self,'f'):
                self.f = f
                return
            
            self.f[self._bl_f2] = f[self._bl_f2]
        ...
        if callable(grad):
            ...
            def update_grad():
                self._update_fun('g')
                self.ngev += 1
                g = approx_derivative(fun_wrapped, self.x, f0=self.f,
                                           **finite_diff_options)
                if not hasattr(self,'g'):
                    self.g = g
                else:
                    self.g[self._bl_f2] = g[self._bl_f2]
        ...
        if isinstance(hess, HessianUpdateStrategy):
            ...
            def update_x_f(x):
                self.x[self._bl_f] = np.atleast_1d(x).astype(float)[self._bl_f]
                self.f_updated[self._bl_f] = False
                self.g_updated[self._bl_f] = False

            def update_x_g(x):
                self.x[self._bl_g] = np.atleast_1d(x).astype(float)[self._bl_g]
                self.f_updated[self._bl_g] = False
                self.g_updated[self._bl_g] = False

        self._update_x_impl_f = update_x_f
        self._update_x_impl_g = update_x_g

    def _update_fun(self,tp='f'):
        if tp=='f':
            bl = self.f_updated[self._bl_f]
            if not bl.all():
                self._bl_f2 = np.array([False]*self.m)
                self._bl_f2[self._bl_f] = bl==False
                self._update_fun_impl()
                self.f_updated[self._bl_f2] = True
        elif tp=='g':
            bl = self.f_updated[self._bl_g]
            if not bl.all():
                self._bl_f2 = np.array([False]*self.m)
                self._bl_f2[self._bl_g] = bl==False
                self._update_fun_impl()
                self.f_updated[self._bl_f2] = True

    def _update_grad(self):
        bl = self.g_updated[self._bl_g]
        if not bl.all():
            self._bl_f2 = np.array([False]*self.m)
            self._bl_f2[self._bl_g] = bl==False
            self._update_grad_impl()
            self.g_updated[self._bl_f2] = True
    ...
    def fun(self, x):
        if not np.array_equal(x[self._bl_f], self.x[self._bl_f]):
            self._update_x_impl_f(x)
        self._update_fun()
        return self.f

    def grad(self, x):
        if not np.array_equal(x[self._bl_g], self.x[self._bl_g]):
            self._update_x_impl_g(x)
        self._update_grad()
        return self.g
