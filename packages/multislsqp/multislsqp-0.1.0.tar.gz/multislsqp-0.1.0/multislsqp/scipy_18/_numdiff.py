...
def approx_derivative(fun, x0, method='3-point', rel_step=None, abs_step=None,
    ...
    # amendment here
    if x0.ndim==2: dim = x0.shape[1]
    else: dim = x0.shape[0]
    if lb.ndim==2: lb_dim=lb.shape[1]
    else:lb_dim=lb.shape[0]
    if ub.ndim==2: ub_dim=ub.shape[1]
    else:ub_dim=ub.shape[0]
    if lb_dim!= dim or ub_dim != dim:
...
def _dense_difference(fun, x0, f0, h, use_one_sided, method):
    ...
    for i in range(n):
        if method == '2-point':
            ...
            dx,df = np.empty(m),np.empty(m)
            _use_one_sided = use_one_sided[:,i]
            if _use_one_sided.any():
                x1[_use_one_sided,i] += h_vecs[_use_one_sided,i]
                x2[_use_one_sided,i] += 2 * h_vecs[_use_one_sided,i]
                f1 = fun(x1)
                f2 = fun(x2)
                dx[_use_one_sided] = (x2[i] - x0[i])[_use_one_sided]
                df[_use_one_sided] = (-3.0 * f0 + 4 * f1 - f2)[_use_one_sided]
            else:
                _use_one_sided_opp = _use_one_sided==False
                x1[_use_one_sided_opp,i] -= h[_use_one_sided_opp,i]
                x2[_use_one_sided_opp,i] += h[_use_one_sided_opp,i]        
                f1 = fun(x1).flatten()
                f2 = fun(x2).flatten()            
                dx[_use_one_sided_opp] = (x2[:,i] - x1[:,i])[_use_one_sided_opp]  # Recompute dx as exactly representable number.
                df[_use_one_sided_opp] = (f2 - f1)[_use_one_sided_opp]
        elif method == 'cs':
