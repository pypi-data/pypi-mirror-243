...
def query_run_embedding(prompt: Union[str, List[str]], config={}):
    ...
    edit_config(config)
    resp = make_web_request(params, "/v1/inference/embedding")
...
def verify_api_key() -> None:
    if lamini.api_key is None and get_configured_key() is None:
        raise llama.error.AuthenticationError(lamini.MISSING_API_KEY_MESSAGE)
