...
class ToyotaOneAuth:
    ...
    def __init__(self, callback=None, initial_tokens=None, otp_url=None, refresh_secs=300):
                ...
                if "callbacks" in data:
                    for cb in data["callbacks"]:
                        if (
                            ...
                            cb["input"][0]["value"] = await self.request_opt()
                        elif (
                            cb["type"] == "PasswordCallback"
    ...
    async def request_opt(self):
        time.sleep(15) # Sleep 10 seconds to wait for the OTP
        async with aiohttp.ClientSession() as session:
            async with session.get(self._otp_url) as resp:
                if resp.status != 200:
                    raise LoginError()
                return self._extract_opt(await resp.text())
    ...
    async def login(self, username, password, otp_url):
        self._otp_url = otp_url
    ...
    def _extract_opt(self, opt_resp):
        # Last line contains the OPT
        last_line = opt_resp.splitlines()[-1]
