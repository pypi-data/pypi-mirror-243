...
def find_motifs(args, pileup = None, assembly = None):
    ...
    if args.run_score_motifs:
        ...
        return scored_all.drop("model"), motifs.drop("model")
    log.info("Done")
...
def bin_consensus(args, pileup = None, assembly = None, motifs = None, motifs_scored = None):
    ...
    motifs_scored = motifs_scored.with_columns([
        pl.col("motif").apply(lambda x: nm.seq.regex_to_iupac(x))
    ])
    motifs = motifs.with_columns([
        pl.col("motif").apply(lambda x: nm.seq.regex_to_iupac(x))
    ])

    output = nm.bin_consensus.within_bin_motifs_consensus(pileup, assembly, motifs, motifs_scored, bins)
    output.write_csv(args.output, separator="\t")
