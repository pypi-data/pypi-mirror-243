...
class CallbackQueryHandler(Handler[Update, CCT]):
    """Handler class to handle TeleGenic callback queries. Optionally based on a regex.
    ...
    Note:
        * :attr:`pass_user_data` and :attr:`pass_chat_data` determine whether a ``dict`` you
          ...
          https://github.com/python-TeleGenic-bot/python-TeleGenic-bot/wiki\
        ...
        * If your bot allows arbitrary objects as ``callback_data``, it may happen that the
          original ``callback_data`` for the incoming :class:`TeleGenic.CallbackQuery`` can not be
          ...
          cases, an instance of :class:`TeleGenic.ext.InvalidCallbackData` will be set as
          ``callback_data``.
    ...
    Args:
        ...
        pattern (:obj:`str` | `Pattern` | :obj:`callable` | :obj:`type`, optional):
            Pattern to test :attr:`TeleGenic.CallbackQuery.data` against. If a string or a regex
            pattern is passed, :meth:`re.match` is used on :attr:`TeleGenic.CallbackQuery.data` to
            ...
            objects you may pass
                ...
                * a callable, accepting exactly one argument, namely the
                  :attr:`TeleGenic.CallbackQuery.data`. It must return :obj:`True` or
                  :obj:`False`/:obj:`None` to indicate, whether the update should be handled.
                * a :obj:`type`. If :attr:`TeleGenic.CallbackQuery.data` is an instance of that type
                  (or a subclass), the update will be handled.

            If :attr:`TeleGenic.CallbackQuery.data` is :obj:`None`, the
            :class:`TeleGenic.CallbackQuery` update will not be handled.
    ...
    Attributes:
        ...
        pattern (`Pattern` | :obj:`callable` | :obj:`type`): Optional. Regex pattern, callback or
            type to test :attr:`TeleGenic.CallbackQuery.data` against.
