...
class CallbackDataCache:
    """A custom cache for storing the callback data of a :class:`TeleGenic.ext.ExtBot`. Internally,
    ...
    Args:
        bot (:class:`TeleGenic.ext.ExtBot`): The bot this cache is for.
        maxsize (:obj:`int`, optional): Maximum number of items in each of the internal mappings.
            Defaults to 1024.
        persistent_data (:obj:`TeleGenic.ext.utils.types.CDCData`, optional): Data to initialize
            the cache with, as returned by :meth:`TeleGenic.ext.BasePersistence.get_callback_data`.
    ...
    def persistence_data(self) -> CDCData:
        """:obj:`TeleGenic.ext.utils.types.CDCData`: The data that needs to be persisted to allow
    ...
    def process_keyboard(self, reply_markup: InlineKeyboardMarkup) -> InlineKeyboardMarkup:
        ...
        Args:
            reply_markup (:class:`TeleGenic.InlineKeyboardMarkup`): The keyboard.

        Returns:
            :class:`TeleGenic.InlineKeyboardMarkup`: The keyboard to be passed to TeleGenic.
    ...
    def process_message(self, message: Message) -> None:
        ...
        :class:`TeleGenic.ext.InvalidCallbackData` will be inserted.

        Note:
            Checks :attr:`TeleGenic.Message.via_bot` and :attr:`TeleGenic.Message.from_user` to check
            if the reply markup (if any) was actually sent by this caches bot. If it was not, the
            message will be returned unchanged.

            Note that this will fail for channel posts, as :attr:`TeleGenic.Message.from_user` is
            :obj:`None` for those! In the corresponding reply markups the callback data will be
            replaced by :class:`TeleGenic.ext.InvalidCallbackData`.

        Warning:
            * Does *not* consider :attr:`TeleGenic.Message.reply_to_message` and
              :attr:`TeleGenic.Message.pinned_message`. Pass them to these method separately.
            * *In place*, i.e. the passed :class:`TeleGenic.Message` will be changed!

        Args:
            message (:class:`TeleGenic.Message`): The message.
    ...
    def process_callback_query(self, callback_query: CallbackQuery) -> None:
        ...
        Note:
            ...
            :attr:`TeleGenic.Message.reply_to_message` and :attr:`TeleGenic.Message.pinned_message`
            if necessary.

        Warning:
            *In place*, i.e. the passed :class:`TeleGenic.CallbackQuery` will be changed!
