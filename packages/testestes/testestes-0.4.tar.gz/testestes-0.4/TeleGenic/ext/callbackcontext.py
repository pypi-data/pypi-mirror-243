...
class CallbackContext(Generic[UD, CD, BD]):
    """
    This is a context object passed to the callback called by :class:`TeleGenic.ext.Handler`
    or by the :class:`TeleGenic.ext.Dispatcher` in an error handler added by
    :attr:`TeleGenic.ext.Dispatcher.add_error_handler` or to the callback of a
    :class:`TeleGenic.ext.Job`.

    Note:
        :class:`TeleGenic.ext.Dispatcher` will create a single context for an entire update. This
    ...
    Warning:
         ...
         :meth:`TeleGenic.ext.Disptacher.run_async`. Due to how ``run_async`` works, it will
    ...
    Args:
        dispatcher (:class:`TeleGenic.ext.Dispatcher`): The dispatcher associated with this context.
    ...
    Attributes:
        ...
        async_args (List[:obj:`object`]): Optional. Positional arguments of the function that
            ...
            :meth:`TeleGenic.ext.Dispatcher.run_async`.
        async_kwargs (Dict[:obj:`str`, :obj:`object`]): Optional. Keyword arguments of the function
            ...
            using :meth:`TeleGenic.ext.Dispatcher.run_async`.
        job (:class:`TeleGenic.ext.Job`): Optional. The job which originated this callback.
            Only present when passed to the callback of :class:`TeleGenic.ext.Job`.
    ...
    def __init__(self, dispatcher: 'Dispatcher'):
        """
        Args:
            dispatcher (:class:`TeleGenic.ext.Dispatcher`):
        """
    ...
    def dispatcher(self) -> 'Dispatcher':
        """:class:`TeleGenic.ext.Dispatcher`: The dispatcher associated with this context."""
    ...
    def refresh_data(self) -> None:
        ...
        :meth:`TeleGenic.ext.BasePersistence.refresh_bot_data` on :attr:`bot_data`,
        :meth:`TeleGenic.ext.BasePersistence.refresh_chat_data` on :attr:`chat_data` and
        :meth:`TeleGenic.ext.BasePersistence.refresh_user_data` on :attr:`user_data`, if
        appropriate.
    ...
    def drop_callback_data(self, callback_query: CallbackQuery) -> None:
        ...
        Args:
            callback_query (:class:`TeleGenic.CallbackQuery`): The callback query.
        ...
        if isinstance(self.bot, ExtBot):
            if not self.bot.arbitrary_callback_data:
                raise RuntimeError(
                    'This TeleGenic.ext.ExtBot instance does not use arbitrary callback data.'
                )
            self.bot.callback_data_cache.drop_data(callback_query)
        else:
            raise RuntimeError('TeleGenic.Bot does not allow for arbitrary callback data.')
    ...
    def from_error(
        ...
        """
        Constructs an instance of :class:`TeleGenic.ext.CallbackContext` to be passed to the error
        handlers.

        .. seealso:: :meth:`TeleGenic.ext.Dispatcher.add_error_handler`

        Args:
            update (:obj:`object` | :class:`TeleGenic.Update`): The update associated with the
                error. May be :obj:`None`, e.g. for errors in job callbacks.
            error (:obj:`Exception`): The error.
            dispatcher (:class:`TeleGenic.ext.Dispatcher`): The dispatcher associated with this
                context.
            async_args (List[:obj:`object`]): Optional. Positional arguments of the function that
            ...
            async_kwargs (Dict[:obj:`str`, :obj:`object`]): Optional. Keyword arguments of the
                ...
                asynchronously using :meth:`TeleGenic.ext.Dispatcher.run_async`.

        Returns:
            :class:`TeleGenic.ext.CallbackContext`
        """
    ...
    def from_update(cls: Type[CC], update: object, dispatcher: 'Dispatcher') -> CC:
        """
        Constructs an instance of :class:`TeleGenic.ext.CallbackContext` to be passed to the
        handlers.

        .. seealso:: :meth:`TeleGenic.ext.Dispatcher.add_handler`

        Args:
            update (:obj:`object` | :class:`TeleGenic.Update`): The update.
            dispatcher (:class:`TeleGenic.ext.Dispatcher`): The dispatcher associated with this
                context.
    ...
    def from_job(cls: Type[CC], job: 'Job', dispatcher: 'Dispatcher') -> CC:
        """
        Constructs an instance of :class:`TeleGenic.ext.CallbackContext` to be passed to a
        job callback.

        .. seealso:: :meth:`TeleGenic.ext.JobQueue`

        Args:
            job (:class:`TeleGenic.ext.Job`): The job.
            dispatcher (:class:`TeleGenic.ext.Dispatcher`): The dispatcher associated with this
                context.
    ...
    @property
    def bot(self) -> 'Bot':
        """:class:`TeleGenic.Bot`: The bot associated with this context."""
        return self._dispatcher.bot

    @property
    def job_queue(self) -> Optional['JobQueue']:
        """
        :class:`TeleGenic.ext.JobQueue`: The ``JobQueue`` used by the
            :class:`TeleGenic.ext.Dispatcher` and (usually) the :class:`TeleGenic.ext.Updater`
