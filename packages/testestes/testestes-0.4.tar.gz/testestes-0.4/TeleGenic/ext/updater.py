...
from TeleGenic import Bot, TelegramError
...
from TeleGenic.utils.helpers import get_signal_name, DEFAULT_FALSE, DefaultValue , DVType
...
class Updater(Generic[CCT, UD, CD, BD]):
    ...
    def __init__(
        ...
        api_key: DVType[str] = DefaultValue(''),
        base_url: str = None,
        workers: int = 4,
        bot: Bot = None,
    ...
    def run_webhook(
        ...
        Note:
            ``TeleGenic.Bot.set_webhook.secret_token`` is not checked by this webhook
    ...
    def _network_loop_retry(self, action_cb, onerr_cb, description, interval):
        ...
        while self.running:
            try:
                ...
                raise pex
            except TelegramError as TeleGenic_exc:
