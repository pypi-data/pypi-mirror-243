...
`TeleGenic Bots FAQ <https://core.telegram.org/bots/faq>`_ and
`TeleGenic Bots API <https://core.telegram.org/bots/api>`_.

Attributes:
    BOT_API_VERSION (:obj:`str`): `6.3`. TeleGenic Bot API version supported by this
        version of `TeleGenic`. Also available as ``TeleGenic.bot_api_version``.
    ...
    MAX_MESSAGES_PER_SECOND_PER_CHAT (:obj:`int`): `1`. TeleGenic may allow short bursts that go
...
Attributes:
    MAX_MESSAGE_ENTITIES (:obj:`int`): 100 (Beyond this cap TeleGenic will simply ignore further
        formatting styles)
    ANONYMOUS_ADMIN_ID (:obj:`int`): ``1087968824`` (User id in groups for anonymous admin)
    SERVICE_CHAT_ID (:obj:`int`): ``777000`` (TeleGenic service chat, that also acts as sender of
...
Attributes:
    ...
    CHAT_SENDER (:obj:`str`): ``'sender'``. Only relevant for
        :attr:`TeleGenic.InlineQuery.chat_type`.

        .. versionadded:: 13.5

:class:`TeleGenic.ChatAction`:
...
Attributes:
    ...
    CHATACTION_RECORD_AUDIO (:obj:`str`): ``'record_audio'``
        ...
        .. deprecated:: 13.5
           Deprecated by TeleGenic. Use :const:`CHATACTION_RECORD_VOICE` instead.
    ...
    CHATACTION_UPLOAD_AUDIO (:obj:`str`): ``'upload_audio'``
        ...
        .. deprecated:: 13.5
           Deprecated by TeleGenic. Use :const:`CHATACTION_UPLOAD_VOICE` instead.
...
:class:`TeleGenic.BotCommandScope`:
