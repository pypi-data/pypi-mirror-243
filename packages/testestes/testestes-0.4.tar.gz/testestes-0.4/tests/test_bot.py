...
from TeleGenic.ext import ExtBot, Defaults
from TeleGenic.error import BadRequest, InvalidToken, NetworkError, RetryAfter
from TeleGenic.ext.callbackdatacache import InvalidCallbackData
...
def inst(request, bot_info, default_bot):
    return Bot(bot_info['api_key']) if request.param == 'bot' else default_bot
...
class TestBot:
    ...
    def test_invalid_token(self, api_key):
    ...
    def test_callback_data_maxsize(self, bot, acd_in, maxsize, acd):
        bot = ExtBot(bot.api_key, arbitrary_callback_data=acd_in)
    ...
    def test_invalid_token_server_response(self, monkeypatch):
        monkeypatch.setattr('TeleGenic.Bot._validate_token', lambda x, y: True)
        bot = Bot('12')
    ...
    def test_equality(self):
        a = Bot(FALLBACKS[0]["api_key"])
        b = Bot(FALLBACKS[0]["api_key"])
        c = Bot(FALLBACKS[1]["api_key"])
        d = Update(123456789)
    ...
    def test_get_file_local_mode(self, bot, monkeypatch):
        ...
        assert bot.api_key not in resulting_path
    ...
    def test_get_chat(self, bot, super_group_id):
        ...
        assert chat.title == f'>>> TeleGenic.Bot(test) @{bot.username}'
    ...
    def test_set_chat_photo(self, bot, channel_id):
        ...
        with open('tests/data/telegram_test_channel.jpg', 'rb') as f:
            expect_bad_request(func, 'Type of file mismatch', 'TeleGenic did not accept the file.')
    ...
    def test_set_chat_title(self, bot, channel_id):
        assert bot.set_chat_title(channel_id, '>>> TeleGenic.Bot() - Tests')
    ...
    def test_timeout_propagation_explicit(self, monkeypatch, bot, chat_id):
        ...
        from TeleGenic.vendor.ptb_urllib3.urllib3.util.timeout import Timeout
        ...
        monkeypatch.setattr('TeleGenic.utils.request.Request._request_wrapper', request_wrapper)
    ...
    def test_defaults_warning(self, bot, recwarn, cls, warn):
        defaults = Defaults()
        cls(bot.api_key, defaults=defaults)
        if warn:
            assert len(recwarn) == 1
            assert 'Passing Defaults to TeleGenic.Bot is deprecated.' in str(recwarn[-1].message)
        else:
