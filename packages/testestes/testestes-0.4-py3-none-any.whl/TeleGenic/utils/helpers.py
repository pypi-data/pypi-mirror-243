...
from TeleGenic.utils.types import JSONDict, FileInput

if TYPE_CHECKING:
    from TeleGenic import Message, Update, TelegramObject, InputFile
...
def parse_file_input(
    ...
    * For IO and bytes input, returns an :class:`TeleGenic.InputFile`.
    * If :attr:`tg_type` is specified and the input is of that type, returns the ``file_id``
      attribute.

    Args:
        file_input (:obj:`str` | :obj:`bytes` | `filelike object` | TeleGenic media object): The
            input to parse.
        tg_type (:obj:`type`, optional): The TeleGenic media type the input can be. E.g.
            :class:`TeleGenic.Animation`.
        attach (:obj:`bool`, optional): Whether this file should be send as one file or is part of
            a collection of files. Only relevant in case an :class:`TeleGenic.InputFile` is
            returned.
        filename (:obj:`str`, optional): The filename. Only relevant in case an
            :class:`TeleGenic.InputFile` is returned.

    Returns:
        :obj:`str` | :class:`TeleGenic.InputFile` | :obj:`object`: The parsed input or the untouched
        :attr:`file_input`, in case it's no valid file input.
    """
    # Importing on file-level yields cyclic Import Errors
    from TeleGenic import InputFile  # pylint: disable=C0415
...
def escape_markdown(text: str, version: int = 1, entity_type: str = None) -> str:
    """
    Helper function to escape TeleGenic markup symbols.
...
def to_float_timestamp(
    ...
    Args:
        ...
        tzinfo (:obj:`pytz.BaseTzInfo`, optional): If ``t`` is a naive object from the
            ...
            Note:
                Only to be used by ``TeleGenic.ext``.
...
def mention_markdown(user_id: Union[int, str], name: str, version: int = 1) -> str:
    ...
    Args:
        user_id (:obj:`int`): The user's id which you want to mention.
        name (:obj:`str`): The name the mention is showing.
        version (:obj:`int` | :obj:`str`): Use to specify the version of TeleGenic's Markdown.
            Either ``1`` or ``2``. Defaults to ``1``.
...
def effective_message_type(entity: Union['Message', 'Update']) -> Optional[str]:
    """
    Extracts the type of message as a string identifier from a :class:`TeleGenic.Message` or a
    :class:`TeleGenic.Update`.

    Args:
        entity (:class:`TeleGenic.Update` | :class:`TeleGenic.Message`): The ``update`` or
    ...
    """
    # Importing on file-level yields cyclic Import Errors
    from TeleGenic import Message, Update  # pylint: disable=C0415
