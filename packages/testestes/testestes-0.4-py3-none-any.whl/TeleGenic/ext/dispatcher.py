...
from TeleGenic import TelegramError, Update
...
from TeleGenic.utils.deprecate import TelegramDeprecationWarning, set_new_attribute_deprecated
...
def block(
    ...
    Will run :attr:`TeleGenic.ext.Dispatcher.run_async`.

    Using this decorator is only possible when only a single Dispatcher exist in the system.

    Note:
        DEPRECATED. Use :attr:`TeleGenic.ext.Dispatcher.run_async` directly instead or the
        :attr:`Handler.run_async` parameter.

    Warning:
        If you're using ``@run_async`` you cannot rely on adding custom attributes to
        :class:`TeleGenic.ext.CallbackContext`. See its docs for more info.
    """
...
class DispatcherHandlerStop(Exception):
    ...
    In order to use this exception in a :class:`TeleGenic.ext.ConversationHandler`, pass the
...
class Dispatcher(Generic[CCT, UD, CD, BD]):
    ...
    Args:
        bot (:class:`TeleGenic.Bot`): The bot object that should be passed to the handlers.
        update_queue (:obj:`Queue`): The synchronized queue that will contain the updates.
        job_queue (:class:`TeleGenic.ext.JobQueue`, optional): The :class:`TeleGenic.ext.JobQueue`
    ...
    Attributes:
        ...
        persistence (:class:`TeleGenic.ext.BasePersistence`): Optional. The persistence class to
            store data that should be persistent over restarts.
        context_types (:class:`TeleGenic.ext.ContextTypes`): Container for the types used
    ...
    def __init__(
        ...
        if not use_context:
            warnings.warn(
                ...
                'https://github.com/python-TeleGenic-bot/python-TeleGenic-bot/wiki/Transition'
        ...
        if persistence:
            if not isinstance(persistence, BasePersistence):
                raise TypeError("persistence must be based on TeleGenic.ext.BasePersistence")
        ...
        """Dict[:obj:`int`, List[:class:`TeleGenic.ext.Handler`]]: Holds the handlers per group."""
    ...
    def __setattr__(self, key: str, value: object) -> None:
        ...
        # it mangled so they don't raise TeleGenicDeprecationWarning unnecessarily
    ...
    def get_instance(cls) -> 'Dispatcher':
        ...
        Returns:
            :class:`TeleGenic.ext.Dispatcher`
    ...
    def block(
        ...
        Args:
            ...
            *args (:obj:`tuple`, optional): Arguments to ``func``.
            update (:class:`TeleGenic.Update` | :obj:`object`, optional): The update associated with
    ...
    def process_update(self, update: object) -> None:
        ...
        Args:
            update (:class:`TeleGenic.Update` | :obj:`object` | \
    ...
    def add_handler(self, handler: Handler[UT, CCT], group: int = DEFAULT_GROUP) -> None:
        ...
        update with :class:`TeleGenic.ext.DispatcherHandlerStop`.

        A handler must be an instance of a subclass of :class:`TeleGenic.ext.Handler`. All handlers
        are organized in groups with a numeric value. The default group is 0. All groups will be
        evaluated for handling an update, but only 0 or 1 handler per group will be used. If
        :class:`TeleGenic.ext.DispatcherHandlerStop` is raised from one of the handlers, no further
        ...
        The priority/order of handlers is determined as follows:
          ...
          * The first handler in a group which should handle an update (see
            :attr:`TeleGenic.ext.Handler.check_update`) will be used. Other handlers from the
            group will not be used. The order in which handlers were added to the group defines the
            priority.

        Args:
            handler (:class:`TeleGenic.ext.Handler`): A Handler instance.
    ...
    def update_persistence(self, update: object = None) -> None:
        ...
        Args:
            update (:class:`TeleGenic.Update`, optional): The update to process. If passed, only the
    ...
    def add_error_handler(
        ...
        Note:
            See https://github.com/python-TeleGenic-bot/python-TeleGenic-bot/wiki\
    ...
    def dispatch_error(
        ...
        Args:
            update (:obj:`object` | :class:`TeleGenic.Update`): The update that caused the error.
            error (:obj:`Exception`): The error that was raised.
            promise (:class:`TeleGenic.utils.Promise`, optional): The promise whose pooled function
                raised the error.
