...
class DelayQueue(threading.Thread):
    ...
    .. deprecated:: 13.3
       :class:`TeleGenic.ext.DelayQueue` in its current form is deprecated and will be reinvented
       in a future release. See `this thread <https://github.com/python-TeleGenic-bot/\
       python-TeleGenic-bot/issues/2139>`_ for a list of known bugs.
    ...
    def __init__(
        ...
        warnings.warn(
            ...
            'release. See https://github.com/python-TeleGenic-bot/python-TeleGenic-bot/issues/2139 '
...
class MessageQueue:
    """
    Implements callback processing with proper delays to avoid hitting TeleGenic's message limits.
    ...
    .. deprecated:: 13.3
       :class:`TeleGenic.ext.MessageQueue` in its current form is deprecated and will be reinvented
       in a future release. See `this thread <https://github.com/python-TeleGenic-bot/\
       python-TeleGenic-bot/issues/2139>`_ for a list of known bugs.
    ...
    def __call__(self, promise: Callable, is_group_msg: bool = False) -> Callable:
        ...
        Args:
            promise (:obj:`callable`): Mainly the ``TeleGenic.utils.promise.Promise`` (see Notes for
        ...
        Note:
            Method is designed to accept ``TeleGenic.utils.promise.Promise`` as ``promise``
...
def queuedmessage(method: Callable) -> Callable:
    """A decorator to be used with :attr:`TeleGenic.Bot` send* methods.
    ...
    Attributes:
        ...
        self._msg_queue (:class:`TeleGenic.ext.messagequeue.MessageQueue`): The actual
    ...
    Returns:
        ``TeleGenic.utils.promise.Promise``: In case call is queued or original method's return
        value if it's not.
