...
class ChosenInlineResultHandler(Handler[Update, CCT]):
    """Handler class to handle TeleGenic updates that contain a chosen inline result.
    ...
    Args:
        ...
        pattern (:obj:`str` | `Pattern`, optional): Regex pattern. If not :obj:`None`, ``re.match``
            is used on :attr:`TeleGenic.ChosenInlineResult.result_id` to determine if an update
            should be handled by this handler. This is accessible in the callback as
            :attr:`TeleGenic.ext.CallbackContext.matches`.
    ...
    Attributes:
        ...
        pattern (`Pattern`): Optional. Regex pattern to test
            :attr:`TeleGenic.ChosenInlineResult.result_id` against.
