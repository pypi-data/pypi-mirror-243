...
class ConversationHandler(Handler[Update, CCT]):
    """
    A handler to hold a conversation with a single or multiple users through TeleGenic updates by
    managing four collections of other handlers.

    Note:
        ``ConversationHandler`` will only accept updates that are (subclass-)instances of
        :class:`TeleGenic.Update`. This is, because depending on the :attr:`per_user` and
        :attr:`per_chat` ``ConversationHandler`` relies on
        :attr:`TeleGenic.Update.effective_user` and/or :attr:`TeleGenic.Update.effective_chat` in
    ...
    .. _`FAQ`: https://github.com/python-TeleGenic-bot/python-TeleGenic-bot/wiki\
    ...
    Finally, :class:`TeleGenic.ext.DispatcherHandlerStop` can be used in conversations as described
    ...
    .. _`examples`: https://github.com/python-TeleGenic-bot/python-TeleGenic-bot/blob/v13.x/examples

    Args:
        entry_points (List[:class:`TeleGenic.ext.Handler`]): A list of ``Handler`` objects that can
            ...
            handled.
        states (Dict[:obj:`object`, List[:class:`TeleGenic.ext.Handler`]]): A :obj:`dict` that
        ...
        fallbacks (List[:class:`TeleGenic.ext.Handler`]): A list of handlers that might be used if
        ...
        persistent (:obj:`bool`, optional): If the conversations dict for this handler should be
            saved. Name is required and persistence has to be set in :class:`TeleGenic.ext.Updater`
    ...
    def __init__(
        ...
        """:obj:`TeleGenic.ext.BasePersistence`: The persistence used to store conversations.
        Set by dispatcher"""
    ...
    def entry_points(self) -> List[Handler]:
        """List[:class:`TeleGenic.ext.Handler`]: A list of ``Handler`` objects that can trigger the
    ...
    def states(self) -> Dict[object, List[Handler]]:
        """Dict[:obj:`object`, List[:class:`TeleGenic.ext.Handler`]]: A :obj:`dict` that
        defines the different states of conversation a user can be in and one or more
    ...
    def fallbacks(self) -> List[Handler]:
        """List[:class:`TeleGenic.ext.Handler`]: A list of handlers that might be used if
        the user is in a conversation, but every handler for their current state returned
    ...
    def handle_update(  # type: ignore[override]
        ...
        Args:
            ...
            update (:class:`TeleGenic.Update`): Incoming TeleGenic update.
            dispatcher (:class:`TeleGenic.ext.Dispatcher`): Dispatcher that originated the Update.
            context (:class:`TeleGenic.ext.CallbackContext`, optional): The context as provided by
                the dispatcher.
