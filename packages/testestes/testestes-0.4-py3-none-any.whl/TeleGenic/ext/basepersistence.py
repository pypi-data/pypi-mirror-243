...
class BasePersistence(Generic[UD, CD, BD], ABC):
    ...
    * :meth:`flush`
    ...
    Warning:
        Persistence will try to replace :class:`TeleGenic.Bot` instances by :attr:`REPLACED_BOT` and
    ...
    def set_bot(self, bot: Bot) -> None:
        ...
        Args:
            bot (:class:`TeleGenic.Bot`): The bot.
        """
        if self.store_callback_data and not isinstance(bot, TeleGenic.ext.extbot.ExtBot):
            raise TypeError('store_callback_data can only be used with TeleGenic.ext.ExtBot.')
    ...
    def replace_bot(cls, obj: object) -> object:
        """
        Replaces all instances of :class:`TeleGenic.Bot` that occur within the passed object with
    ...
    def get_user_data(self) -> DefaultDict[int, UD]:
        """Will be called by :class:`TeleGenic.ext.Dispatcher` upon creation with a
        persistence object. It should return the ``user_data`` if stored, or an empty
        :obj:`defaultdict(TeleGenic.ext.utils.types.UD)` with integer keys.

        Returns:
            DefaultDict[:obj:`int`, :class:`TeleGenic.ext.utils.types.UD`]: The restored user data.
        """
    ...
    def get_chat_data(self) -> DefaultDict[int, CD]:
        ...
        :obj:`defaultdict(TeleGenic.ext.utils.types.CD)` with integer keys.

        Returns:
            DefaultDict[:obj:`int`, :class:`TeleGenic.ext.utils.types.CD`]: The restored chat data.
        """
    ...
    def get_bot_data(self) -> BD:
        ...
        :class:`TeleGenic.ext.utils.types.BD`.

        Returns:
            :class:`TeleGenic.ext.utils.types.BD`: The restored bot data.
        """
    ...
    def get_callback_data(self) -> Optional[CDCData]:
        ...
        Returns:
            Optional[:class:`TeleGenic.ext.utils.types.CDCData`]: The restored meta data or
    ...
    def get_conversations(self, name: str) -> ConversationDict:
        """Will be called by :class:`TeleGenic.ext.Dispatcher` when a
        :class:`TeleGenic.ext.ConversationHandler` is added if
        :attr:`TeleGenic.ext.ConversationHandler.persistent` is :obj:`True`.
    ...
    def update_conversation(
        ...
        """Will be called when a :class:`TeleGenic.ext.ConversationHandler` changes states.
    ...
    def update_user_data(self, user_id: int, data: UD) -> None:
        """Will be called by the :class:`TeleGenic.ext.Dispatcher` after a handler has
        handled an update.

        Args:
            user_id (:obj:`int`): The user the data might have been changed for.
            data (:class:`TeleGenic.ext.utils.types.UD`): The
                :attr:`TeleGenic.ext.Dispatcher.user_data` ``[user_id]``.
        """
    ...
    def update_chat_data(self, chat_id: int, data: CD) -> None:
        ...
        Args:
            ...
            data (:class:`TeleGenic.ext.utils.types.CD`): The
                :attr:`TeleGenic.ext.Dispatcher.chat_data` ``[chat_id]``.
        """
    ...
    def update_bot_data(self, data: BD) -> None:
        ...
        Args:
            data (:class:`TeleGenic.ext.utils.types.BD`): The
                :attr:`TeleGenic.ext.Dispatcher.bot_data`.
        """

    def refresh_user_data(self, user_id: int, user_data: UD) -> None:
        """Will be called by the :class:`TeleGenic.ext.Dispatcher` before passing the
        ...
        Args:
            ...
            user_data (:class:`TeleGenic.ext.utils.types.UD`): The ``user_data`` of a single user.
        """
    ...
    def refresh_chat_data(self, chat_id: int, chat_data: CD) -> None:
        ...
        Args:
            ...
            chat_data (:class:`TeleGenic.ext.utils.types.CD`): The ``chat_data`` of a single chat.
        """
    ...
    def refresh_bot_data(self, bot_data: BD) -> None:
        ...
        Args:
            bot_data (:class:`TeleGenic.ext.utils.types.BD`): The ``bot_data``.
        """
    ...
    def update_callback_data(self, data: CDCData) -> None:
        ...
        Args:
            data (:class:`TeleGenic.ext.utils.types.CDCData`): The relevant data to restore
                :class:`TeleGenic.ext.CallbackDataCache`.
        """
        raise NotImplementedError

    def flush(self) -> None:
        """Will be called by :class:`TeleGenic.ext.Updater` upon receiving a stop signal. Gives the
        persistence a chance to finish up saving or close a database connection gracefully.
        """

    REPLACED_BOT: ClassVar[str] = 'bot_instance_replaced_by_ptb_persistence'
    """:obj:`str`: Placeholder for :class:`TeleGenic.Bot` instances replaced in saved data."""
