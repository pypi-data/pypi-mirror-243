    ...
    def __call__(self, assay: xr.Dataset) -> xr.Dataset:
        ...
        for t in assay.time:
            for search_channel in self.search_channels:
                ...
                edges = 255 - cv.adaptiveThreshold(utils.to_uint8(grad),255,cv.ADAPTIVE_THRESH_GAUSSIAN_C,
                                           cv.THRESH_BINARY, 2 * self.max_bead_radius + 1, -2)
                ...
                kernel = cv.getStructuringElement(
                    cv.MORPH_ELLIPSE, (edge_width, edge_width)
                )
                ...
                dist = cv.distanceTransform(utils.to_uint8(bg_area), cv.DIST_L2, cv.DIST_MASK_PRECISE)
                dx = cv.Scharr(dist, ddepth=cv.CV_32F, dx=1, dy=0)
                dy = cv.Scharr(dist, ddepth=cv.CV_32F, dx=0, dy=1)
                grad = np.sqrt(dx ** 2 + dy ** 2)
                max_mask = np.zeros_like(dist).astype(np.int16)
                for i in range(1, l.max() + 1):
                    if is_fg[i]:
                        c = np.unravel_index(np.argmin(10000 * (1 - ((l == i) & (dist >= self.min_bead_radius))) + ((l == i) & (grad >= self.min_bead_radius)) * grad, axis=None), shape=l.shape)
                        if dist[c[0], c[1]] < self.min_bead_radius or dist[c[0], c[1]] > self.max_bead_radius:
                            continue
                        print(dist[c[0], c[1]])
                ...
                print(max_mask)
                # plt.imshow(image)
                # plt.imshow(max_mask, alpha=0.5)
                fig = px.imshow(fg_area)
                fig.show()
                return
                ...
                for i in range(len(max_idxs)):
                    l += utils.circle(max_mask.shape[0], c[0][i], c[1][i], round(local_max[c[0][i], c[1][i]]), i + 1).astype(np.uint16) * (l == 0)
