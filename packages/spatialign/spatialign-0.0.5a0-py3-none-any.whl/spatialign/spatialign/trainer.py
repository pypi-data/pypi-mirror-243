...
class Spatialign:
    ...
    def __init__(self,
        ...
        self.dataset = Dataset(merge_data,
                               batch_key=batch_key,
                               is_hvg=is_hvg,
                               is_reduce=is_reduce,
                               n_pcs=n_pcs,
    ...
    def train(self,
        ...
        for eph in range(max_epoch):
            ...
            for idx, data in enumerate(self.dataset.loader_list):
                ...
                for i in np.delete(range(self.dataset.n_domain), idx):
    ...
    def alignment(self):
        ...
        for idx, dataset in enumerate(self.dataset.loader_list):
            ...
            data.obs = self.dataset.merge_data[self.dataset.merge_data.obs["batch"] == str(idx)].obs

            data.obsm["spatial"] = dataset.pos.detach().cpu().numpy()
            data.obs.index = self.dataset.merge_data[self.dataset.merge_data.obs["batch"] == str(idx)].obs.index
        ...
        return AnnData.concatenate(*data_list)
