...
from typing import Any,List,Optional,Protocol,Tuple,TypedDict,Union,cast
...
class LicenseStaleError(LicenseActivationError):
	...
	def __init__(A,license,msg=_B,*B):super().__init__(msg,*B);A.license=license
...
@dataclasses.dataclass(kw_only=_A)
class License:
	id:str;license_format:str;signature:Optional[str];offline_data:dict[str,str]=dataclasses.field(default_factory=dict)
	def copy(A):return copy.deepcopy(A)
	def to_log_string(A):return A.id
@dataclasses.dataclass(kw_only=_A)
class LicenseV1(License):
	license_type:str;issue_date:datetime;expiry_date:datetime;products:List[ProductInfo]=dataclasses.field(default_factory=list);license_status:LicenseStatus=LicenseStatus.UNKNOWN;license_secret:Optional[str]=_B;last_activated:Optional[datetime]=_B;reactivate_after:Optional[datetime]=_B
...
class LicenseSerializer:
	def serialize(B,license):
		...
		if license.license_format=='1':A=dataclasses.asdict(license);A.pop(_I,_B);return json.dumps(A,cls=_LicenseJsonEncoder,indent=2).encode(_D)
...
class LicenseV1ClientBase(LicensingClient):
	...
	def current_product_version_matches(G,license_):
		...
		try:
			...
			if A.products[0][_F].startswith('*'):return _A
			D=B[0],B[1];E=C[0],C[1]
...
class LicensedLocalstackEnvironment:
	...
	def __init__(A,client,parser=_B,serializer=_B):A.client=client;A.parser=parser or LicenseParser();A.serializer=serializer or LicenseSerializer();A.license=_B;A.license_file_path=_B;A._activated=_C;A._mutex=threading.RLock()
	@property
	...
	def activate_license(A,offline_only=_C):
		D=offline_only;C=A.require_valid_credentials();license=_B
		try:
			try:
		...
		if license:
			try:license=A.client.activate_license_online(C,license);A.client.validate_license(C,license);A._activated=_A;A.license=license;LOG.info('Successfully activated license %s 🔑✅',license.to_log_string());A.save_license();return
			except LicensingError as B:LOG.debug('There was an error activating the license: %s. Trying to get a new one.',B)
		license=A.client.request_new_license(C);A.client.validate_license(C,license);A._activated=_A;A.license=license;LOG.info('Successfully requested and activated new license %s 🔑✅',license.to_log_string());A.save_license()
	def _try_activate_offline(A,credentials):
		C=[]
		for B in A.get_license_file_read_locations():
			if not os.path.isfile(B):continue
			try:
				with open(B,'rb')as E:license=A.parser.parse(E.read());A.client.activate_license_offline(credentials,license);A._activated=_A;return license,B
	...
	def has_product_license(A,product_name):
		...
		for B in A.license.products:
			if'*'in B[_E]:
				if fnmatch.fnmatch(C,B[_E]):return _A
			elif B[_E]==C:return _A
		return _C
	...
	def is_decryption_enabled(A):
		try:from localstack_ext.utils.decryption_check import decryption_check;return _A
	...
	def save_license(A):
		...
		try:
			os.makedirs(os.path.dirname(B),exist_ok=_A)
...
class DevLocalstackEnvironment(LicensedLocalstackEnvironment):
	def activate_license(A,offline_only=_C):LOG.debug('Using test license, skipping activation.');A._activated=_A
	def has_product_license(A,product_name):return _A
...
class LicensedPluginLoaderGuard(PluginLifecycleListener):
	def __init__(A,environment=_B):A.environment=environment or get_licensed_environment()
...
def configure_container_licensing(cfg):
	...
	if os.path.exists(C):E=str(PurePosixPath(config.Directories.for_container().config)/LICENSE_FILE_NAME);B.volumes.add(D(C,E,read_only=_A))