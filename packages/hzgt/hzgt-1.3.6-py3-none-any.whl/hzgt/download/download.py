...
try:
    ...
    from hzgt import getUrlFileSize, Bit_Unit_Conversion
except ImportError as err:
...
def UrlFileDownload(url, savepath="download_Files\\urldownload"):
    try:
        ...
        endfilename = f"{savepath}\\{_endfilename}"
        ...
        if url_file_size[2] - temp_size > 0:
            ...
            with open(endfilename, 'ab') as file, \
                    tqdm(desc='下载中', total=url_file_size[2] - temp_size, colour='#66CDAA',
                         unit='b', unit_scale=True, unit_divisor=1024, ncols=120) as bar:
...
def downloadmain():
    filepaths = ["download_Files\\urldownload", "download_Files\\git", "download_Files\\youget"]
    for fp in filepaths:
