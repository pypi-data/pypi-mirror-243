...
class VodService(VodServiceConfig):
    ...
    def upload_tob(self, space_name, file_path, file_type, file_name):
        ...
        if file_size < MinChunkSize:
            ...
            logger.info("[vop-upload]start chunk upload, storeUir: {}, file_path: {}".format(oid, file_path))
    ...
    def chunk_upload(self, file_path, host, oid, auth, size, is_large_file):
        ...
        logger.info("[vop-upload]init upload part success, storeUri:{}, upload_id:{}".format(oid, upload_id))
        n = size // MinChunkSize
        last_num = n - 1
        parts = []
        with open(file_path, 'rb') as f:
            for i in range(0, last_num):
                ...
                if is_large_file:
                    part_number = i + 1
                logger.info("[vop-upload] start to upload {}/{} part, upload_id: {}".format(part_number, n, upload_id))
                part = self.upload_part(host, oid, auth, upload_id, part_number, data, is_large_file)
                logger.info("[vop-upload] upload {}/{} part success, upload_id: {}".format(part_number, n, upload_id))
                parts.append(part)
            data = f.read()
            if is_large_file:
                last_num = last_num + 1
            logger.info("[vop-upload] start to upload {}/{} part, upload_id: {}".format(last_num, n, upload_id))
            part = self.upload_part(host, oid, auth, upload_id, last_num, data, is_large_file)
            logger.info("[vop-upload] upload {}/{} part success, upload_id: {}".format(last_num, n, upload_id))
            parts.append(part)
            logger.info("[vop-upload] start to merge part, upload_id: {}".format(upload_id))
    ...
    def upload_merge_part(self, host, oid, auth, upload_id, check_sum_list, is_large_file):
        ...
        if resp.get('success') is None or resp['success'] != 0:
            raise Exception("init upload error")
        logger.info("[vop-upload] merge part success, upload_id:{}".format(upload_id))
