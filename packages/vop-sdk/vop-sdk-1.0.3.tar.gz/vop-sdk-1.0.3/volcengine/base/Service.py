...
class Service(object):
    ...
    def init(self):
        if 'VOP_ACCESSKEY' in os.environ and 'VOP_SECRETKEY' in os.environ:
            self.service_info.credentials.set_ak(os.environ['VOP_ACCESSKEY'])
            self.service_info.credentials.set_sk(os.environ['VOP_SECRETKEY'])
        else:
            if os.environ.get('HOME', None) is None:
                return
            # 先尝试从credentials中读取ak、sk，credentials不存在则从config中读取
            path_ini = os.environ['HOME'] + '/.vop/credentials'
            path_json = os.environ['HOME'] + '/.vop/config'
            if os.path.isfile(path_ini):
                conf = configparser.ConfigParser()
                conf.read(path_ini)
                default_section, ak_option, sk_option, host_option = "default", "VOP_ACCESSKEY", "VOP_SECRETKEY", "VOP_HOST"
                if conf.has_section(default_section):
                    ...
                    if conf.has_option(default_section, host_option):
                        host = conf.get(default_section, host_option)
                        self.set_host(host)
                ...
                with open(path_json, 'r') as f:
                    ...
                    if 'host' in j:
                        self.set_host(j['host'])
        if 'VOP_HOST' in os.environ and os.environ['VOP_HOST'] != '':
            self.set_host(os.environ['VOP_HOST'])
