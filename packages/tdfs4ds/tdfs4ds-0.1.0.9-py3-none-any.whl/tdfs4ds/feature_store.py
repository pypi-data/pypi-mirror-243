...
def build_dataset(entity_id, selected_features, view_name,
    ...
    if type(entity_id) == dict:
        ...
        ENTITY_ID_ = ','.join(['COALESCE('+','.join(['AA'+str(i+1)+'.'+k for i,c in enumerate(selected_features)])+') as '+k for k in entity_id.keys()])
    elif type(entity_id) == list:
        ENTITY_NAMES = ','.join([k for k in entity_id])
        ENTITY_ID = ', \n'.join([k for k in entity_id])
        ENTITY_ID_ = ','.join(['COALESCE('+','.join(['AA'+str(i+1)+'.'+k for i,c in enumerate(selected_features)])+') as '+k for k in entity_id])
    elif type(entity_id) == str:
        ENTITY_NAMES = entity_id
        ENTITY_ID = entity_id
        ENTITY_ID_ = 'COALESCE('+','.join(['AA'+str(i+1)+'.'+entity_id for i,c in enumerate(selected_features)])+') as '+entity_id
