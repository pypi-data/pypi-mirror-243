...
class GameObjectUI(AbstractUIComponent):
    ...
    def on_value_set(self):
        new_value = self.before_set(dpg.get_value(self.dpg_tag_setter))
...
class TeleportUI(AbstractUIComponent):
    ...
    def _tp_add_dash_hotkeys_if_needed(self):
        if self.tp.movement_vector_updater_script is None or self.hotkey_dash is None:
            return
