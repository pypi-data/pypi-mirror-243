...
class IniUts():
    ...
    def format_data(self,dtClass,k,v):
        ...
        if cls == bool:
            val = v.strip().lower()
            v = True if val and val in ['true','1'] else False
            v = False if val in ['false','','0'] else True
        ...
        if not self.in_prd:
            for k in MissingKeysFromClass(dt):
                if not k in dt2.keys():
                    if not skip_missing:
                        raise Exception(f"Cound not find '{MissingKeysFromClass}' keys at section '{section}' in ini file")
                    continue
